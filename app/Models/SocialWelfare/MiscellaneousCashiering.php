...
class MiscellaneousCashiering extends Model
{
    public function getTransactions($id){
        $sql = DB::table('cto_top_transactions as ctt')->join('cto_top_bplo as ctb','ctb.id', '=', 'ctt.transaction_ref_no')->select('ctt.id','ctt.transaction_no')->where('tfoc_is_applicable',9);
        if($id<=0){
            $sql->where('is_paid','0');
        }
        return $sql->get();
    ...
    public function gettopnoAjax($search=""){
      ...
      $sql = DB::table('cto_top_transactions as ctt')->join('cto_top_bplo as ctb','ctb.id', '=', 'ctt.transaction_ref_no')
      ...
      $sql->where('is_paid','0')->where('tfoc_is_applicable',9); 
    ...
    public function getLatestOrNumber($ortype_id){
       ...
       return DB::table('cto_cashier')->select('or_no')->whereIN('tfoc_is_applicable',[1,9])->where('ortype_id',$ortype_id)->where('created_by',$createdBy)->orderby('id','DESC')->first();
    }
    
    public function getPreviousIssueNumber(){
        return DB::table('cto_cashier')->select('cashier_issue_no')->where('tfoc_is_applicable',9)->where('cashier_year',date("Y"))->orderby('id','DESC')->first();
    }
    ...
    public function getCertificateDetails($id){
        ...
        ->leftjoin('citizens AS c', 'c.id', '=', 'cc.client_citizen_id') 
            ->select('cc.*','c.cit_fullname','c.cit_last_name','c.cit_first_name','payee_type','c.cit_middle_name','c.cit_suffix_name','c.cit_full_address')->where('cc.id',$id)->first();
    }

    public function getCertificateDetailsfortaxpayer($id){
         return DB::table('cto_cashier AS cc')
        ->join('clients AS c', 'c.id', '=', 'cc.client_citizen_id') 
            ->select('cc.*','c.full_name as cit_fullname')->where('cc.id',$id)->first();
    }
    ...
    public function getTaxFees($payeetype=0){
        $sql= DB::table('cto_tfocs AS ctot')
           ...
           ->select('ctot.id','aas.description','tfoc_status')->where('tfoc_is_applicable','9')->where('tfoc_status',1);;
            if($payeetype==2){
    ...
    public function getTaxFeesEdit($id=0){
        $sql= DB::table('cto_tfocs AS ctot')
           ->join('acctg_account_subsidiary_ledgers AS aas', 'aas.id', '=', 'ctot.sl_id')
           ->select('ctot.id','aas.description','tfoc_status')->where('tfoc_is_applicable','9');
    ...
    public function getTaxFeesDetails($id=0){
        return DB::table('cto_tfocs AS ctot')
          ...
          ->where('tfoc_is_applicable','9')->where('ctot.id',$id)->first();
    }
    ...
    public function GetReqiestfeesdefault($id){
        return DB::table('ho_req_per_details as hrpd')->leftjoin('ho_services as hs','hrpd.service_id','=','hs.id')->select('hrpd.id','hs.ho_service_name','hrpd.permit_fee','hrpd.tfoc_id')->where('req_permit_id',$id)->orderby('id','ASC')->get();
    }

    public function getUserDetailsbytopid($topid){
        return DB::table('cto_top_transactions as ctt')->leftjoin('ho_request_permit as hrp','ctt.transaction_ref_no','=','hrp.id')->leftjoin('citizens as ct','ct.id','=','hrp.payor_id')->select('hrp.id as hlrid','hrp.request_amount','hrp.payor_id as cit_id','cit_last_name','cit_first_name','cit_middle_name','cit_suffix_name','cit_full_address')->where('ctt.id',$topid)->first();
    }
    ...
    public function getList($request){
        ...
        $sql = DB::table('cto_cashier AS cc')
            ...
            ->select('cc.*','he.fullname','c.full_name','ct.cit_fullname','c.rpo_first_name','c.rpo_custom_last_name','c.rpo_middle_name','c.suffix','cit_last_name','cit_first_name','cit_middle_name','cit_suffix_name');
        $sql->where('tfoc_is_applicable','=','9');  
        if ($status == '3') {
            } else {
        ...
        if(!empty($q) && isset($q)){
            $sql->where(function ($sql) use($q) {
				...
				->orWhere(DB::raw('LOWER(c.full_name)'),'like',"%".strtolower($q)."%")->orWhere(DB::raw('LOWER(ct.cit_fullname)'),'like',"%".strtolower($q)."%")
