...
{
    public $table = 'ho_blood_sugar_tests';
    ...
    public function getFields($id){
        return HealthSafetySetupDataService::select('ho_service_name')->where([['id',$id],['ho_is_active',1]])->first();
    }
    ...
    public function updateActiveInactive($id,$columns){
        return DB::table('ho_blood_sugar_tests')->where('id',$id)->update($columns);
    }  
    public function updateData($id,$columns){
        return DB::table('ho_blood_sugar_tests')->where('id',$id)->update($columns);
    }
    public function addData($postdata){
        DB::table('ho_blood_sugar_tests')->insert($postdata);
        return DB::getPdo()->lastInsertId();
    }
    
    public function getEditDetails($id){
        return DB::table('ho_blood_sugar_tests')->where('id',$id)->first();
    }
	...
	public function getDesignation($employee_id){
        try {
            ...
            ->select('hr_designations.describsion', 'hr_employees.identification_no AS licence_no')
                ->first();
        } catch (\Excebsion $e) {
            return ($e->getMessage());
        }
    ...
    public function getList($request){
        ...
        $sql = self::
              join('citizens AS cit', 'cit.id', '=', 'ho_blood_sugar_tests.cit_id')
              ->join('barangays AS brgy', 'brgy.id', '=', 'cit.brgy_id')
              ->select('ho_blood_sugar_tests.*','cit.cit_fullname','cit.cit_age','cit.cit_first_name','cit.cit_middle_name','cit.cit_gender','brgy.brgy_name','cit.cit_last_name');
        if(!empty($q) && isset($q)){
            $sql->where(function ($sql) use($q) {
                $sql->where(DB::raw('LOWER(ho_blood_sugar_tests.bs_lab_num)'),'like',"%".strtolower($q)."%")
				...
				->orWhere(DB::raw('LOWER(ho_blood_sugar_tests.bs_or_num)'),'like',"%".strtolower($q)."%")
                ->orWhere(DB::raw('LOWER(ho_blood_sugar_tests.bs_date)'),'like',"%".strtolower($q)."%")
				->orWhere(DB::raw('LOWER(ho_blood_sugar_tests.bs_result)'),'like',"%".strtolower($q)."%")
				->orWhere(DB::raw('LOWER(ho_blood_sugar_tests.is_posted)'),'like',"%".strtolower($q)."%")
				->orWhere(DB::raw('LOWER(ho_blood_sugar_tests.is_active)'),'like',"%".strtolower($q)."%")
                ; 
            });
        }
        /*  #######  Set Order By  ###### */
        if(isset($params['order'][0]['column']))
          ...
          $sql->orderBy('ho_blood_sugar_tests.id','DESC');
