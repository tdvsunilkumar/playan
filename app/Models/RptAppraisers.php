...
use DB;
class RptAppraisers extends Model
{

    public function updateData($id,$columns){
        return DB::table('rpt_appraisers')->where('id',$id)->update($columns);
    }
    public function addData($postdata){
        return DB::table('rpt_appraisers')->insert($postdata);
    }
    ...
    public function getList($request){
      ...
      $columns = array( 
          ...
          2 =>"p.fullname",
          3 =>"ad.name",
          4 =>"rpt.ra_appraiser_position",
          5 =>"p.fullname",
          6 =>"ra_is_active"
              );
              $sql = DB::table('rpt_appraisers AS rpt')
                    ->join('hr_employees AS p', 'p.id', '=', 'rpt.ra_appraiser_id')
					...
					->Leftjoin('users AS user', 'user.id', '=', 'rpt.created_by')
                    ->select('rpt.*','p.firstname','p.middlename','p.lastname','p.suffix','p.fullname','ad.name AS departments_name','add.name AS division_name','user.name');
              if(!empty($q) && isset($q)){
                  $sql->where(function ($sql) use($q) {
                      $sql->where(DB::raw('LOWER(p.fullname)'),'like',"%".strtolower($q)."%")
                          ...
                          ->orWhere(DB::raw('LOWER(rpt.ra_appraiser_position)'),'like',"%".strtolower($q)."%")
						  ->orWhere(DB::raw('LOWER(rpt.ra_is_active)'),'like',"%".strtolower($q)."%")
              ...
              if(isset($params['order'][0]['column']))
                ...
                $sql->orderBy('rpt.ra_is_active','DESC');
