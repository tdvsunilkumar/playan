...
class EngFecingType extends Model
{
    public function updateActiveInactive($id,$columns){
     return DB::table('eng_fecing_types')->where('id',$id)->update($columns);
    }  
    public function updateData($id,$columns){
        return DB::table('eng_fecing_types')->where('id',$id)->update($columns);
    }
    public function addData($postdata){
         DB::table('eng_fecing_types')->insert($postdata);
    ...
    public function getList($request){
        ...
        $sql = DB::table('eng_fecing_types')
              ->select('*');
        if(!empty($q) && isset($q)){
            $sql->where(function ($sql) use($q) {
                $sql->where(DB::raw('LOWER(eft_description)'),'like',"%".strtolower($q)."%")
                ; 
            });
        }
        /*  #######  Set Order By  ###### */
