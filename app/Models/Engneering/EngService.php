...
class EngService extends Model
{
    public function getServices(){
    	return DB::table('cto_tfocs as ctot')->leftjoin('acctg_account_general_ledgers AS aal', 'aal.id', '=', 'ctot.gl_account_id')
          ->leftjoin('acctg_account_subsidiary_ledgers AS aas', 'aas.id', '=', 'ctot.sl_id')->select('ctot.id','aal.code','aal.description as gldescription','aas.prefix','aas.description')->where('tfoc_status',1)->where('tfoc_is_applicable',3)->get();
    }
    ...
    public function getTransactiontype(){
        return DB::table('cto_top_transaction_type')->select('id','ttt_desc')->where('tfoc_is_applicable',3)->get();
    }
    public function updateData($id,$columns){
        return DB::table('eng_services')->where('id',$id)->update($columns);
    }
    public function addData($postdata){
        DB::table('eng_services')->insert($postdata);
    ...
    public function deleteRequirements($id,$ids){
      return DB::table('eng_service_requirements')->where('es_id',$id)->whereNotIn('req_id',$ids)->delete();
    }

    public function checkRequirementExists($serviceid,$reqid){
         return DB::table('eng_service_requirements')->select('id')->where('es_id','=',$serviceid)->where('req_id','=',$reqid)->get();
    }

    public function getRequirementforedit($serviceid){
         return DB::table('eng_service_requirements')->select('id','req_id','orderno','esr_is_required')->where('es_id','=',$serviceid)->get();
    }

    public function getRequirementforview($serviceid){
         return DB::table('eng_service_requirements as sr')->leftjoin('requirements as rq','sr.req_id', '=', 'rq.id')->select('req_description')->where('es_id','=',$serviceid)->get();
    }
    ...
    $sql = DB::table('eng_services AS es')
