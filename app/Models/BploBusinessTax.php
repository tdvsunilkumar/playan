...
class BploBusinessTax extends Model
{
    public function updateData($id,$columns){
        return DB::table('bplo_business_taxes')->where('id',$id)->update($columns);
    }
    public function addData($postdata){
         DB::table('bplo_business_taxes')->insert($postdata);
    ...
    public function getList($request){
        ...
        $columns = array( 
          ...
          3 =>"bbc_classification_code"
        );

        $sql = DB::table('bplo_business_taxes AS bt')
              ->join('tax_classes AS tc', 'tc.id', '=', 'bt.tax_class_id')
              ->join('tax_types AS tt', 'tt.id', '=', 'bt.tax_type_id')
               ->join('bplo_business_classifications AS bbc', 'bbc.id', '=', 'bt.bbc_classification_code')
              ->select('bt.id','tc.tax_class_desc','tax_type_short_name','bbc.bbc_classification_code','bbt_grsar_minimum','bbt_grsar_maximum','bbt_tax_amount','bbt_taxation_percent');
        //$sql->where('bt.created_by', '=', \Auth::user()->creatorId());
        if(!empty($q) && isset($q)){
