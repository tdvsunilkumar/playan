...
class AccountReceivableRental extends Model
{
    ...
    public function getpaymentList($request,$id){
        $params = $columns = $totalRecords = $data = array();
		    ...
		    $sql = DB::table('cto_cashier_details AS cc')
              ->leftjoin('eco_rental_application as era','era.id','=','cc.cemetery_application_id')
    ...
    $sql = DB::table('cto_cashier AS cc')
		  ...
		  ->leftjoin('eco_rental_application as era','era.or_no','=','cc.or_no')
		  ->leftjoin('barangays as b','b.id','=','era.location_id')
          ->select('era.id as id','era.transaction_no','b.brgy_name','c.cit_fullname','era.location_id','era.full_address','era.total_amount','era.remaining_amount','cc.top_transaction_id','cc.or_no','cc.total_paid_amount','era.status')->where('era.remaining_amount','>',0);
    //$sql->where('psc.subclass_generated_by', '=', \Auth::user()->creatorId());
     if($cemetery > 0){
     	 $sql->where('era.cemetery_id','=',$cemetery);     
     }  
     if($location > 0){
     	 $sql->where('era.location_id','=',$location);     
     }    
	 if(!empty($q) && isset($q)){
			$sql->where(function ($sql) use($q){
				$sql->where(DB::raw('LOWER(c.cit_fullname)'),'like',"%".strtolower($q)."%")
					->orWhere(DB::raw('LOWER(cc.top_transaction_id)'),'like',"%".strtolower($q)."%")
					->orWhere(DB::raw('LOWER(era.status)'),'like',"%".strtolower($q)."%");
			});
		}
