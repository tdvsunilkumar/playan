...
class AccountReceivableHousing extends Model
{
    ...
    public function getpaymentList($request,$id){
        $params = $columns = $totalRecords = $data = array();
		    ...
		    $sql = DB::table('cto_cashier_details AS cc')
              ->leftjoin('eco_housing_application as eha','eha.id','=','cc.cemetery_application_id')
    ...
    $sql = DB::table('cto_cashier AS cc')
		  ...
		  ->leftjoin('eco_housing_application as eha','eha.or_no','=','cc.or_no')
		  ->leftjoin('barangays as b','b.id','=','eha.barangay_id')
          ->select('eha.id as id','eha.reference_id as transaction_no','b.brgy_name','c.cit_fullname','eha.current_address as full_address','eha.total_amount','eha.remaining_amount','cc.top_transaction_id','cc.or_no','cc.total_paid_amount','cc.status')->where('eha.remaining_amount','>',0);
    //$sql->where('psc.subclass_generated_by', '=', \Auth::user()->creatorId());
     // if($cemetery > 0){
     // 	 $sql->where('era.cemetery_id','=',$cemetery);     
     // }  
     if($location > 0){
     	 $sql->where('eha.barangay_id','=',$location);     
     }    
	 if(!empty($q) && isset($q)){
			$sql->where(function ($sql) use($q){
