...
class UrinalysisModel extends Model
{
    ...
    public function updateActiveInactive($id,$columns){
     return DB::table('ho_urinalysis')->where('id',$id)->update($columns);
    }  
    public function updateData($id,$columns){
        return DB::table('ho_urinalysis')->where('id',$id)->update($columns);
    }
    public function addData($postdata){
        DB::table('ho_urinalysis')->insert($postdata);
    ...
    public function getEditDetails($id){
        return DB::table('ho_urinalysis')->where('id',$id)->first();
    }
    ...
    public function getList($request){
        ...
        $sql = self::
              join('citizens AS cit', 'cit.id', '=', 'ho_urinalysis.cit_id')
              ->join('barangays AS brgy', 'brgy.id', '=', 'cit.brgy_id')
              ->select('ho_urinalysis.*','cit.cit_fullname','cit.cit_age','cit.cit_first_name','cit.cit_middle_name','cit.cit_last_name','cit.cit_gender','brgy.brgy_name');
        
		
		if(!empty($q) && isset($q)){
            $sql->where(function ($sql) use($q) {
                $sql->where(DB::raw('LOWER(ho_urinalysis.urin_lab_num)'),'like',"%".strtolower($q)."%")
				->orWhere(DB::raw('LOWER(urin_date)'),'like',"%".strtolower($q)."%")
				...
				->orWhere(DB::raw('LOWER(urin_or_num)'),'like',"%".strtolower($q)."%")
				->orWhere(DB::raw('LOWER(created_at)'),'like',"%".strtolower($q)."%")
				->orWhere(DB::raw('LOWER(urin_is_posted)'),'like',"%".strtolower($q)."%")
				->orWhere(DB::raw('LOWER(urin_is_active)'),'like',"%".strtolower($q)."%")
                ; 
            });
        }
        /*  #######  Set Order By  ###### */
        if(isset($params['order'][0]['column']))
          ...
          $sql->orderBy('ho_urinalysis.id','DESC');
