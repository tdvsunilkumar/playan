...
class ForApprovalsResolutionController extends Controller
{   
    ...
    {   
        ...
        return view('for-approvals.bac.resolution.index');
    }
    ...
    {           
        ...
        $result = $this->bacResolutionRepository->approval_listItems($request);
        $res = $result->data->map(function($resolution) use ($actions, $actions2, $statusClass) {
            $project_name = ($resolution->rfq->project_name !== NULL) ? wordwrap($resolution->rfq->project_name, 25, "\n") : '';
            $agencies = wordwrap($this->get_agencies($resolution->rfq->id), 25, "\n");
            if ($resolution->identityDispprovedBy !== NULL) {
                $approvedBy = '<strong>'.$this->fetchApprovedBy($resolution->identityDispprovedBy).'</strong><br/>'.date('d-M-Y H:i a', strtotime($resolution->identityDisapprovedAt));
            } else if($resolution->identityApprovedBy !== NULL) {
                $approvedBy = '<strong>'.$this->fetchApprovedBy($resolution->identityApprovedBy).'</strong><br/>'.date('d-M-Y H:i a', strtotime($resolution->identityApprovedAt));
            } else {
                $approvedBy = '';
            }
            return [
                'id' => $resolution->identity,
                'rfq' => $resolution->rfq->id,
                'control_no' => $resolution->rfq->control_no,
                'control_no_label' => '<strong class="text-primary">'.$resolution->rfq->control_no.'</strong>',
                'project_name' => '<div class="showLess">' . $project_name . '</div>',
                'agencies' => '<div class="showLess">' . $agencies . '</div>',
                'modified' => ($resolution->identityUpdated !== NULL) ? date('d-M-Y', strtotime($resolution->identityUpdated)).'<br/>'. date('h:i A', strtotime($resolution->identityUpdated)) : date('d-M-Y', strtotime($resolution->identityCreated)).'<br/>'. date('h:i A', strtotime($resolution->identityCreated)),
                'approved_by' => $approvedBy,
                'status' => $statusClass[$resolution->identityStatus]->status,
                'status_label' => '<span class="badge badge-status rounded-pill ' . $statusClass[$resolution->identityStatus]->bg. ' p-2">' . $statusClass[$resolution->identityStatus]->status . '</span>',
                'actions' => ($resolution->identityStatus == 'cancelled') ? $actions2 : $actions
            ];
        });
    ...
    {   
        ...
        $result = $this->bacResolutionRepository->committee_listItems($request, $id);
    ...
    public function fetch_status(Request $request, $id)
    {
        $this->is_permitted($this->slugs, 'read'); 
        return response()->json([
            'status' => $this->bacResolutionRepository->find_resolution($id)->status
        ]);
    }
    ...
    {
        if (!empty($approvers)) {
            return $this->bacResolutionRepository->fetchApprovedBy($approvers);
        }
        return '';
    }
    ...
    {
        $approvers = explode(',',$this->bacResolutionRepository->find_resolution($id)->approved_by);
    ...
    public function approve(Request $request, $rfqID)
    {
        if ($this->is_permitted($this->slugs, 'approve', 1) > 0) {
            ...
            $this->bacResolutionRepository->update_request($rfqID, $details);
            $details2 = array(
                'status' => 'completed',
                'approved_at' => $timestamp,
                'approved_by' => Auth::user()->id
            );
            $this->bacResolutionRepository->update($rfqID, $details2);
            return response()->json([
    ...
    public function disapprove(Request $request, $rfqID)
    {
        if ($this->is_permitted($this->slugs, 'disapprove', 1) > 0) {
            ...
            $this->bacResolutionRepository->update($rfqID, $details);
            $details2 = array(
                'status' => 'cancelled',
                'updated_at' => $timestamp,
                'updated_by' => Auth::user()->id
            );
            $this->bacResolutionRepository->update_request($rfqID, $details2);
            // $this->bacResolutionRepository->updateRequest($rfqID, $details2);
            $lineDetails = array(
                'status' => 'cancelled',
                'updated_at' => $timestamp,
                'updated_by' => Auth::user()->id
            );
            $this->bacResolutionRepository->updateLines($rfqID, $lineDetails);
            $details3 = array(
            ...
            $this->bacResolutionRepository->disapprove_request($rfqID, $details3);
            return response()->json([
    ...
    public function fetch_remarks(Request $request, $rfq_ID)
    {
        $this->is_permitted($this->slugs, 'read'); 
        return response()->json([
            'remarks' => $this->bacResolutionRepository->find($rfq_ID)->disapproved_remarks
        ]);
    }
