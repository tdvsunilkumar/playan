...
use App\Models\Engneering\ConsultantExternals;
...
class EngsConsultantExternalContt extends Controller
{
     public $data = [];
     public $postdata = [];
     private $slugs;
     public function __construct(){
        ...
        $this->_ConsultantExternal = new ConsultantExternals();
        ...
        foreach ($data['data'] as $row){
            ...
            if ($this->is_permitted($this->slugs, 'update', 1) > 0) {
                $actions .= '<div class="action-btn bg-warning ms-2">
                    <a href="#" class="mx-3 btn btn-sm  align-items-center" data-url="'.url('/eng/consultantexternal/store?id='.$row->id).'" data-ajax-popup="true"  data-size="lg" data-bs-toggle="tooltip" title="Edit"  data-title="Manage Consultant External">
    ...
    public function store(Request $request){
        ...
        if($request->input('submit')!=""){
            ...
            $this->_commonmodel->updateLog($logDetails);
            //return redirect()->route('consultantexternals.index')->with('success', __($success_msg));
