...
class AcctgPayablesVouchersController extends Controller
{
        ...
        Carbon $carbon) 
    {
        ...
        $this->carbon = $carbon;
        $this->slugs = 'accounting/journal-entries/payables';
    }
    ...
    {   
        ...
        return view('accounting.journal-entries.payables.index')->with(compact('segment', 'accounting_permission', 'gl_accounts', 'uoms', 'evats', 'ewts', 'trans_types', 'vat_types'));
    }
    ...
    {  
        ...
        return view('accounting.journal-entries.payables.create')->with(compact('approvers', 'segment', 'accounting_permission', 'treasury_permission', 'fund_codes', 'payees', 'payment_types', 'sl_accounts', 'sl_accountx', 'gl_accounts', 'uoms', 'evats', 'ewts', 'trans_types', 'vat_types'));
    }
    ...
    {  
        ...
        $segment = request()->segment(count(request()->segments()) - 2);
        $approvers = $this->acctgAccountVoucherRepository->get_voucher_approvers();
        return view('accounting.journal-entries.payables.view')->with(compact('approvers', 'segment', 'accounting_permission', 'treasury_permission', 'fund_codes', 'payees', 'payment_types', 'sl_accounts', 'sl_accountx', 'gl_accounts', 'uoms', 'evats', 'ewts', 'trans_types', 'vat_types'));
    }
    ...
    { 
        ...
        $result = $this->acctgAccountVoucherRepository->listItems($request);
    ...
    { 
        ...
        $res = $result->data->map(function($payment) use ($statusClass, $actions, $actions2, $actions3, $actions4, $iconClass) {
            ...
            return [
                ...
                'actions' => $statusClass[$payment->identityStatus]->action . '' . $buttons
            ];
        });
    ...
    public function view_available_payables(Request $request, $voucherID)
    {
        $this->is_permitted($this->slugs, 'read');
        return response()->json([
            'data' => $this->acctgAccountVoucherRepository->view_available_payables($voucherID, $request->get('fund_code'), $request->get('payee'), $request->get('replenish'))->map(function($payable) {
                $gl_account = $payable->gl_account ? wordwrap($payable->gl_account->code . ' - ' . $payable->gl_account->description, 25, "\n") : ''; 
                $items = $payable->items ? wordwrap($payable->items, 25, "\n") : '';   
                return (object) [
                    'id' => $payable->identity,
    ...
    {
        if ($voucherID > 0) {
            $this->acctgAccountVoucherRepository->add_payables($request, $voucherID);
        } else {
            $details = array(
        ...
        return response()->json([
            ...
            'title' => 'Well done!',
            'text' => 'The payable has been successfully added.',
            'type' => 'success',
            'class' => 'btn-brand'
        ]);
    }
    ...
    public function remove_payables(Request $request, $id): JsonResponse 
    {   
        $this->is_permitted($this->slugs, 'delete'); 
        $voucher = $this->acctgAccountPayableRepository->find($id)->voucher_id;
        $details = array(
            'voucher_id' => NULL,
            'updated_at' => $this->carbon::now(),
            'updated_by' => Auth::user()->id
        );
        $this->acctgAccountVoucherRepository->remove_payables($id, $details);
        return response()->json([
    ...
    public function remove_all_payables(Request $request, $voucherID)
    {   
        if ($this->is_permitted($this->slugs, 'delete', 1) > 0) {
            $details = array(
                'voucher_id' => NULL,
            ...
            $res = $this->acctgAccountVoucherRepository->remove_all_payables($request, $details);
            return response()->json([
    ...
    public function send_all_payables(Request $request, $voucherID)
    {   
        if ($this->is_permitted($this->slugs, 'update', 1) > 0) {            
            $timestamp = $this->carbon::now();
            if ($this->is_permitted($this->slugs, 'approve', 1) > 0) {
            ...
            $res = $this->acctgAccountVoucherRepository->send_all_payables($request, $details);
            return response()->json([
                'data' => $this->update_vouchers($voucherID),
                'title' => 'Well done!',
                'text' => 'The payables has been successfully sent.',
                'type' => 'success',
                'class' => 'btn-brand'
            ]);
        }
    ...
    {   
        if ($this->is_permitted($this->slugs, 'delete', 1) > 0) {
            ...
            return response()->json([
                'data' => $this->update_vouchers($voucherID),
                'title' => 'Well done!',
                'text' => 'The payments has been successfully removed.',
                'type' => 'success',
                'class' => 'btn-brand'
            ]);
        }
    ...
    {   
        if ($this->is_permitted($this->slugs, 'update', 1) > 0) {
            $timestamp = $this->carbon::now();
            if ($this->is_permitted($this->slugs, 'approve', 1)) {
                $details = array(
                    'status' => 'posted',
                    'sent_at' => $timestamp,
                    'sent_by' => Auth::user()->id,
            ...
            return response()->json([
                'data' => $this->update_vouchers($voucherID),
                'title' => 'Well done!',
                'text' => 'The payments has been successfully sent.',
                'type' => 'success',
                'class' => 'btn-brand'
            ]);
        }
    ...
    {
        $request->amount = currency_to_float($request->amount);
        if ($voucherID > 0) {
            ...
            if (!($res->is_replenish > 0)) {
                ...
                if (!empty($ewt_dues)) {
                    ...
                    foreach ($ewt_dues as $due) {
                        ...
                        PDF::MultiCell(25, 5, '', 0, 'C', 0, 0, '', '', true);
                        PDF::MultiCell(30, 5, '', 0, 'R', 0, 0, '', '', true);
                        PDF::MultiCell(30, 5, number_format($this->format_money($due->totalEwt), 2), 0, 'R', 0, 0, '', '', true);
                        $totalCredit += floatval($this->format_money($due->totalEwt));
                    }
                }
                ...
                if (!empty($evat_dues)) {
                    ...
                    foreach ($evat_dues as $due) {
                        ...
                        PDF::MultiCell(25, 5, '', 0, 'C', 0, 0, '', '', true);
                        PDF::MultiCell(30, 5, '', 0, 'R', 0, 0, '', '', true);
                        PDF::MultiCell(30, 5, number_format($this->format_money($due->totalEvat), 2), 0, 'R', 0, 0, '', '', true);
                        $totalCredit += floatval($this->format_money($due->totalEvat));
                    }
                    PDF::ln(6);
                }
            }
            ...
            if (!empty($glPayments)) {
                foreach ($glPayments as $glPayment) {
                    if (!($res->is_replenish > 0)) {
                        ...
                        PDF::MultiCell(30, 5, '', 0, 'R', 0, 0, '', '', true);
                        PDF::MultiCell(30, 5,'', 0, 'R', 0, 0, '', '', true);
                        // get_sl_payments($voucher, $glPayment->id, $request->get('reference_no'), $posted = 0);   
                        $slPayments = $this->acctgAccountVoucherRepository->get_sl_payments($voucher, $glPayment->id, $request->get('reference_no'));   
                        if (!empty($slPayments)) {
                            PDF::SetFont('Helvetica', 'B', 9);
                            foreach ($slPayments as $slPayment) {
        ...
        if ($request->get('type') == 'payables') {
            ...
            if (!($res->is_replenish > 0)) {
                ...
                if (!empty($lines)) {
                    ...
                    foreach ($lines as $line) {
                        PDF::MultiCell(25.85, 5, $line->responsibility_center, 0, 'C', 0, 0, '', '', true);
                        PDF::MultiCell(85, 5, $line->gl_name , 0, 'L', 0, 0, '', '', true);
                        PDF::MultiCell(25, 5, $line->gl_code , 0, 'C', 0, 0, '', '', true);
                        PDF::MultiCell(30, 5, $this->money_formatx($line->totalAmt), 0, 'R', 0, 0, '', '', true);
                        PDF::MultiCell(30, 5, '', 0, 'R', 0, 0, '', '', true);    
                        PDF::ln();
