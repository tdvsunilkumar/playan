...
use App\Models\Treasury\AccountReceivableRental;
...
class AccountReceivableRentalController extends Controller
{
    public $data = [];
     public $postdata = [];
     private $slugs;
     public function __construct(){
        $this->_accrcblrental = new AccountReceivableRental(); 
        $this->_commonmodel = new CommonModelmaster();
        $this->data = array('id'=>'','busloc_desc'=>'');  
        $this->slugs = 'rental-ar';
    }
    public function index(Request $request)
    {
        ...
        $arrDepaertments = array('0'=>'All');
         foreach ($this->_accrcblrental->GetCemeteryName() as $val) {
             $arrDepaertments[$val->id]=$val->cem_name;
         }
        $arrlocations = array(""=>"Select Location");
        foreach ($this->_accrcblrental->Getlocationarray() as $val) {
             $arrlocations[$val->id]=$val->brgy_name;
         } 
        // print_r($arrlocations); exit;
        return view('treasury.accountreceivable.rental.index',compact('startdate','enddate','arrDepaertments','arrlocations'));
    }

    public function getpaymentlist(Request $request){
    	$id = $request->input('id');

    	$data=$this->_accrcblrental->getpaymentList($request,$id);
    ...
    public function getList(Request $request){
        $this->is_permitted($this->slugs, 'read'); 
        $data=$this->_accrcblrental->getList($request);
        //echo "<pre>"; print_r($data); exit;
        $arr=array();
        $i="0"; 
        $sr_no=(int)$request->input('start')-1; 
    ...
    public function exportdepartmentalcollection(Request $request){
        $data =$this->_accrcblrental->getListexport($request);
