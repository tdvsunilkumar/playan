...
class AcctgIncomeVouchersController extends Controller
{
        ...
        Carbon $carbon) 
    {
        ...
        $this->carbon = $carbon;
        $this->slugs = 'accounting/journal-entries/incomes';
    }
    ...
    {   
        ...
        return view('accounting.journal-entries.incomes.index')->with(compact('segment', 'accounting_permission', 'gl_accounts', 'uoms', 'evats', 'ewts', 'trans_types', 'vat_types'));
    }
    ...
    {  
        ...
        return view('accounting.journal-entries.incomes.create')->with(compact('approvers', 'segment', 'accounting_permission', 'treasury_permission', 'fund_codes', 'payees', 'payment_types', 'sl_accountx', 'sl_accounts', 'gl_accounts', 'uoms', 'evats', 'ewts', 'trans_types', 'vat_types'));
    }
    ...
    {  
        ...
        $segment = request()->segment(count(request()->segments()) - 2);
        $approvers = $this->acctgAccountVoucherRepository->get_voucher_approvers();
        return view('accounting.journal-entries.incomes.view')->with(compact('approvers', 'segment', 'accounting_permission', 'treasury_permission', 'fund_codes', 'payees', 'payment_types', 'sl_accounts', 'sl_accountx', 'gl_accounts', 'uoms', 'evats', 'ewts', 'trans_types', 'vat_types'));
    }
    ...
    { 
        ...
        $res = $result->data->map(function($payment) use ($statusClass, $iconClass) {
            ...
            if ($statusClass[$payment->identityStatus]->status == 'posted' && $payment->type->name == 'Cash') {
                if ($this->is_permitted('treasury/journal-entries/incomes', 'update', 1) > 0) {
                    $buttons .= $iconClass['Deposit']->button;
                }
            }
            if ($statusClass[$payment->identityStatus]->status == 'posted' && $payment->type) {
                if ($payment->type->name == 'Cash' || $payment->type->name == 'Cheque') {
    ...
    public function add_payables(Request $request, $voucherID) 
    {
        ...
        return response()->json([
            ...
            'title' => 'Well done!',
            'text' => 'The collection has been successfully added.',
            'type' => 'success',
            'class' => 'btn-brand'
        ]);
    }
    ...
    {   
        $this->is_permitted($this->slugs, 'update'); 
        $income = $this->acctgAccountIncomeRepository->find($id);
        $details = array(
        ...
        $this->acctgAccountIncomeRepository->update($id, $details);
        return response()->json([
            'data' => $this->update_vouchers($this->acctgAccountIncomeRepository->find($id)->voucher_id),
            'title' => 'Well done!',
    ...
    public function store_payments(Request $request, $voucherID) 
    {
        ...
        return response()->json([
            'data' => $this->acctgAccountVoucherRepository->find($voucherID),
            'title' => 'Well done!',
            'text' => 'The deposit has been successfully added.',
            'type' => 'success',
            'class' => 'btn-brand'
        ]);
    }
    ...
    public function update_payments(Request $request, $paymentID) 
    {
        ...
        return response()->json([
            'data' => $this->acctgAccountVoucherRepository->find($voucherID),
            'title' => 'Well done!',
            'text' => 'The deposit has been successfully updated.',
            'type' => 'success',
            'class' => 'btn-brand'
        ]);
    }
    ...
    public function fetch_payable_status(Request $request, $payableID)
    {
        $this->is_permitted($this->slugs, 'read'); 
        return response()->json([
            'status' => $this->acctgAccountIncomeRepository->find($payableID)->status
        ]);
    }

    public function validate_payables_approver(Request $request, $id)
    {
        $approvers = explode(',',$this->acctgAccountIncomeRepository->find($id)->approved_by);
        if (in_array(Auth::user()->id, $approvers)) {
            return true;
        }
        return false;
    ...
    public function approve_payable(Request $request, $payableID): JsonResponse 
    {
        if ($this->is_permitted($this->slugs, 'approve', 1) > 0) {
            ...
            return response()->json([
                'data' => $this->acctgAccountIncomeRepository->approve($payableID, $details),
    ...
    public function disapprove_payable(Request $request, $payableID)
    {
        if ($this->is_permitted($this->slugs, 'disapprove', 1) > 0) {
            ...
            $this->acctgAccountIncomeRepository->disapprove($payableID, $details);
            return response()->json([
    ...
    public function fetch_payable_remarks(Request $request, $payableID)
    {
        $this->is_permitted($this->slugs, 'read'); 
        return response()->json([
            'remarks' => $this->acctgAccountIncomeRepository->find($payableID)->disapproved_remarks
        ]);
    }

    public function fetch_payment_status(Request $request, $paymentID)
    {
        $this->is_permitted('treasury/journal-entries/incomes', 'read'); 
        return response()->json([
    ...
    {
        if ($this->is_permitted('treasury/journal-entries/incomes', 'approve', 1) > 0) {
            $timestamp = $this->carbon::now();
            $details = array(
                'status' => 'posted',
                'approved_at' => $timestamp,
    ...
    {
        if ($this->is_permitted('treasury/journal-entries/incomes', 'disapprove', 1) > 0) {
            $timestamp = $this->carbon::now();
            $details = array(
                'status' => 'cancelled',
                'disapproved_at' => $timestamp,
    ...
    public function print(Request $request, $voucher)
    {
        ...
        if (!($deposits->count() > 0)) {
            ...
            if (count($currentTax) > 0) {
                foreach ($currentTax as $current) {
                    $rpt_taxes = $this->acctgAccountVoucherRepository->get_rpt_current_tax($res->fund_code_id, $current->id);
                    if ($rpt_taxes->count() > 0) 
                    {
                        foreach ($rpt_taxes as $rpt_tax) {
                            if (!in_array($rpt_tax->gl_account->code, $gl_codes)) {
                                PDF::SetFont('Helvetica', 'B', 9);
                                PDF::MultiCell(25.85, 5, '', 0, 'C', 0, 0, '', '', true);
                                PDF::MultiCell(85, 5, $rpt_tax->gl_account->description, 0, 'L', 0, 0, '', '', true);
                                PDF::MultiCell(25, 5, $rpt_tax->gl_account->code, 0, 'C', 0, 0, '', '', true);
                                PDF::MultiCell(30, 5, '', 0, 'R', 0, 0, '', '', true);
                                PDF::MultiCell(30, 5, '', 0, 'R', 0, 0, '', '', true);
                                PDF::ln(6);
                                $gl_codes[] = $rpt_tax->gl_account->code;
                            }
                            PDF::SetFont('Helvetica', '', 9);
                            PDF::MultiCell(25.85, 5, '', 0, 'C', 0, 0, '', '', true);
                            PDF::MultiCell(85, 5, '     '.$rpt_tax->sl_account->description, 0, 'L', 0, 0, '', '', true);
                            PDF::MultiCell(25, 5, $rpt_tax->sl_account->code, 0, 'C', 0, 0, '', '', true);
                            if ($rpt_tax->is_debit > 0) {
                                PDF::MultiCell(30, 5, number_format($current->total, 2), 0, 'R', 0, 0, '', '', true);
                                PDF::MultiCell(30, 5, '', 0, 'R', 0, 0, '', '', true);
                                PDF::ln(6);
                                $totalDebit += floatval($current->total);
                            } else {
                                PDF::MultiCell(30, 5, '', 0, 'R', 0, 0, '', '', true);
                                PDF::MultiCell(30, 5, number_format($current->total, 2), 0, 'R', 0, 0, '', '', true);
                                PDF::ln(6);
                                $totalCredit += floatval($current->total);
                            }
                        }
                    }
                }
