...
use Carbon\Carbon;
use File;
class CertificateOfNoImprovementController extends Controller
{
  ...
  $this->data = array('id'=>'','rvy_revision_year'=>'','rpc_requestor_code'=>'','rpc_year'=>'','rpc_control_no'=>'','rpc_city_assessor_code'=>'','rpc_certified_by_code'=>'','rpc_or_no'=>'','rpc_or_date'=>'','rpc_or_amount'=>'','rpc_certified_by_position'=>'','rpc_cert_type'=>'','rpc_remarks'=>'','rpc_owner_code'=>'','cashierd_id'=>'','cashier_id'=>'','rpc_date'=>'');
   ...
   $this->dataCleint = array('id'=>'','rpo_custom_last_name'=>'','rpo_first_name'=>'','rpo_middle_name'=>'','suffix'=>'','p_code'=>'','rpo_address_house_lot_no'=>'','rpo_address_street_name'=>'','rpo_address_subdivision'=>'','brgy_code'=>'','p_barangay_id_no'=>'','p_telephone_no'=>'','p_mobile_no'=>'','p_fax_no'=>'','p_tin_no'=>'','p_email_address'=>'','country'=>'','gender'=>'','dateofbirth'=>'');
        ...
        $this->slugs = 'certificate-of-no-improvement';
    }
    ...
    public function getOrNumbersAjax(Request $request){
        $search = $request->input('search');
           $id = $request->input('id');
           $arrRes = $this->_rptpropertycert->getctoCashierIdPropertyNoimprovementAjax($search,$id);
            $arr = array();
    ...
    public function getCertPositionDetailsImprovement(Request $request){
     ...
     public function getClientCodeImprovement(Request $request){
    ...
    public function getctoCashierIdPropertyImprovment(Request $request){
       $id=$request->input('id');
       $getgroups = $this->_rptpropertycert->getctoCashierIdPropertyImprovment($id);
       $htmloption ="";
    ...
    foreach ($data as $row) {
        ...
        $record = [
            ...
            'rpa_base_market_value' => number_format($row->rp_market_value, 3, '.', ','),
            'rpa_assessed_value' => ($row->pk_id == 1) ? number_format($row->rpb_assessed_value, 3, '.', ',') : number_format($row->rp_assessed_value, 3, '.', ','),
            // 'action' => '',
        ];
        ...
        $record['srno'] = '<input type="radio" name="rp_code[]" class="myCheckbox" value="' . $row->propertyId . '" id="'.$arrAppHoldingId.'"';
 ...
 // public function getClientsAppraisalsMechImprovement(Request $request){
       
       
    //     $id = $request->input('id');
        
    //     $result = $this->_rptpropertycert->getClientsAppraisalsMechImprovement($id);
    ...
    // }
    //  public function getClientsAppraisalsBuildingImprovement(Request $request){
       
       
    //     $id = $request->input('id');
        
    //     $result = $this->_rptpropertycert->getClientsAppraisalsBuildingImprovement($id);
    ...
    if ($rp_code) {
     ...
     public function addRptCertRelationMech($request,$rptCertId){
        ...
        foreach ($rpam_code as $key => $value) {
                $arr[$i]['rpc_code']=$rptCertId;
                $arr[$i]['rp_code']=$request->input('rp_code')[$key];
                $arr[$i]['rpa_code']=0;
                $arr[$i]['rpab_code']=0;
                $arr[$i]['rpam_code']=$request->input('rpam_code')[$key];
                $arr[$i]['updated_by'] =  \Auth::user()->creatorId();
                $arr[$i]['updated_at'] = date('Y-m-d H:i:s');
                // print_r($arr);exit;
 ...
 public function addRptCertRelationBuilding($request,$rptCertId){
        ...
        $i=0;
         // print_r($rp_code);exit;
        foreach ($rpab_code as $key => $value) {
                $arr[$i]['rpc_code']=$rptCertId;
                $arr[$i]['rp_code']=$request->input('rp_code')[$key];
                $arr[$i]['rpa_code']=0;
                $arr[$i]['rpam_code']=0;
                $arr[$i]['rpab_code']=$request->input('rpab_code')[$key];
                $arr[$i]['updated_by'] =  \Auth::user()->creatorId();
                $arr[$i]['updated_at'] = date('Y-m-d H:i:s');
                // print_r($arr);exit;
    ...
    {   
        ...
        return view('rptcertificateofnoImprovement.index',compact('revisionyear'));
       
    }
    
    
    public function getList(Request $request){
        $data=$this->_rptpropertycert->getListImprovement($request);
        $arr=array();
        $i="0";    
        ...
        foreach ($data['data'] as $row){
            ...
            $arr[$i]['action']='
                    ...
                    <div class="action-btn bg-info ms-2">
                        <a href="#" title="Print Certificate Of No Land Holding"  data-title="Print Certificate  Of No Improvement" class="mx-3 btn print btn-sm" id="'.$row->id.'">
    ...
    public function store(Request $request){
        ...
        if($request->input('id')>0 && $request->input('submit')==""){
            ...
            foreach ($this->_rptpropertycert->getctoCashierIdPropertyImprovment($data->rpc_owner_code) as $val) {
        ...
        if($request->input('submit')!=""){
            ...
            return redirect('certificate-of-no-improvement')->with('success', __($success_msg));
        }
        return view('rptcertificateofnoImprovement.create',compact('data','arrRequestor','arrRevisionCode','arrHrEmplyees','arrNoHolding','arrAppHolding','arrAppHoldingMech','arrAppHoldingBuilding','arrOrnumber','arrHrEmplyeesAppraisers','arrRequestorTaxpayer','arrHrEmplyeesApproved'));
    ...
    public function storeClient(Request $request){
        ...
        if($request->input('submitClient')){
            ...
            if($request->ajax()){
                ...
                return redirect()->route('rptcertificateofnoImprovement.store')->with('success', __($success_msg));
            }
            
        }
        
        return view('rptcertificateofnoImprovement.client',compact('arrgetBrgyCode','arrgetCountries'));
    ...
    public function CertPholdingPrintImprovement(Request $request){
            ...
            $filename="";
            $html = file_get_contents(resource_path('views/layouts/templates/CertificateOfNoImprovement.html'));
