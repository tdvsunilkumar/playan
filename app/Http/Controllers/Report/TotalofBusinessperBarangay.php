...
use App\Models\Report\TotalofBusinessperBar;
...
use File;
use Response;
class TotalofBusinessperBarangay extends Controller
{
     public $data = [];
     public $postdata = [];
     private $slugs;
     ...
     public function __construct(){
        $this->_TotalofBusinessperBar = new TotalofBusinessperBar(); 
        $this->_commonmodel = new CommonModelmaster();
        $this->data = array();  
        $this->slugs = 'total-of-business-per-barangay';
		$arrYrs = $this->_TotalofBusinessperBar->getYearDetails();
    ...
    public function index(Request $request)
    {
		...
		$year =  date('Y');
		return view('report.totalofBusiness.index')->with(compact('year','arrYears'));
	}
	
    public function getList(Request $request){
		
       $this->is_permitted($this->slugs, 'read');
       $data=$this->_TotalofBusinessperBar->getList($request);
   ...
   public function exportlistofbusiness(Request $request){
        $data=$this->_TotalofBusinessperBar->getDataPerBarngay($request);
        $headers = array(
