...
class HrOffsetController extends Controller
{
    public $data = [];
     public $postdata = [];
    ...
    public function getList(Request $request){
        ...
        $this->is_permitted($this->slugs, 'read');
        $data=$this->_offset->getList($request,$hr_employeesid);
        ...
        foreach ($data['data'] as $row){
            ...
            $arr[$i]['action']='
                <div class="action-btn bg-warning ms-2">
                    <a href="#" class="mx-3 btn btn-sm  align-items-center" data-url="'.url('/hr-offset/store?id='.$row->id).'" data-ajax-popup="true"  data-size="xl" data-bs-toggle="tooltip" title="Edit"  data-title="Edit Offset">
    ...
    {
        return $this->_offset->validate_approver($this->_offset->getUserdapartment($id)->acctg_department_id, $sequence, 'sub modules', $this->slugs, Auth::user()->id);
    }
    ...
    {   $id = $request->input('id');
        ...
        if ($this->is_permitted($this->slugs, 'approve', 1) > 0) {
            if ($offset_data->hro_approved_by == NULL) {
            ...
            if($sequence=='1'){
                ...
                //$details['hro_approved_at'] = $positionname;  
    ...
    public function disapprove(Request $request)
    {
        ...
        if ($this->is_permitted($this->slugs, 'disapprove', 1) > 0) {
            ...
            // dd($offset_data->hro_status > 1);
            if ($offset_data->hro_status > 1) {
                $updatedata = array();
    ...
    public function store(Request $request){
        ...
        $data->hro_status =0;
        $data->hro_remaining_offset_hours = ($this->_offset->chkBalanceHour($hr_emp->id)) ? $this->_offset->chkBalanceHour($hr_emp->id)[0]->hroh_balance_offset_hours : 0;
        if($request->input('id')>0 && $request->input('submit')==""){
            $data = $this->_offset->getRecordforEdit($request->input('id'));
            $status=$arrChangeSchedulestatus[$data->hro_status];
		...
		if($request->input('submit')!=""){
            ...
            $userdata = $this->_offset->getUserdapartment(Auth::user()->id);
            // $this->data['department_id']=$userdata->acctg_department_id;
            if ($request->submit_type === '3') {
                $remaining_hours = $this->_offset->chkBalanceHour($request->	hr_employeesid)[0];
                $balance_hrs = $remaining_hours->hroh_balance_offset_hours;
                $used_hrs = ($request->hro_id === '1') ? 8 : 4;
                // dd($used_hrs);
                $updatedata = array();
                $updatedata['hroh_balance_offset_hours'] = $balance_hrs - $used_hrs;
                $updatedata['hroh_used_offset_hours'] = $remaining_hours->hroh_used_offset_hours + $used_hrs;
                $this->_offset->updateOffserHourData($request->hr_employeesid,$updatedata);
            }
        ...
        return view('HR.offset.create',compact('data','status','arrEmployee','validateapprove','validatenoted','validatereview','date','arrApplicationtypes','arrDocuments'));
	}
    ...
    public function formValidation(Request $request){
        $validator = \Validator::make(
            $request->all(), [
                // 'hr_employeesid'=>'required|unique:hr_offsets,hr_employeesid,'.(int)$request->input('id').',id,hro_work_date,'.$request->input('hro_work_date'),
                'hro_work_date'=>'required',
                'hro_id'=>'required',
                'hro_reason'=>'required',
                'hro_remaining_offset_hours'=>'required|numeric|gt:0',
                'hr_employeesid' => [
                    'required',
                    Rule::unique('hr_offsets')->where(function ($query) use ($request) {
                        return $query->where('hro_work_date', $request->input('hro_work_date'))
        ...
        if($request->submit_type === '3' && $request->hro_id === '1' && $request->hro_remaining_offset_hours < 8){
            $arr['field_name'] = 'hro_remaining_offset_hours';
            $arr['error'] = 'Oops! Offset hours credit is insufficient to continue with the application creation.';
            $arr['ESTATUS'] = 1;
        } else if ($request->submit_type === '3' && $request->hro_remaining_offset_hours < 4){
            $arr['field_name'] = 'hro_remaining_offset_hours';
            $arr['error'] = 'Oops! Offset hours credit is insufficient to continue with the application creation.';
            $arr['ESTATUS'] = 1;
        } 
