...
class ChangeofScheduleApprovalController extends Controller
{
    ...
    public $postdata = [];
     ...
     public function __construct(HrInterface $hrRepository, Carbon $carbon){
        ...
        $this->data = array('id'=>'','hr_employeesid'=>'','hrcos_start_date'=>'','hrcos_end_date'=>'','hrcos_original_schedule'=>'','hrcos_new_schedule'=>'');  
        $this->slugs = 'change-of-schedule-approval'; 
    ...
    {       $slug = $this->slugs; 
            ...
            return view('HR.changeofschedule.approval',compact('slug'));
    }


    public function getList(Request $request){
        $this->is_permitted($this->slugs, 'read');
        $data=$this->_chnageofschedule->getListApprove($request);
        ...
        foreach ($data['data'] as $row){
            ...
            $arr[$i]['action']='
                <div class="action-btn bg-warning ms-2">
                    <a href="#" class="mx-3 btn btn-sm  align-items-center" data-url="'.url('/change-of-schedule-approval/store?id='.$row->id).'" data-ajax-popup="true"  data-size="xl" data-bs-toggle="tooltip" title="Edit"  data-title="Edit Change Schedule">
    ...
    {
        return $this->_chnageofschedule->validate_approver($this->_chnageofschedule->getRecordforEdit($id)->department_id, $sequence, 'sub modules', 'hr-change-schedule', Auth::user()->id);
    }
    ...
    {   $id = $request->input('id');
        ...
        if ($this->is_permitted($this->slugs, 'approve', 1) > 0) {
            ...
            if($sequence=='1'){
                ...
                $startdate = $recorddata->hrcos_start_date; 
                $newschedule =$recorddata->hrcos_new_schedule;
                $enddate = $recorddata->hrcos_end_date; 
                $employeeid = $recorddata->hr_employeesid;
                $this->_chnageofschedule->updateworkschedule($startdate,$enddate,$newschedule,$employeeid);
    ...
    public function store(Request $request){
        ...
        if($request->input('id')>0 && $request->input('submit')==""){
            ...
            $approve_btn = $this->hrRepository->approveButton(Auth::user()->id,$data->department_id, $this->slugs,$data->status);
        ...
        return view('HR.changeofschedule.createapprove',compact('data','status','arrdefaultschedule','arrEmployee','approve_btn','arrDocuments'));
	}
    ...
    public function formValidation(Request $request){
            ...
            $request->all(), [
                'hr_employeesid'=>'required|unique:hr_changeof_schedules,hr_employeesid,'.(int)$request->input('id'),
                'hrcos_start_date'=>'required',
                'hrcos_end_date'=>'required',
