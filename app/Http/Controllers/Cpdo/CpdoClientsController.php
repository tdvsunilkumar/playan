...
use App\Models\Cpdo\CpdoClients;
...
use File;
class CpdoClientsController extends Controller
{
     public $data = [];
    public $postdata = [];
    ...
    public function __construct(){
        $this->_cpdoClients = new CpdoClients();
        ...
        foreach ($this->_cpdoClients->getCountries() as $val) {
            $this->arrgetCountries[$val->id]=$val->nationality;
        }
        foreach ($this->_cpdoClients->getClients() as $val) {
            $this->arrgetClients[$val->id]=$val->full_name;
        }
        $this->slugs = 'cpdoclients'; 
    ...
    public function index(Request $request)
    {
            ...
            return view('cpdo.cpdoclients.index');
    }
  
  
    public function getList(Request $request){
    	$this->is_permitted($this->slugs, 'read');
        $data=$this->_cpdoClients->getList($request);
        $arr=array();
        $i="0";
        ...
        foreach ($data['data'] as $row){
            ...
            $arr[$i]['action']='
                <div class="action-btn bg-warning ms-2">
                    ...
                    <a href="#" class="mx-3 btn btn-sm  align-items-center" data-url="'.url('/cpdoclients/store?id='.$row->id).'" data-ajax-popup="true"  data-size="lg" data-bs-toggle="tooltip" title="Edit"  data-title="Manage Taxpayers">
    ...
    public function ActiveInactive(Request $request){
        ...
        $this->_cpdoClients->updateActiveInactive($id,$data);
    }
    public function getProfileDetails(Request $request){
        $id= $request->input('pid');
        $data = $this->_cpdoClients->getProfileDetails($id);
    ...
    public function store(Request $request){
        ...
        if($request->input('id')>0 && $request->input('submit')==""){
            $data = CpdoClients::find($request->input('id'));
             foreach ($this->_commonmodel->getBarangay($data->p_barangay_id_no)['data'] as $val) {
        ...
        if($request->isMethod('post')){
            ...
            {    $updatearay = array();
                 unset($this->data['id']);
                 $this->data['is_engg'] = '1';
            	 $this->_cpdoClients->updateData($request->input('clientsregistered'),$this->data);
            	 $success_msg = 'Taxpayer updated successfully';
                 $client_id = $request->input('clientsregistered');
            }else {
	            if($request->input('id')>0){  //echo "<pre>"; print_r($this->data);  echo $request->input('id'); exit;
	                $this->_cpdoClients->updateData($request->input('id'),$this->data);
	                $success_msg = 'Taxpayer updated successfully.';
	                ...
	                $client_id = $this->_cpdoClients->addData($this->data);
	                $success_msg = 'Taxpayer added successfully';
	            }
	        }
            ...
            if($request->ajax()){
                ...
                return redirect()->route('cpdoclients.index')->with('success', __($success_msg));
            }
        ...
        return view('cpdo.cpdoclients.create',compact('data','arrgetBrgyCode','arrgetCountries','arrgetClients'));
	...
	public function uploadDocument(Request $request){
        ...
        $arrEndrosment = CpdoClients::find($healthCertId);
        ...
        if(empty($message)){
            if($image = $request->file('file')) {
                ...
                $this->_cpdoClients->updateData($healthCertId,$data);
