...
class AcctgReceivablesController extends Controller
{
    ...
    public function __construct(AcctgAccountReceivableInterface $acctgAccountReceivableRepository, Carbon $carbon) 
    {
        ...
        $this->carbon = $carbon;
        $this->slugs = 'accounting/account-receivables';
    }
    ...
    {   
        ...
        return view('accounting.account-receivables.index')->with(compact('permission'));
    }
    ...
    { 
        ...
        $res = $result->data->map(function($receivable) use ($statusClass, $actions) {
            ...
            $funds = $receivable->fund ? wordwrap($receivable->fund->code . ' - ' . $receivable->fund->description, 25, "\n") : ''; 
            ...
            return [
                ...
                'funds' => '<div class="showLess" title="' . ($receivable->fund ? $receivable->fund->code . ' - ' . $receivable->fund->description : '') . '">' . $funds . '</div>',
