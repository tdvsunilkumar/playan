...
class AcctgDebitMemoController extends Controller
{
    ...
    private AcctgDebitMemoInterface $acctgDebitMemoRepository;
    ...
    public function __construct(
        ...
        AcctgDebitMemoInterface $acctgDebitMemoRepository,
        ...
        Carbon $carbon) 
    {
        ...
        $this->acctgDebitMemoRepository = $acctgDebitMemoRepository;
        $this->treasuryDisburseRepository = $treasuryDisburseRepository;
        $this->carbon = $carbon;
        $this->slugs = 'accounting/journal-entries/debit-memo';
    }
    ...
    {   
        ...
        return view('accounting.journal-entries.debit-memo.index')->with(compact('segment', 'accounting_permission', 'gl_accounts', 'uoms', 'evats', 'ewts', 'trans_types', 'vat_types'));
    }
    ...
    { 
        $statusClass = [
            ...
            'posting' => (object) ['bg' => 'bg-secondary', 'status' => 'draft'],
        ];
        $actions = ''; $actions2 = '';
        ...
        $result = $this->acctgDebitMemoRepository->listItems($request);
        $res = $result->data->map(function($voucher) use ($statusClass, $actions, $actions2) {
            ...
            $totalAmount = floatval($voucher->identityPayablesAmount) - floatval(currency_to_float($voucher->identityEWTAmount) + currency_to_float($voucher->identityEVATAmount));                  
            return [                
                'id' => $voucher->identity,
                'voucher' => $voucher->voucher_no ? $voucher->voucher_no : '',
                ...
                'total_payables' => '₱' .currency_format($totalAmount),
                'total_ewt' => '₱' .currency_format($voucher->identityEWTAmount),
                'total_evat' => '₱' .currency_format($voucher->identityEVATAmount),
                'total_disbursement' => '₱' .currency_format($voucher->identityDisbursementAmount),
                'total_deduction' => '₱' .currency_format($voucher->identityDeduction),
    ...
    {  
        ...
        $treasury_permission = array(
            'create' => $this->is_permitted('treasury/journal-entries/payables', 'create', 1),
            'read' => $this->is_permitted('treasury/journal-entries/payables', 'read', 1),
            'update' => $this->is_permitted('treasury/journal-entries/payables', 'update', 1),
            'delete' => $this->is_permitted('treasury/journal-entries/payables', 'delete', 1),
            'approve' => $this->is_permitted('treasury/journal-entries/payables', 'approve', 1),
            'disapprove' => $this->is_permitted('treasury/journal-entries/payables', 'disapprove', 1),
            'download' => $this->is_permitted('treasury/journal-entries/payables', 'download', 1)
        );
        $payees = [
            various_user()->cbo_payee->id => various_user()->cbo_payee->paye_name
        ];
        $gl_accounts = $this->acctgAccountVoucherRepository->allGLAccounts();
        ...
        $disburse_type = $this->acctgDebitMemoRepository->allDisburseType();
        return view('accounting.journal-entries.debit-memo.create')->with(compact('approvers', 'segment', 'accounting_permission', 'treasury_permission', 'fund_codes', 'payees', 'payment_types', 'sl_accounts', 'gl_accounts', 'uoms', 'evats', 'ewts', 'trans_types', 'vat_types','disburse_type'));
    }
    public function edit(Request $request, $id)
    {  
        $voucher = AcctgVoucher::find($id);
        // dd($voucher);
        if ($voucher->is_debit_memo === 0) {
            return abort(404);
        }
        ...
        $treasury_permission = array(
            'create' => $this->is_permitted('treasury/journal-entries/incomes', 'create', 1),
            'read' => $this->is_permitted('treasury/journal-entries/incomes', 'read', 1),
            'update' => $this->is_permitted('treasury/journal-entries/incomes', 'update', 1),
            'delete' => $this->is_permitted('treasury/journal-entries/incomes', 'delete', 1),
            'approve' => $this->is_permitted('treasury/journal-entries/incomes', 'approve', 1),
            'disapprove' => $this->is_permitted('treasury/journal-entries/incomes', 'disapprove', 1),
            'download' => $this->is_permitted('treasury/journal-entries/incomes', 'download', 1)
        );
    ...
    public function get_voucher()
    {
        ...
        return response()->json([
            ...
            'payroll' => (Session::get('voucher') > 0) ? $this->acctgDebitMemoRepository::get_payroll_data(Session::get('voucher')) : 0,
    ...
    public function approve_deduction(Request $request, $deductionID): JsonResponse 
    {
        if ($this->is_permitted($this->slugs, 'approve', 1) > 0) {
            ...
            return response()->json([
                'data' => $this->acctgDebitMemoRepository->approve_deduction($deductionID, $details),
    ...
    { 
        ...
        if ($this->is_permitted($this->slugs, 'update', 1) > 0) {
            $actions .= '<a href="javascript:;" class="second-modal-show action-btn bg-warning btn m-1 btn-sm align-items-center" title="view this" data-bs-toggle="tooltip" data-bs-placement="top" data-url="{{view_link}}"><i class="ti-search text-white"></i></a>';
        }
        ...
        $actions2 .= '<a href="javascript:;" class="second-modal-show action-btn bg-warning btn m-1 btn-sm align-items-center" title="view this" data-bs-toggle="tooltip" data-bs-placement="top" data-url="{{view_link}}"><i class="ti-search text-white"></i></a>';
        $actions3 .= '<a href="javascript:;" class="second-modal-show action-btn bg-warning btn m-1 btn-sm align-items-center" title="view this" data-bs-toggle="tooltip" data-bs-placement="top" data-url="{{view_link}}"><i class="ti-search text-white"></i></a>';
        $actions4 .= '<a href="javascript:;" class="second-modal-show action-btn bg-warning btn m-1 btn-sm align-items-center" title="view this" data-bs-toggle="tooltip" data-bs-placement="top" data-url="{{view_link}}"><i class="ti-search text-white"></i></a>';
        ...
        $result = $this->acctgDebitMemoRepository->collections_listItems($request, $id);
        $res = $result->data->map(function($payable) use ($statusClass, $actions, $actions2, $actions3, $actions4) {
            $gl_account = isset($payable->gl_account) ? wordwrap($payable->gl_account->code . ' - ' . $payable->gl_account->description, 25, "\n") : ''; 
            $items = $payable->items ? wordwrap($payable->items, 25, "\n") : '';         
            $view_link = '';
            if ($payable->gl_account) {
                $view_link = route('acctg-debit-memo.view',
                [
                    'gl_id'=>$payable->gl_account_id,
                    'voucher_id'=>$payable->voucher_id,
                ]);      
            }          
            return [
                'id' => $payable->identity,
                'checkbox' => ($payable->identityStatus == 'draft') ? '<div class="form-check"><input class="form-check-input" type="checkbox" value="'.$payable->gl_account_id.'"></div>' : '<div class="form-check"><input class="form-check-input" type="checkbox" value="'.$payable->gl_account_id.'" disabled="disabled"></div>',
                ...
                'uom' => '',
                'amount' => currency_format($payable->identityAmount),
                'total' => '<strong>' . currency_format($payable->identityTotal) . '</strong>',
                ...
                'debit' => $payable->gl_account->normal_balance === 'Debit' ? currency_format($payable->identityTotal): '0.00',
                'credit' => $payable->gl_account->normal_balance === 'Credit' ? currency_format($payable->identityTotal): '0.00',
                'responsibility_center' => $payable->responsibility_center,
                ...
                'actions' => str_replace('{{view_link}}', $view_link, $statusClass[$payable->identityStatus]->action )
            ];
        });
    ...
    { 
        $actions = ''; $actions2 = ''; $actions3 = ''; $actions4 = ''; $break = 0;
        if ($this->is_permitted($this->slugs, 'update', 1) > 0) {
            $actions .= '<a href="javascript:;" data-url="{{view_link}}" class="action-btn second-modal-show bg-warning btn m-1 btn-sm align-items-center" title="view this" data-bs-toggle="tooltip" data-bs-placement="top"><i class="ti-search text-white"></i></a>';
        }
        if ($this->is_permitted($this->slugs, 'delete', 1) > 0) {
            // $actions .= '<a href="javascript:;" class="action-btn remove-btn bg-danger btn m-1 btn-sm  align-items-center" title="remove this" data-bs-toggle="tooltip" data-bs-placement="top"><i class="ti-trash text-white"></i></a>';
        }
        $actions2 .= '<a href="javascript:;" data-url="{{view_link}}" class="action-btn second-modal-show bg-warning btn m-1 btn-sm align-items-center" title="view this" data-bs-toggle="tooltip" data-bs-placement="top"><i class="ti-search text-white"></i></a>';
        $actions3 .= '<a href="javascript:;" data-url="{{view_link}}" class="action-btn second-modal-show bg-warning btn m-1 btn-sm align-items-center" title="view this" data-bs-toggle="tooltip" data-bs-placement="top"><i class="ti-search text-white"></i></a>';
        $actions4 .= '<a href="javascript:;" data-url="{{view_link}}" class="action-btn second-modal-show bg-warning btn m-1 btn-sm align-items-center" title="view this" data-bs-toggle="tooltip" data-bs-placement="top"><i class="ti-search text-white"></i></a>';
        $actions4 .= '<a href="javascript:;" class="action-btn view-disapprove-btn bg-danger btn m-1 btn-sm align-items-center" title="view this" data-bs-toggle="tooltip" data-bs-placement="top"><i class="ti-comment-alt text-white"></i></a>';
        if ($this->is_permitted($this->slugs, 'approve', 1) > 0) {
            $actions2 .= '<br/><a href="javascript:;" class="action-btn approve-btn bg-success btn m-1 btn-sm align-items-center" title="approve this" data-bs-toggle="tooltip" data-bs-placement="top"><i class="ti-thumb-up text-white"></i></a>';
            $break++;
        ...
        $result = $this->acctgDebitMemoRepository->deductions_listItems($request, $id);
        $res = $result->data->map(function($payable) use ($statusClass, $actions, $actions2, $actions3, $actions4) {
            $view_link = '';
            if ($payable->gl_account) {
                $view_link = route('acctg-debit-memo.view-deduction',
                [
                    'gl_id'=>$payable->gl_account_id,
                    'voucher_id'=>$payable->voucher_id,
                ]);      
    ...
    public function employees_lists(Request $request, $id)
    {      
        $actions = '';
        if ($this->is_permitted($this->slugs, 'read', 1) > 0) {
            $actions .= '<a href="javascript:;" class="action-btn view-btn bg-warning btn m-1 btn-sm align-items-center" title="view this" data-bs-toggle="tooltip" data-bs-placement="top"><i class="ti-eye text-white"></i></a>';
        }
        $result = $this->acctgDebitMemoRepository->listEmployees($request, $id);
        $res = $result->data->map(function($requisition) use ($actions) {
            
            return [
                'id' => $requisition->payId,
                'employee_name' => wordwrap($requisition->employeeName, 25, "\n"),
                'department' => wordwrap($requisition->departmentName, 25, "\n"),
                'division' => wordwrap($requisition->divisionName, 25, "\n"),
                // 'particulars' => '<div class="showLess">' . $particulars . '</div>',
                'salary' => '₱'.currency_format($requisition->hrpr_net_salary),
                'deductions' => '<strong class="text-primary">'.'₱'.currency_format($requisition->hrpr_deductions).'</strong>',
                'actions' => $actions.'<input value="'.$requisition->payId.'" type="hidden" class="emp-id">'
            ];
        });
    ...
    {   
        if ($this->is_permitted($this->slugs, 'update', 1) > 0) {            
            ...
            $res = $this->acctgDebitMemoRepository->send_all_collections($request, $details);
            return response()->json([
    ...
    {   
        if ($this->is_permitted($this->slugs, 'update', 1) > 0) {            
            ...
            $res = $this->acctgDebitMemoRepository->send_all_deductions($request, $details);
            return response()->json([
                'data' => $this->update_vouchers($voucherID),
                'title' => 'Well done!',
    ...
    {
        $this->acctgDebitMemoRepository->update_vouchers($voucherID);
        return $this->acctgAccountVoucherRepository->find($voucherID);
    }

    public function send_all_payments(Request $request, $voucherID)
    {   
        // if ($this->is_permitted('treasury/journal-entries/debit-memo', 'update', 1) > 0) {
            $timestamp = $this->carbon::now();
            if ($this->is_permitted($this->slugs, 'approve', 1) > 0) {
                $details = array(
                    'status' => 'posted',
            ...
            AcctgVoucher::find($voucherID)->update(['status'=>'posting']);
            return response()->json([
                'data' => $this->update_vouchers($voucherID),
                'title' => 'Well done!',
    ...
    public function find_payments(Request $request, $id) 
    {   
        $this->is_permitted($this->slugs, 'read'); 
        return response()->json([
    ...
    public function edit_payables(Request $request, $id) 
    {   
        $this->is_permitted($this->slugs, 'read'); 
        return response()->json([
    ...
    { 
        ...
        $actions3 .= '<a href="javascript:;" class="action-btn print-btn bg-print btn m-1 btn-sm align-items-center" title="download this" data-bs-toggle="tooltip" data-bs-placement="top"><i class="ti-printer text-white"></i></a>';
        ...
        $res = $result->data->map(function($payment) use ($statusClass, $actions, $actions2, $actions3, $actions4, $iconClass) {
            ...
            $buttons = '';
            if ($statusClass[$payment->identityStatus]->status == 'posted' && $payment->type) {
                if ($payment->type == 'Cash' || $payment->type == 'Cheque') {
                    $buttons .= $iconClass[$payment->type->name]->button;
                }
            }
            $printVoucher = route('acctg-debit-memo.print',[
                'voucher' => $payment->voucher->voucher_no,
                'type' =>'cheque',
                'disburse_type' =>$payment->disburse->code,
            ]);
            $printVoucher = '<a href="'.$printVoucher.'" target="_blank" class="action-btn bg-info btn m-1 btn-sm align-items-center text-white" title="cheque disbursement" data-bs-toggle="tooltip" data-bs-placement="top"><i class="la la-file-text""></i></a>';
            return [
                'id' => $payment->identity,
                ...
                'disburse' => $payment->disburse ? $payment->disburse->name : '',
                // 'gl_account_label' => '<div class="showLess" title="' . $payment->sl_account->code . ' - ' . $payment->sl_account->description . '">' . $sl_account . '</div>',
                ...
                'total' => currency_format($payment->identityAmount),
                ...
                'actions' => $statusClass[$payment->identityStatus]->action . '' . $buttons . '' . $printVoucher
            ];
        });
        return response()->json([
            'request' => $request,
    ...
    public function view_available_payables(Request $request, $voucherID)
    {
        $this->is_permitted($this->slugs, 'read');
        return response()->json([
            'data' => $this->acctgAccountVoucherRepository->view_available_incomes($voucherID, $request->get('fund_code'), $request->get('payee'))->map(function($payable) {
    ...
    {           
        ...
        if ($id > 0) {
            ...
            $details = array(
                ...
                'is_replenish' => $request->get('replenish'),
                'updated_at' => $timestamp,
                'updated_by' => Auth::user()->id
            );
    ...
    public function fetch_status(Request $request, $voucherID)
    {
        $this->is_permitted($this->slugs, 'read'); 
        return response()->json([
            'status' => $this->acctgAccountVoucherRepository->find($voucherID)->status === 'posting' ? 'draft' : $this->acctgAccountVoucherRepository->find($voucherID)->status
        ]);
    }
    ...
    {   
        if ($this->is_permitted('treasury/journal-entries/incomes', 'delete', 1) > 0) {
            $details = array(
                'is_active' => 0,
                'updated_at' => $this->carbon::now(),
                'updated_by' => Auth::user()->id
    ...
    public function print(Request $request, $voucher)
    {
        ...
        if ($request->get('type') == 'cheque') {
            ...
            // payables
            $payables = $this->acctgDebitMemoRepository->get_to_pay($voucher, $request->get('disburse_type'), $posted = 1);
            foreach ($payables as $line) {
                $debit_sub = $this->acctgDebitMemoRepository->get_credit_sub($voucher, $line->gl_id, $posted = 1);
                $line_amt = ($debit_sub->isEmpty()) ? number_format($line->totalAmt, 2) : '';
                ...
                PDF::MultiCell(30, 5, $line_amt, 0, 'R', 0, 0, '', '', true);
                PDF::MultiCell(30, 5, '', 0, 'R', 0, 0, '', '', true);
                PDF::ln();

                $totalAmt += floatval($line->totalAmt);
                $totalDebit += floatval($line->totalAmt);
                foreach ($debit_sub as $line_sub) {
                    PDF::MultiCell(25.85, 5, $line_sub->centre, 0, 'C', 0, 0, '', '', true);
                    PDF::MultiCell(85, 5, '- '.$line_sub->sl_account, 0, 'L', 0, 0, '', '', true);
                    PDF::MultiCell(25, 5, '', 0, 'C', 0, 0, '', '', true);
                    PDF::MultiCell(30, 5, number_format($line_sub->totalAmt, 2), 0, 'R', 0, 0, '', '', true);
                    PDF::MultiCell(30, 5, '', 0, 'R', 0, 0, '', '', true);
                    PDF::ln();
                }
                PDF::ln(6);
            ...
            $glPayments = $this->acctgDebitMemoRepository->get_gl_payments($voucher, $request->get('type'), $request->get('reference_no'), $request->get('disburse_type'), 1);   
            if (!empty($glPayments)) {
                foreach ($glPayments as $glPayment) {
                    if (!($res->is_replenish > 0)) {
                        ...
                        PDF::MultiCell(30, 5, '', 0, 'R', 0, 0, '', '', true);
                        PDF::MultiCell(30, 5,'', 0, 'R', 0, 0, '', '', true);
                        $slPayments = $this->acctgDebitMemoRepository->get_sl_payments($voucher, $glPayment->gl_id, $request->get('reference_no'), $request->get('disburse_type'), 1);   
                        if (!empty($slPayments)) {
                            PDF::SetFont('Helvetica', 'B', 9);
            ...
            PDF::MultiCell(30, 5, '', 0, 'R', 0, 0, '', '', true);
            // cheque end
        } else if ($request->get('type') == 'others') {
            $debit = $this->acctgDebitMemoRepository->get_debit($voucher, $posted = 1);
            // dd($debit);
            if (!empty($debit)) {          
                $iteration = 0; $identities = array();
                foreach ($debit as $line) {  
                    $debit_sub = $this->acctgDebitMemoRepository->get_debit_sub($voucher, $line->gl_id, $posted = 1);
                    $line_amt = ($debit_sub->isEmpty() && $debit_sub) ? number_format($line->totalAmt, 2) : '';
                    ...
                    foreach ($debit_sub as $line_sub) {
                        if ($line_sub) {
                            ...
                            PDF::MultiCell(25, 5, $line->gl_code, 0, 'C', 0, 0, '', '', true);
                            PDF::MultiCell(30, 5, currency_format($line->totalAmt), 0, 'R', 0, 0, '', '', true);
                            PDF::MultiCell(30, 5, '', 0, 'R', 0, 0, '', '', true);
                            PDF::ln(6);
                        }
                    }
            ...
            $credit = $this->acctgDebitMemoRepository->get_credit($voucher, $posted = 1);
            // dd($credit);
            if (!empty($credit)) {          
                $iteration = 0; $identities = array();
                foreach ($credit as $line) {  
                    $credit_sub = $this->acctgDebitMemoRepository->get_credit_sub($voucher, $line->gl_id, $posted = 1);
                    $line_amt = ($credit_sub->isEmpty()) ? number_format($line->totalAmt, 2) : '';
                    ...
                    PDF::ln(6);
                    $totalAmt += floatval($line->totalAmt);
                    $totalCredit += floatval($line->totalAmt);
                    foreach ($credit_sub as $line_sub) {
                        PDF::MultiCell(25.85, 5, $line_sub->centre, 0, 'C', 0, 0, '', '', true);
                        PDF::MultiCell(85, 5, '- '.$line_sub->sl_account, 0, 'L', 0, 0, '', '', true);
                        PDF::MultiCell(25, 5, '', 0, 'C', 0, 0, '', '', true);
                        PDF::MultiCell(30, 5, '', 0, 'R', 0, 0, '', '', true);
    ...
    public function preview(Request $request, $voucher)
    {
        ...
        if ($request->get('type') == 'cheque') {
            ...
            // payables
            $payables = $this->acctgDebitMemoRepository->get_to_pay($voucher, $request->get('disburse_type'), $posted = 0);
            foreach ($payables as $line) {
                $debit_sub = $this->acctgDebitMemoRepository->get_credit_sub($voucher, $line->gl_id, $posted = 0);
                $line_amt = ($debit_sub->isEmpty()) ? number_format($line->totalAmt, 2) : '';
                PDF::MultiCell(25.85, 5, $line->centre, 0, 'C', 0, 0, '', '', true);
                PDF::MultiCell(85, 5, $line->gl_account, 0, 'L', 0, 0, '', '', true);
                PDF::MultiCell(25, 5, $line->gl_code, 0, 'C', 0, 0, '', '', true);
            ...
            $glPayments = $this->acctgDebitMemoRepository->get_gl_payments($voucher, $request->get('type'), $request->get('reference_no'), $request->get('disburse_type'), 0);   
            if (!empty($glPayments)) {
                foreach ($glPayments as $glPayment) {
                    if (!($res->is_replenish > 0)) {
                        PDF::SetFont('Helvetica', '', 9);
                        ...
                        PDF::MultiCell(30, 5, '', 0, 'R', 0, 0, '', '', true);
                        PDF::MultiCell(30, 5,'', 0, 'R', 0, 0, '', '', true);
                        $slPayments = $this->acctgDebitMemoRepository->get_sl_payments($voucher, $glPayment->gl_id, $request->get('reference_no'), $request->get('disburse_type'), 0);   
                        if (!empty($slPayments)) {
                            PDF::SetFont('Helvetica', 'B', 9);
                            foreach ($slPayments as $slPayment) {
                                PDF::ln();
            ...
            $debit = $this->acctgDebitMemoRepository->get_debit($voucher, $posted = 0);
            // dd($debit);
            if (!empty($debit)) {          
                $iteration = 0; $identities = array();
                foreach ($debit as $line) {  
                    $debit_sub = $this->acctgDebitMemoRepository->get_debit_sub($voucher, $line->gl_id, $posted = 0);
                    $line_amt = ($debit_sub->isEmpty() && $debit_sub) ? number_format($line->totalAmt, 2) : '';
            ...
            $credit = $this->acctgDebitMemoRepository->get_credit($voucher, $posted = 0);
            // dd($credit);
            if (!empty($credit)) {          
                $iteration = 0; $identities = array();
                foreach ($credit as $line) {  
                    $credit_sub = $this->acctgDebitMemoRepository->get_credit_sub($voucher, $line->gl_id, $posted = 0);
                    $line_amt = ($credit_sub->isEmpty()) ? number_format($line->totalAmt, 2) : '';
    ...
    {  
        ...
        $segment = request()->segment(count(request()->segments()) - 2);
        $approvers = $this->acctgAccountVoucherRepository->get_voucher_approvers();
        return view('accounting.journal-entries.debit-memo.view')->with(compact('accounting_permission', 'treasury_permission','approvers', 'segment', 'fund_codes', 'payees', 'payment_types', 'sl_accounts', 'sl_accountx', 'gl_accounts', 'uoms', 'evats', 'ewts', 'trans_types', 'vat_types'));
    }
    public function viewGL($gl_id, $voucher_id){
        $payrolls = AcctgDebitMemo::where([
            'gl_account_id' => $gl_id,
            'voucher_id' => $voucher_id,
        ])
        ->groupby('trans_no')->get();
        return view('accounting.journal-entries.debit-memo.viewGL')->with(compact('payrolls'));
    } 
    public function view_deduction($gl_id, $voucher_id){
        $payrolls = AcctgAccountDeduction::where([
            'gl_account_id' => $gl_id,
            'voucher_id' => $voucher_id,
        ])->get();
        return view('accounting.journal-entries.debit-memo.viewGL')->with(compact('payrolls'));
    ...
    {   
        ...
        return response()->json([
            'data' => $this->acctgAccountVoucherRepository->update($id, $details),
            'title' => 'Well done!',
            'text' => 'The journal entry voucher has been successfully completed.',
            'type' => 'success',
            'class' => 'btn-brand'
        ]);
    }
