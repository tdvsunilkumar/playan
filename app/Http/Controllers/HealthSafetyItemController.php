...
class HealthSafetyItemController extends Controller
{
    ...
    public function __construct(GsoItemRepositoryInterface $gsoItemRepository, Carbon $carbon) 
    {
        ...
        $this->carbon = $carbon;
        $this->slugs = 'health-and-safety/setup-data/item-managements';
    }
    ...
    {   
        ...
        $medical_categories = $this->gsoItemRepository->allMedicalCategories();
        $canDownload = $this->is_permitted($this->slugs, 'download', 1);
        return view('health-and-safety.setup-data.item-managements.index')->with(compact('canDownload', 'medical_categories', 'gl_accounts', 'item_categories', 'item_types', 'pur_types', 'unit_of_measurements'));
    }
    ...
    { 
        ...
        $result = $this->gsoItemRepository->health_listItems($request);
    ...
    {       
        ...
        $details = array(
            ...
            'is_expirable' => $request->get('is_expirable'),
    ...
    public function validate_category(Request $request, $item_category)
    {
        $this->is_permitted($this->slugs, 'read');
        return response()->json([
            'data' => $this->gsoItemRepository->validate_category($item_category)
        ]);
    }
