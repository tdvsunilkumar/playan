...
class EndrosementList implements WithEvents, WithStyles, WithColumnWidths, WithTitle, FromView
{
   private $keywords;
    ...
    {   
        ...
        $res = $this->_Endrosement->getDataExport();
        ...
        return [
            AfterSheet::class => function(AfterSheet $event) use ($res, $keywords) {
                ...
                if (!empty($res)) {
                    ...
                    $event->sheet->setCellValue('B'.$rows, 'BUSINESS ID-NO.'); 
                    $event->sheet->setCellValue('C'.$rows, 'OWNER NAME'); 
					$event->sheet->setCellValue('D'.$rows, 'CONTACT NUMBER');
                    $event->sheet->setCellValue('E'.$rows, 'BUSINESS NAME');
					$event->sheet->setCellValue('F'.$rows, 'ADDRESS');
                    $event->sheet->setCellValue('G'.$rows, 'TYPE'); 
                    $event->sheet->setCellValue('H'.$rows, 'DATE');
					$event->sheet->setCellValue('I'.$rows, 'O.R. NUMBER');
					$event->sheet->setCellValue('J'.$rows, 'O.R. AMOUNT');
					$event->sheet->setCellValue('K'.$rows, 'O.R. DATE');
					$event->sheet->setCellValue('L'.$rows, 'FSIC NUMBER');
					$event->sheet->setCellValue('M'.$rows, 'ISSUANCE DATE');
                    $event->sheet->setCellValue('N'.$rows, 'EXPIRATION DATE'); 
					$event->sheet->setCellValue('O'.$rows, 'METHOD');
                    $event->sheet->setCellValue('P'.$rows, 'APPLICATION STATUS'); 
                    $event->sheet->setCellValue('Q'.$rows, 'STATUS'); 
                    $event->sheet->setCellValue('R'.$rows, 'DURATION');
                    ...
                    foreach ($res as $val) {
                        ...
                        $event->sheet->setCellValue('C'.$rows, ($val->ownar_name ? $val->ownar_name : ''));
						$event->sheet->setCellValue('D'.$rows, ($val->p_mobile_no ? $val->p_mobile_no : '')); 
                        $event->sheet->setCellValue('E'.$rows, ($val->busn_name ? $val->busn_name : ''));
						$event->sheet->setCellValue('F'.$rows,  $this->_commonmodel->getTaxPayerAddress($val->client_id));
                        $event->sheet->setCellValue('G'.$rows, ($val->app_code>0)?config('constants.arrBusinessApplicationType')[(int)$val->app_code]:'');
                        $event->sheet->setCellValue('H'.$rows,  $val->created_at);
						$event->sheet->setCellValue('I'.$rows, ($val->orno ? $val->orno : ''));
						$event->sheet->setCellValue('J'.$rows, ($val->oramount ? number_format($val->oramount, 2) : ''));
						$event->sheet->setCellValue('K'.$rows, ($val->ordate ? date("d-m-Y", strtotime($val->ordate)) : ''));
						$event->sheet->setCellValue('L'.$rows, ($val->bfpcert_no ? $val->bfpcert_no : ''));
						$event->sheet->setCellValue('M'.$rows, ($val->bfpcert_date_issue ? date("d-m-Y", strtotime($val->bfpcert_date_issue)) : ''));
						$event->sheet->setCellValue('N'.$rows, ($val->bfpcert_date_expired ? $val->bfpcert_date_expired : ''));
                        $event->sheet->setCellValue('O'.$rows, $val->busn_app_method); 
                        $event->sheet->setCellValue('P'.$rows, config('constants.arrBusinessApplicationStatus')[$val->busn_app_status]);
						$event->sheet->setCellValue('Q'.$rows, (config('constants.arrBusEndorsementStatus')[(int)$val->bend_status]));
                        $startCarbon = date('Y-m-d',strtotime($val->start_date));

                            if ($val->bend_completed_date != null) {
                                $endCarbon = date('Y-m-d',strtotime($val->bend_completed_date));
                            } else {
                                $endCarbon = date('Y-m-d'); // Use Carbon to get the current date and time
                            }
                            //  dd($endCarbon);
    ...
    {   
        return view('Bplo.endorsement.excelView', ['keywords' => $this->keywords]);
    }
