...
class BusinessGovDtiList implements WithEvents, WithStyles, WithColumnWidths, WithTitle, FromView
{
   private $keywords;
    ...
    {   
        ...
        $res = $this->_businessgovdtilist->getDataExport();
        ...
        return [
            AfterSheet::class => function(AfterSheet $event) use ($res, $keywords) {
                ...
                if (!empty($res)) {
                    ...
                    $event->sheet->setCellValue('E'.$rows, 'DATE ISSUED'); 
                    ...
                    $event->sheet->setCellValue('I'.$rows, 'OWNER NAME'); 
                    $event->sheet->setCellValue('J'.$rows, 'BUSINESS ADDRESS'); 
                    $event->sheet->setCellValue('K'.$rows, 'LINE OF BUSINESS');
                    $event->sheet->setCellValue('L'.$rows, 'CAPITAL INVESTMENT');
                    $event->sheet->setCellValue('M'.$rows, 'GROSS SALE');
                    $event->sheet->setCellValue('N'.$rows, 'SIZE OF BUSINESS');
                    $event->sheet->setCellValue('O'.$rows, 'O.R NUMBER');
                    $event->sheet->setCellValue('P'.$rows, 'CONTACT NO.');
                    $event->sheet->setCellValue('Q'.$rows, 'EMAIL ADDRESS');
                    $rows++;
                    $count=0;
                    foreach ($res as $val) {
                        $count=$count+1;
                        ...
                        $event->sheet->setCellValue('E'.$rows, ($val->bpi_issued_date !== NULL) ? date('d-M-Y H:i', strtotime($val->bpi_issued_date)) : date('d-M-Y H:i', strtotime($val->bpi_issued_date)));
                        ...
                        $event->sheet->setCellValue('H'.$rows, $val->application_date); 
                        $complete_address=$this->_commonmodel->getbussinesAddress($val->busn_id);
                        $event->sheet->setCellValue('I'.$rows, $val->rpo_first_name." ".$val->rpo_middle_name." ".$val->rpo_custom_last_name);; 
                        $lineofbdata=$this->_businessgovdtilist->getBusinessPSIC($val->busn_id); $lineofbusiness = "";
                        ...
                        $event->sheet->setCellValue('J'.$rows, $complete_address); 
                        $event->sheet->setCellValue('K'.$rows, $lineofbusiness); 
                        $event->sheet->setCellValue('L'.$rows, $capitalinvestment); 
                        $event->sheet->setCellValue('M'.$rows, $grosssale); 
                        $event->sheet->setCellValue('N'.$rows, $this->getSizeByCap($capitalinvestment)); 
                        $event->sheet->setCellValue('O'.$rows, $val->ornumber); 
                        $event->sheet->setCellValue('P'.$rows, $val->p_mobile_no);
                        $event->sheet->setCellValue('Q'.$rows, $val->p_email_address);  
                        $rows++;
                    }
                }
            },
    ...
    {   
        return view('report.businessdtilist.view', ['keywords' => $this->keywords]);
    }
