...
@section('page-title')
    {{__('Annual Procurement Plan')}}
@endsection

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{route('dashboard')}}">{{ __('Dashboard') }}</a></li>
    <li class="breadcrumb-item">{{ __('Finance') }}</li>
    <li class="breadcrumb-item">{{ __('Annual Procurement Plan') }}</li>
@endsection
@section('action-btn')
    <div class="float-end">
        <a href="{{ url('finance/procurement-plan') }}" data-size="lg" data-bs-toggle="tooltip" data-bs-placement="left" title="{{__('Back to Manage APP')}}" class="btn btn-sm btn-primary add-btn">
...
@section('content')
        ...
        <div class="col-xl-12">
            <div id="ppmp-card" class="card table-card">
                <div class="card-header">
                    <h5>Annual Procurement Plan Details</h5>
                </div>
                <div class="card-body">
                    {{ Form::open(array('url' => 'finance/procurement-plan', 'class'=>'formDtls needs-validation', 'name' => 'ppmpForm')) }}
                    @csrf
                    <div class="row">
                        <div class="col-sm-6">
                                                ...
                                                <tbody data-row-gl="{{ $budget->gl_account->id }}" data-row-gl-code="{{ $budget->gl_account->code }}">
                                                    ...
                                                    @if (!empty($lines))
                                                        @foreach ($lines as $line)
                                                            <tr class="{{ ($ppmp->budget_status != 'draft' || $line->is_departmental > 0) ? 'active' : '' }}" data-row-id="{{ $line->id }}" data-row-gl-code="{{ $line->gl_account ? $line->gl_account->code : '' }}" data-row-division="{{ $line->division->id }}" data-row-division-code="{{ $line->division->code }}" data-row-cost="{{ $line->budget_amount }}" data-row-total="{{ $line->budget_total_amount }}" data-row-item="{{ $line->item_id }}" data-row-uom="{{ $line->uom_id }}" data-row-quantity="{{ $line->budget_quantity }}" data-row-gl="{{ $line->gl_account_id }}">
                                                                ...
                                                                <td rowspan="1" colspan="1" class="text-left" width="25%">
                                                                    @if ($ppmp->budget_status != 'draft' || $line->is_departmental > 0)
                                                                ...
                                                                <td rowspan="1" colspan="1" class="text-center" width="10%">
                                                                    @if ($ppmp->budget_status != 'draft')
                                                                        <input type="text" class="text-center numeric-double" name="quantity[]" value="{{ $line->budget_quantity }}" disabled="disabled">
                                                                    @else
                                                                        <input type="text" class="text-center numeric-double" name="quantity[]" value="{{ $line->budget_quantity }}">
                                                                    @endif
                                                                </td>
                                                                <td rowspan="1" colspan="1" class="text-center fw-bold">{{ $line->uom ? $line->uom->code : '' }}</td>
                                                                <td rowspan="1" colspan="1" class="text-center fw-bold">{{ number_format($line->budget_total_amount, 2) }}</td>
                                                                ...
                                                                <td rowspan="1" colspan="1" class="text-center">
                                                                    <div class="d-flex m-auto justify-content-center">
                                                                        <a href="javascript:;" class="action-btn add-btn bg-info btn btn-sm ms-1 mt-1 mb-1 align-items-center d-flex" data-bs-toggle="tooltip" data-bs-placement="top" title="add this"><i class="ti-plus text-white"></i></a>
                                                            ...
                                                            @php $totalAmt += floatval($line->budget_total_amount); $iteration++; @endphp
                                                        @endforeach
                                                    @endif     
                                                </tbody>
                                                <tfoot>
    ...
    @endif
    @if ($ppmp->budget_status == 'draft')
    <!-- <button id="ppmp-send-btn" class="btn btn-lg bg-info align-items-center btn-circle me-2 active" data-bs-toggle="tooltip" data-bs-placement="top" title="lock this">
