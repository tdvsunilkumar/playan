...
@section('page-title')
    {{__('Planning & Development: Cashiering')}}
@endsection
@push('script-page')
@endpush
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{route('dashboard')}}">{{__('Dashboard')}}</a></li>
    <li class="breadcrumb-item">{{__('Planning & Development: Cashiering')}}</li>
@endsection
@section('action-btn')
    <div class="float-end">
        ...
        <a href="#" data-size="xxll" data-url="{{ url('/cpdocashering/store') }}" data-ajax-popup="true" data-bs-toggle="tooltip" title="{{__('Manage Zoning Clearance Fee')}}" class="btn btn-sm btn-primary">
            <i class="ti-plus"></i>
        </a>
    </div>
@endsection
@section('content')
    ...
    @if(session()->has('CPDO_PRINT_CASHIER_ID'))
        <iframe id="openPrintDialog" src="@php echo url('/cpdocashering/printReceipt?id='.Session::get('CPDO_PRINT_CASHIER_ID')) @endphp" class="hide" width="100%" height="800px"></iframe>
    @php  Session::forget('CPDO_PRINT_CASHIER_ID'); @endphp
    @endif
    <script src="{{ asset('js/Cpdo/cpdocashering.js') }}?rand={{ rand(000,999) }}"></script>
@endsection
