...
@section('page-title')
    {{__('Community Tax:Cashiering')}}
@endsection
@push('script-page')
@endpush
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{route('dashboard')}}">{{__('Dashboard')}}</a></li>
    <li class="breadcrumb-item">{{__('Cashiering')}}</li>
...
@section('action-btn')
    <div class="float-end">
        ...
        <a href="#" data-size="xxll" data-url="{{ url('/community-tax/store') }}" data-ajax-popup="true" data-bs-toggle="tooltip" title="{{__('Manage Community Tax Certificate Payment')}}" class="btn btn-sm btn-primary">
            <i class="ti-plus"></i>
        </a>
    </div>
@endsection
...
@section('content')
   <div class="row" id="this_is_filter">
    ...
    @if(session()->has('COMMUNITY_PRINT_CASHIER_ID'))
         <iframe id="openPrintDialog" src="@php echo url('/cashier/community-tax/printReceipt?id='.Session::get('COMMUNITY_PRINT_CASHIER_ID')) @endphp" class="hide" width="100%" height="800px"></iframe>
        @php  Session::forget('COMMUNITY_PRINT_CASHIER_ID'); @endphp
    @endif
    
    <script src="{{ asset('js/communitytax.js') }}?rand={{ rand(000,999) }}"></script>
@endsection
