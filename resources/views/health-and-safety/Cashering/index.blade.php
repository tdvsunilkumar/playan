...
@section('page-title')
    {{__('Health And Safety Cashiering')}}
@endsection
@push('script-page')
@endpush
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{route('dashboard')}}">{{__('Dashboard')}}</a></li>
    <li class="breadcrumb-item">{{__('Health And Safety Cashiering')}}</li>
@endsection
@section('action-btn')
    <div class="float-end">
        <a href="#" id="filter_box" class="btn btn-sm btn-primary action-item" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        <i class="ti-filter"></i></a>
        <a href="#" data-size="xxll" data-url="{{url('/cashier/HealthandSafety/store') }}" data-ajax-popup="true" data-bs-toggle="tooltip" title="{{__('Manage Health And Safety Cashiering')}}" class="btn btn-sm btn-primary">
            <i class="ti-plus"></i>
        </a>
    </div>
@endsection
@section('content')
    ...
    @if(session()->has('HEALT_PRINT_CASHIER_ID'))
         <iframe id="openPrintDialog" src="@php echo url('/cashier/HealthandSafety/printReceipt?id='.Session::get('HEALT_PRINT_CASHIER_ID')) @endphp" class="hide" width="100%" height="800px"></iframe>
        @php  Session::forget('HEALT_PRINT_CASHIER_ID'); @endphp
    @endif
    <script src="{{ asset('js/HealthandSafety/HealthandSafetyCashering.js') }}?rand={{ rand(000,999) }}"></script>
@endsection
