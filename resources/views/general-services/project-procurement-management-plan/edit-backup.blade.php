                    ...
                    <nav>
                        <div class="nav nav-tabs" id="nav-tab" role="tablist">
                            ...
                            @foreach ($budgets as $budget)
                                <button class="nav-link {{ ($i == 0) ? 'active' : '' }}" id="nav-{{ $budget->gl_account->code }}" data-bs-toggle="tab" data-bs-target="#tab-{{ $budget->gl_account->code }}" type="button" role="tab" aria-controls="nav-home" aria-selected="{{ ($i == 0) ? 'true' : 'false' }}"><span data-bs-toggle="tooltip" data-bs-placement="top" title="{{ $budget->gl_account->description }}">{{ $budget->gl_account->code }}</span></button>
                            @php $i++; @endphp
                            @endforeach
                        </div>
                    </nav>
                    ...
                    <div class="tab-content" id="nav-tabContent">
                        ...
                        @foreach ($budgets as $budget)
                            <div class="tab-pane fade {{ ($i == 0) ? 'show active' : '' }}" id="tab-{{ $budget->gl_account->code }}" role="tabpanel" aria-labelledby="nav-{{ $budget->gl_account->code }}">
                                <h5 class="d-flex">
                                    <div class="me-auto total-gl-budget" data-row-total-gl-budget="{{ number_format(floor(($budget->annual_budget*100))/100, 2, '.', '') }}">
                                        [ <span class="text-primary">{{ $budget->gl_account->code }}</span> ] <span class="text-primary">{{ $budget->gl_account->description }}</span> - <span class="fs-4 text-danger">{{ number_format(floor(($budget->annual_budget*100))/100, 2) }}</span>
                                    </div>
                                </h5>
                                <!-- START -->
                                <div class="row mt-4">
                                                ...
                                                <tbody data-row-gl="{{ $budget->gl_account->id }}" data-row-gl-code="{{ $budget->gl_account->code }}">
                                                    ...
                                                    @if (!empty($lines))
                                                        @foreach ($lines as $line)
                                                            <tr class="{{ ($locked > 0) ? 'active' : '' }}" data-row-id="{{ $line->id }}" data-row-cost="{{ $line->amount }}" data-row-total="{{ $line->total_amount }}" data-row-item="{{ $line->item_id }}" data-row-uom="{{ $line->uom_id }}" data-row-quantity="{{ $line->quantity }}" data-row-gl="{{ $line->gl_account_id }}">
