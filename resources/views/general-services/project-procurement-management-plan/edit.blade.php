...
@section('content')
    ...
    <div class="row">
        <div class="col-xl-12">
            <div id="ppmp-details-card" class="card table-card noflow">
                    ...
                    <div class="tab-content" id="nav-tabContent">
                        ...
                        @foreach ($budgets as $budget)    
                            <div class="tab-pane fade {{ ($i == 0) ? 'show active' : '' }}" id="tab-{{ $budget->gl_account->code }}" role="tabpanel" aria-labelledby="nav-{{ $budget->gl_account->code }}">
                                <h5 class="d-flex">
                                    <div class="me-auto total-gl-budget" data-row-total-gl-budget="{{ ($budget_plans[$budget->gl_account->id]->amount ? number_format(floor(($budget_plans[$budget->gl_account->id]->amount*100))/100, 2, '.', '') : number_format(0, 2, '.', '') ) }}">
                                        [ <span class="text-primary">{{ $budget->gl_account->code }}</span> ] <span class="text-primary">{{ $budget->gl_account->description }}</span> - <span class="fs-4 text-danger">{{ ($budget_plans[$budget->gl_account->id]->amount ? number_format(floor(($budget_plans[$budget->gl_account->id]->amount*100))/100, 2) : number_format(0, 2) ) }}</span>
                                    </div>
                                </h5>    
                                <!-- START -->
                                <div class="row mt-4">
                                    <div class="col-md-12">
                                        <div class="table-responsive">
                                            <table id="ppmp-table" class="table table-{{ $budget->gl_account->id }} table-striped table-bordered">
                                                <thead>
                                                    <tr>
                                                        <th rowspan="2" colspan="1" class="text-center">#</th>
                                                    ...
                                                    @if (!empty($lines))
                                                        @foreach ($lines as $line)
                                                            <tr class="{{ ($locked > 0) ? 'active' : '' }}" data-row-id="{{ $line->id }}" data-row-gl-code="{{ $line->gl_account ? $line->gl_account->code : '' }}" data-row-division="{{ $line->division->id }}" data-row-division-code="{{ $line->division->code }}" data-row-cost="{{ $line->amount }}" data-row-total="{{ $line->total_amount }}" data-row-item="{{ $line->item_id }}" data-row-uom="{{ $line->uom_id }}" data-row-quantity="{{ $line->quantity }}" data-row-gl="{{ $line->gl_account_id }}">
                                                                <td rowspan="1" colspan="1" class="text-center">{{ $iteration}}</td>
