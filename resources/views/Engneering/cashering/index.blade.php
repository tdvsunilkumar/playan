...
@section('page-title')
    {{__('Engineering Cashiering')}}
@endsection
@push('script-page')
@endpush
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{route('dashboard')}}">{{__('Dashboard')}}</a></li>
    <li class="breadcrumb-item">{{__('Engineering Cashiering')}}</li>
...
@section('action-btn')
    <div class="float-end">
        ...
        <a href="#" data-size="xxll" data-url="{{ url('/engcashering/store') }}" data-ajax-popup="true" data-bs-toggle="tooltip" title="{{__('Manage Engineering Cashiering')}}" class="btn btn-sm btn-primary">
            <i class="ti-plus"></i>
        </a>
    </div>
@endsection
...
@section('content')
    ...
    <div class="row">
      ...
      @if(session()->has('ENGG_PRINT_CASHIER_ID'))
         <iframe id="openPrintDialog" src="@php echo url('/engcashering/printengCasheringtax?id='.Session::get('ENGG_PRINT_CASHIER_ID')) @endphp" class="hide" width="100%" height="800px"></iframe>
        @php  Session::forget('ENGG_PRINT_CASHIER_ID'); @endphp
    @endif
    <script src="{{ asset('js/Engneering/engcashering.js') }}?rand={{ rand(000,999) }}"></script>
@endsection
