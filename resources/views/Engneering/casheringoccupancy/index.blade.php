...
@section('page-title')
    {{__('Occupancy Cashiering')}}
@endsection
@push('script-page')
@endpush
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{route('dashboard')}}">{{__('Dashboard')}}</a></li>
    <li class="breadcrumb-item">{{__('Occupancy Cashiering')}}</li>
...
@section('action-btn')
    <div class="float-end">
        ...
        <a href="#" data-size="xxll" data-url="{{ url('/occupancycashering/store')}}" data-ajax-popup="true" data-bs-toggle="tooltip" title="{{__('Manage Occupancy Cashiering')}}" class="btn btn-sm btn-primary">
            <i class="ti-plus"></i>
        </a>
    </div>
@endsection
...
@section('content')
    ...
    @if(session()->has('OCCUPANCY_PRINT_CASHIER_ID'))
         <iframe id="openPrintDialog" src="@php echo url('/occupancycashering/printoccupancyCasheringtax?id='.Session::get('OCCUPANCY_PRINT_CASHIER_ID')) @endphp" class="hide" width="100%" height="800px"></iframe>
        @php  Session::forget('OCCUPANCY_PRINT_CASHIER_ID'); @endphp
    @endif
    <script src="{{ asset('js/Engneering/occupancycashering.js') }}?rand={{ rand(000,999) }}"></script>
@endsection
