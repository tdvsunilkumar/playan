...
@section('action-btn')
    <div class="float-end">
        ...
        <a href="#" data-size="xxll" data-url="{{url('/cashier/Miscellaneous/store') }}" data-ajax-popup="true" data-bs-toggle="tooltip" title="{{__('Manage Miscellaneous Cashiering')}}" class="btn btn-sm btn-primary">
            <i class="ti-plus"></i>
        </a>
    </div>
@endsection
@section('content')
    ...
    @if(session()->has('MISCE_PRINT_CASHIER_ID'))
         <iframe id="openPrintDialog" src="@php echo url('/cashier/Miscellaneous/printReceipt?id='.Session::get('MISCE_PRINT_CASHIER_ID')) @endphp" class="hide" width="100%" height="800px"></iframe>
        @php  Session::forget('MISCE_PRINT_CASHIER_ID'); @endphp
    @endif
    <script src="{{ asset('js/SocialWelfare/MiscellaneousCashiering.js') }}?rand={{ rand(000,999) }}"></script>
@endsection
