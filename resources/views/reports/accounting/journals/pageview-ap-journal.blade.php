...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item">{{ __('Accounts Payable Journal') }}</li>
@endsection
                            ...
                            <table class="table" cellspacing="0" cellpadding="0">
                                <thead>
                                    <tr>
                                        ...
                                        <th rowspan="1" colspan="2"  class="text-center">Debit</th>
                                        <th rowspan="1" colspan="1"  class="text-center">Credit</th>
                                    </tr>
                                    <tr>
                                        <th rowspan="2" colspan="1" class="text-center">Account Code</th>
                                        <th rowspan="2" colspan="1"  class="text-center">Amount</th>
                                        <th rowspan="2" colspan="1"  class="text-center">{{$account_payable->description}}<br/>{{$account_payable->code}}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @if (!empty($rows)) 
                                        ...
                                        @foreach ($rows as $row)
                                            <tr>
                                                ...
                                                <td rowspan="1" colspan="1"  class="text-left">{{ $row->payee }}r</td>
                                                ...
                                                <td rowspan="1" colspan="1"  class="text-center">
                                                    @foreach ($row->payables as $payable)
                                                        {{ $payable->account_code }}<br/>
                                                    @endforeach
                                                </td>
                                                <td rowspan="1" colspan="1"  class="text-end">
                                                    @php $totalRowAmt = 0; @endphp
                                                    @foreach ($row->payables as $payable)
                                                        @php 
                                                            $totalRowAmt += floatval($payable->amount); 
                                                            $totalAmt += floatval($payable->amount); 
                                                        @endphp
                                                        {{ $controller->money_format($payable->amount) }}<br/>
                                                    @endforeach
                                                </td>
                                                <td rowspan="1" colspan="1"  class="text-end">{{ $controller->money_format($totalRowAmt) }}</td>
                                            </tr>
                                        @endforeach
                                        <tr>
                                            <td colspan="5" class="text-start fw-bold">TOTAL AMOUNT</td>
                                            <td rowspan="1" colspan="1"  class="text-end fw-bold">{{ $controller->money_format($totalAmt) }}</td>
                                            <td rowspan="1" colspan="1"  class="text-end fw-bold">{{ $controller->money_format($totalAmt) }}</td>
                                        </tr>
                                    @else
                                        <tr>
