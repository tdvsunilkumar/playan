...
@section('action-btn')
    <div class="float-end">
            ...
            <a href="#" data-size="xll" data-url="{{ url('/cashier-real-property/store') }}" title="{{__('Manage Real Property Cashiering')}}" class="btn btn-sm btn-primary addNewCashierRealProperty">
            <i class="ti-plus"></i>
        </a>
...
@section('content')
    ...
    @if(session()->has('RPT_PRINT_CASHIER_ID'))
         <iframe id="openPrintDialog" src="@php echo url('/cashier-real-property/printReceipt?id='.Session::get('RPT_PRINT_CASHIER_ID')) @endphp" class="hide" width="100%" height="800px"></iframe>
        @php  Session::forget('RPT_PRINT_CASHIER_ID'); @endphp
    @endif
    <script src="{{ asset('js/cashierrealproperty/index.js') }}?rand={{ rand(000,999) }}"></script>
@endsection
