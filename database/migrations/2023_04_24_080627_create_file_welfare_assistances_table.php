...
class CreateFileWelfareAssistancesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('files_welfare_assistance', function (Blueprint $table) {
            $table->id();
            $table->integer('wswa_id')->length(11)->comment('ref-Table: welfare_social_welfare_assistance.wswa_id');
            $table->string('fwa_name');
            $table->string('fwa_type');
            $table->decimal('fwa_size');
            $table->string('fwa_path');
            $table->integer('fwa_is_active')->length(1)->comment('if the status of the application type is active or not');
    ...
    {
        Schema::dropIfExists('files_welfare_assistance');
    }
}
