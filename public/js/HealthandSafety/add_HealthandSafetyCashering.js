...
$(document).ready(function(){
    ...
    select3Ajax("clientcitizenid","clientdivselect","citizens/getCitizens");
    ...
    $("#various_payment").click(function(){
        if ($(this).is(':checked')) {
            $("#cashier_particulars").val("Permit Payments")
        }else{
...
function uploadAttachment(){
    ...
    showLoader();
    $.ajax({
       url : DIR+'cashier/HealthandSafety/uploadDocument',
       type : 'POST',
       data : formData,
   ...
   swalWithBootstrapButtons.fire({
       ...
       reverseButtons: true
           ...
           if(result.isConfirmed){
               showLoader();
              $.ajax({
               url :DIR+'cashier/HealthandSafety/deleteAttachment', // json datasource
               type: "POST", 
               data: {
                    "id":$("#id").val(),
                    "fname": fname,
...
function refreshCitizen(){
   $.ajax({
        url :DIR+'cashier/HealthandSafety/getRefreshCitizen', // json datasource
        type: "POST", 
        data: {
          // "id": id, 
          "_token": $("#_csrf_token").val(),
...
function getUserDetails(){
    $.ajax({
        url :DIR+'cashier/HealthandSafety/getUserDetails', // json datasource
        type: "POST", 
        dataType: "JSON", 
        data: {
            "payee_type": "2", 
...
function getUserDetailstopid(){
     $.ajax({
        url :DIR+'cashier/HealthandSafety/getUserbytoid', // json datasource
        type: "POST", 
        dataType: "JSON", 
        data: {
        ...
        success: function(data){
            if(!data.ESTATUS){
                ...
                $("#ownar_address").html(data.address);
                $("#cashier_particulars").val("Laboratory Request")
                   var hoid = data.hoid;  var filtervars = {
                    hoid:hoid
                  }; 
                $.ajax({
                    type: "post",
                    url: DIR+'cashier/HealthandSafety/getallFees',
                    data: filtervars,
                    dataType: "html",
                    success: function(html){ 
                      $('.loadingGIF').hide();
...
function getAmountDetails(thisVar){
    ...
    if(isDuplicate){
        ...
        $.ajax({
            url :DIR+'cashier/HealthandSafety/getAmountDetails', // json datasource
            type: "POST", 
            dataType: "JSON", 
            data: {
                "tfoc_id":thisVar.val(),
...
async function checkOrUsedOrNot(e){
     await $.ajax({
        url :DIR+'cashier/HealthandSafety/checkOrUsedOrNot', // json datasource
        type: "POST", 
        dataType: "JSON", 
        data: {
            "or_no":$('#or_no').val(),
...
function getOrNumber(checked){
    $.ajax({
        url :DIR+'cashier/HealthandSafety/getOrnumber', // json datasource
        type: "POST", 
        data: {
...
function cancelNaturePaymentOption(thisval){
    $.ajax({
        url :DIR+'cashier/HealthandSafety/cancelNaturePaymentOption', // json datasource
        type: "POST", 
        data: {
