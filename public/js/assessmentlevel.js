$(document).ready(function(){	
	...
	$("#propertykind").select3({dropdownAutoWidth : false,dropdownParent: $("#this_is_filter").css('display', 'block')});
	datatablefunction();
...
function datatablefunction()
{
	...
	var table = $('#Jq_datatablelist').DataTable({ 
		...
		"pageLength": 10,
		"ajax":{ 
			url :DIR+'assessmentlevel/getList', // json datasource
			type: "GET", 
			"data": {
				"q":$("#q").val(),
				"revisionyear":$("#revisionyear").val(),
				"kind":$("#propertykind").val(),
...
function ActiveInactiveUpdate(id,is_activeinactive){
   ...
   swalWithBootstrapButtons.fire({
	   ...
	   {
		  $.ajax({
			   url :DIR+'assessmentlevel/ActiveInactive', // json datasource
			   type: "POST", 
			   data: {
				 "id": id,
...
function ApproveUnapproveUpdate(id,is_approve){
   ...
   swalWithBootstrapButtons.fire({
	   ...
	   {
		  $.ajax({
			   url :DIR+'assessmentlevel/ApproveUnapprove', // json datasource
			   type: "POST", 
			   data: {
				 "id": id,
