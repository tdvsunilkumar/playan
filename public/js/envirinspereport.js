...
function printapp(id) {
    var appliid = id;
    $.ajax({
        url: DIR + 'environmental-inspection-report/printreport',
        type: 'get',
        data: {
...
function datatablefunction() {
    ...
    var table = $('#Jq_datatablelist').DataTable({
        ...
        "pageLength": 10,
        "ajax": {
            url: DIR + 'environmental-inspection-report/getList', // json datasource
            type: "GET",
            "data": {
                "q": $("#q").val(),
				"year": $("#year").val(),
        ...
        "columns": [
			...
			{ "data": "taxper" },
            { "data": "ebir_date" },
            { "data": "ebir_inspection_date" },
			{ "data": "ebir_control_no" },
			{ "data": "ebir_inspected_by"},
			{ "data": "ebir_approved_by"},
			{ "data": "ebir_status"},
            { "data": "action" },
        ],
...
function inspectionPrints(id){
  var id = id;
  $.ajax({
	url: DIR+'environmental-inspection-report/printreportsss',
	type: 'POST',
	data: {
   ...
   swalWithBootstrapButtons.fire({
	   ...
	   reverseButtons: true
	   	...
	   	if(result.isConfirmed){
		  	$.ajax({
			   url :DIR+'environmental-inspection-report/ActiveInactive', // json datasource
			   type: "POST", 
			   data: {
				 "id": id,
