$(document).ready(function () {
  $('body').on('keypress', '.numeric-double', function (event) {
         ...
         swalWithBootstrapButtons.fire({
           ...
           reverseButtons: true
            ...
            if(result.isConfirmed){
              showLoader();
              $.ajax({
               url :DIR+'hr-official-work/deleteAttachment', // json datasource
               type: "POST", 
               data: {
               "rid": rid,
    ...
    $('.approve-btn').click(function(){
      ...
      const swalWithBootstrapButtons = Swal.mixin({
       ...
       if(result.isConfirmed){
              $.ajax({
                type: "POST",
                url: DIR+'hr-official-work/approve',
                data: filtervars,
                dataType: "html",
                success: function(html){ 
                   hideLoader();
    ...
    $('#disapprove-btn').click(function(){
      sequence = $(this).attr('sequence');
      stat = sequence === 0 ? 'Disapproved' : 'Cancelled';
       var filtervars = {
          id:$(this).val(),
          sequence:sequence,
          "_token": $("#_csrf_token").val()
      ...
      const swalWithBootstrapButtons = Swal.mixin({
       ...
       if(result.isConfirmed){
              $.ajax({
                type: "POST",
                url: DIR+'hr-official-work/disapprove',
                data: filtervars,
                dataType: "html",
                success: function(html){ 
                   hideLoader();
