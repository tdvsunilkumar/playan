      ...
      function deleteDocuments(id,thisval){
           ...
           swalWithBootstrapButtons.fire({
             ...
             reverseButtons: true
              ...
              if(result.isConfirmed){
                showLoader();
                $.ajax({
                 url :DIR+'hr-missed-logapproval/deleteAttachment', // json datasource
                 type: "POST", 
                 data: {
                 "rid": rid,
        ...
        const swalWithBootstrapButtons = Swal.mixin({
         ...
         if(result.isConfirmed){
                $.ajax({
                  type: "POST",
                  url: DIR+'hr-missed-logapproval/approve',
                  data: filtervars,
                  dataType: "html",
                  success: function(html){ 
                     hideLoader();
      ...
      $('#disapprove-btn').click(function(){
        ...
        const swalWithBootstrapButtons = Swal.mixin({
         ...
         if(result.isConfirmed){
                $.ajax({
                  type: "POST",
                  url: DIR+'hr-missed-logapproval/disapprove',
                  data: filtervars,
                  dataType: "html",
                  success: function(html){ 
                     hideLoader();
                     ...
                     Swal.fire({
                      position: 'center',
                      icon: 'success',
                      title: 'The request disapproved successfully',
                      showConfirmButton: false,
                      timer: 1500
                  }).then(() => {
                      // Reload the current URL after success
