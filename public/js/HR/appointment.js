$(document).ready(function(){	
	...
	$('#filter_department').select3({
		dropdownAutoWidth : false,
		dropdownParent: $("#this_is_filter"),
	})
});
...
function datatablefunction()
{
	...
	var table = $('#Jq_datatablelist').DataTable({ 
		...
		"pageLength": 10,
		"ajax":{ 
			url :DIR+'hr-appointment/getList', // json datasource
			type: "GET", 
			"data": {
				"q":$("#q").val(),
...
function ActiveInactiveUpdate(id,is_activeinactive){
   ...
   swalWithBootstrapButtons.fire({
	   ...
	   {
		  $.ajax({
			   url :DIR+'hr-appointment/ActiveInactive', // json datasource
			   type: "POST", 
			   data: {
				 "id": id,
