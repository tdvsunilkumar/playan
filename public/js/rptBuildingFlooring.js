...
function datatablefunction()
{
	...
	var table = $('#Jq_datatablelist').DataTable({ 
		...
		"pageLength": 10,
		"ajax":{ 
			url :DIR+'rptbuildingflooring/getList', // json datasource
			type: "GET", 
			"data": {
				"q":$("#q").val(),
       ...
       "columns": [
       	    { "data": "srno" },
        	{ "data": "rbf_building_flooring_desc" },
        	{ "data": "is_active" },
            { "data": "action" }
        ],
...
function DeleteRecord(id){
	// alert(id);
        ...
        swalWithBootstrapButtons.fire({
            ...
            {
               $.ajax({
			        url :DIR+'rptbuildingflooring/delete', // json datasource
			        type: "POST", 
			        data: {
			          "id": id, 
			          "_token": $("#_csrf_token").val(),
...
function ActiveInactiveUpdate(id,is_activeinactive){
   ...
   swalWithBootstrapButtons.fire({
	   ...
	   {
		  $.ajax({
			   url :DIR+'rptbuildingflooring/ActiveInactive', // json datasource
			   type: "POST", 
			   data: {
				 "id": id,
