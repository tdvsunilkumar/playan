...
function datatablefunction()
{
	...
	var table = $('#Jq_datatablelist').DataTable({ 
		...
		"pageLength": 10,
		"ajax":{ 
			url :DIR+'EcoDataCemetery/getList', // json datasource
			type: "GET", 
			"data": {
				"q":$("#q").val(),
...
function DeleteRecord(id){
	// alert(id);
        ...
        swalWithBootstrapButtons.fire({
            ...
            {
               $.ajax({
			        url :DIR+'EcoDataCemetery/delete', // json datasource
			        type: "POST", 
			        data: {
			          "id": id, 
			          "_token": $("#_csrf_token").val(),
...
function isDefaultUpdate(id,is_print){
    ...
    swalWithBootstrapButtons.fire({
        ...
        {
           $.ajax({
		        url :DIR+'EcoDataCemetery/defaultUpdateCode', // json datasource
		        type: "POST", 
		        data: {
		          "id": id,
		          "is_print": is_print,  
...
function ActiveInactiveUpdate(id,is_activeinactive){
   ...
   swalWithBootstrapButtons.fire({
	   ...
	   {
		  $.ajax({
			   url :DIR+'EcoDataCemetery/ActiveInactive', // json datasource
			   type: "POST", 
			   data: {
				 "id": id,
