...
function datatablefunction()
{
	...
	var table = $('#Jq_datatablelist').DataTable({ 
		...
		"pageLength": 10,
		"ajax":{ 
			url :DIR+'rptPropertysubclassification/getList', // json datasource
			type: "GET", 
			"data": {
				"q":$("#q").val(),
       ...
       "columns": [
        	...
        	{ "data": "ps_subclass_desc" },
        	{ "data": "ps_is_for_plant_trees" },
        	{ "data": "is_active" },
            { "data": "action" }
        ],
...
function DeleteRecord(id){
	// alert(id);
        ...
        swalWithBootstrapButtons.fire({
            ...
            {
               $.ajax({
			        url :DIR+'rptPropertysubclassification/delete', // json datasource
			        type: "POST", 
			        data: {
			          "id": id, 
			          "_token": $("#_csrf_token").val(),
...
function ActiveInactiveUpdate(id,is_activeinactive){
   ...
   swalWithBootstrapButtons.fire({
	   ...
	   {
		  $.ajax({
			   url :DIR+'rptPropertysubclassification/ActiveInactive', // json datasource
			   type: "POST", 
			   data: {
				 "id": id,
