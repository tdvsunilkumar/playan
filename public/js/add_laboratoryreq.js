$(document).ready(function(){
         ...
         if ($(this).hasClass('new-btn')) {
            ...
            swalWithBootstrapButtons.fire({
                ...
                if(result.isConfirmed){
                    $.ajax({
                        type: "GET",
                        url: DIR+'laboratory-request/del-fees/'+inputVal,
                        dataType: "json",
    ...
    $("#hp_code").change(function(){
		var hp_code=$(this).val();
		getReqPhysDetails(hp_code);
	});
    
 });  
 
 
 function getReqPhysDetails(id){
	$('.loadingGIF').show();
	var filtervars = {
	    id:id
	};
	$.ajax({
	    type: "GET",
	    url: DIR+'laboratory-request/getReqPhysDetails',
	    data: filtervars,
	    dataType: "json",
	    success: function(html){ 
	    	$('.loadingGIF').hide();
	    	$("#req_phys").val(html.req_phys);
	    }
	}); 
}
 function addmorehealthcert(){
     var prevLength = $("#Healthcerti").find(".removehealthcertidata").length;
     var row = $("#Healthcerti").find(".row").length + 1;
     ...
     $('#Healthcerti').append('<div class="removehealthcertidata row pt10">'+
            ...
            '</div>'+
            '<div class="col-sm-1">'+
                '<button class="btn btn-danger btn_cancel_healthcert new-btn" style="padding: 0.4rem 1rem !important;" type="button"><i class="ti-trash text-white"></i></button>'+                               
            '</div>'+
        '</div>');
     select3Ajax('select-service-'+prevLength,'contain-select-service-'+prevLength,'getServices');
 ...
 function computeTotal() {
     var total = 0;
     $('.fee[type="text"]').not('#lab_req_amount').each(function(i) {
         var val = $(this).val();
         if (val) {
            total += removeComma(val);
         }
     });
     $('#lab_req_amount').val(numFormat(total));
     $('#lab_req_amount').prop('readOnly', true);
 }
