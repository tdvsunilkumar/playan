$(document).ready(function(){	
	$("#pk_id").select3({dropdownAutoWidth : false,dropdownParent: $("#this_is_filter")});
	datatablefunction();
...
function datatablefunction()
{
	...
	var table = $('#Jq_datatablelist').DataTable({ 
		...
		"pageLength": 10,
		"ajax":{ 
			url :DIR+'realpropertyarsetup/getList', // json datasource
			type: "GET", 
			"data": {
...
function ActiveInactiveUpdate(id,is_activeinactive){
   ...
   swalWithBootstrapButtons.fire({
	   ...
	   {
		  $.ajax({
			   url :DIR+'rptbuildingkind/ActiveInactive', // json datasource
			   type: "POST", 
			   data: {
				 "id": id,
