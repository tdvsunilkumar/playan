...
function datatablefunction()
{
	...
	var table = $('#Jq_datatablelist').DataTable({ 
		...
		"pageLength": 10,
		"ajax":{ 
			url :DIR+'icdcode/getList', // json datasource
			type: "GET", 
			"data": {
				"q":$("#q").val(),
       ...
       "columns": [
            ...
            { "data": "ICD10 Group" },
            { "data": "Case Rate" },
            { "data": "Professional Rate" },
            { "data": "Health Care Institurion Fee" },
        	{ "data": "icd_is_active" },
            { "data": "action" }
        ],
...
function DeleteRecord(id){
	// alert(id);
        ...
        swalWithBootstrapButtons.fire({
            ...
            {
               $.ajax({
			        url :DIR+'icdcode/delete', // json datasource
			        type: "POST", 
			        data: {
			          "id": id, 
			          "_token": $("#_csrf_token").val(),
			        ...
			        success: function(html){
			        	Swal.fire({
	    				  position: 'center',
	    				  icon: 'success',
	    				  title: 'Icd10 Code Deleted Successfully.',
...
function ActiveInactiveUpdate(id,is_activeinactive){
   ...
   swalWithBootstrapButtons.fire({
	   ...
	   {
		  $.ajax({
			   url :DIR+'icdcode/icdCodeActiveInactive', // json datasource
			   type: "POST", 
			   data: {
				 "id": id,
