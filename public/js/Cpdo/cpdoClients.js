...
function datatablefunction()
{
	...
	var table = $('#Jq_datatablelist').DataTable({ 
		...
		"searching": false,
		"order": [],
		"columnDefs": [{ orderable: false, targets: [0,5] },{ orderable: false, targets: [0,5] }],
		"pageLength": 10,
		"ajax":{ 
			url :DIR+'cpdoclients/getList', // json datasource
			type: "GET", 
			"data": {
				"q":$("#q").val(),
...
function DeleteRecord(id){
	alert(id);
        ...
        swalWithBootstrapButtons.fire({
            ...
            {
               $.ajax({
			        url :DIR+'cpdoclients/delete', // json datasource
			        type: "POST", 
			        data: {
			          "id": id, 
			          "_token": $("#_csrf_token").val(),
...
function ActiveInactiveUpdate(id,is_activeinactive){
   ...
   swalWithBootstrapButtons.fire({
	   ...
	   {
		  $.ajax({
			   url :DIR+'cpdoclients/ActiveInactive', // json datasource
			   type: "POST", 
			   data: {
				 "id": id,
