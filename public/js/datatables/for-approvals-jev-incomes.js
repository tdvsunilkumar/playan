!function($) {
    ...
    var for_approval_jev_income = function() {
        this.$body = $("body");
    };

    var _documentID = 0; var _table; var _prTable; var _projectName = ''; var _supplierStatus = ''; var _supplierID = 0; var _page = 0;
    var _voucherSegment = 'incomes'; var _status = 'all';

    for_approval_jev_income.prototype.validate = function($form, $required)
    {   
        $required = 0;
    ...
    for_approval_jev_income.prototype.required_fields = function() {
    ...
    for_approval_jev_income.prototype.load_contents = function(_page = 0) 
    {   
        _table = new DataTable('#jevIncomeTable', {
            ajax: { 
                url : _baseUrl + 'for-approvals/journal-entries/incomes/lists?status=' + _status,
                type: "GET", 
                data: {
                    "_token": _token
                },
                complete: function() {
                    $.for_approval_jev_income.shorten();
                    $('[data-bs-toggle="tooltip"]').tooltip({
                        trigger : 'hover'
                    });        
                    $.for_approval_jev_income.hideTooltip();
                }
            },
            language: {
    ...
    for_approval_jev_income.prototype.reload_available_control_no = function()
    {   
    ...
    for_approval_jev_income.prototype.fetchID = function()
    {
        return _documentID;
    }

    for_approval_jev_income.prototype.updateID = function(_id)
    {
        return _documentID = _id;
    }

    for_approval_jev_income.prototype.fetch_status = function (_id)
    {   
        var _status = '';
        if (_id == 0) {
            return _status = 'draft';
        }
        console.log(_baseUrl + 'for-approvals/journal-entries/incomes/fetch-status/' + _id);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'for-approvals/journal-entries/incomes/fetch-status/' + _id,
            success: function(response) {
    ...
    for_approval_jev_income.prototype.validate_approver = function (_id)
    {   
        var _status = false;
        console.log(_baseUrl + 'for-approvals/journal-entries/incomes/validate-approver/' + _id);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'for-approvals/journal-entries/incomes/validate-approver/' + _id,
            success: function(response) {
                console.log(response);
                _status = response;
            },
    ...
    for_approval_jev_income.prototype.shorten = function() 
    {   
        if ($('.showLess')) {
            $('.showLess').shorten({
                "showChars" : 20,
    ...
    for_approval_jev_income.prototype.preload_select3 = function()
    {
        if ( $('.select3') ) {
    ...
    for_approval_jev_income.prototype.perfect_scrollbar = function()
    {
        if ($(".table-responsive")) {
    ...
    for_approval_jev_income.prototype.validate_table = function($table)
    {   
        $('.pager').remove();
        $table.each(function() {
            var currentPage = 0;
    ...
    | ---------------------------------
    */
    for_approval_jev_income.prototype.price_separator = function(input) {
        var output = input
        if (parseFloat(input)) {
    ...
    for_approval_jev_income.prototype.fetch_remarks = function (_id)
    {   
        var _remarks = '';
        console.log(_baseUrl + 'for-approvals/journal-entries/incomes/fetch-remarks/' + _id);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'for-approvals/journal-entries/incomes/fetch-remarks/' + _id,
            success: function(response) {
                console.log(response);
    ...
    for_approval_jev_income.prototype.hideTooltip = function()
    {
        $('.tooltip').remove();
    },

    for_approval_jev_income.prototype.init = function()
    {   
        /*
        | ---------------------------------
        ...
        | ---------------------------------
        */
        $.for_approval_jev_income.preload_select3();
        $.for_approval_jev_income.load_contents();
        $.for_approval_jev_income.perfect_scrollbar();
        ...
        */
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#jevIncomeTable .view-btn', function (e) {
            var _self   = $(this);
            var _id     = $(this).closest('tr').attr('data-row-id');
            var _code   = $(this).closest('tr').attr('data-row-code');
            var _status = $(this).closest('tr').attr('data-row-status');
            var _modal  = $('#rfq-modal');
            var _url    = _baseUrl + 'for-approvals/journal-entries/incomes/edit/' + _id;
            console.log(_url);
            _documentID = _id;
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.data.total_budget) {
                        _modal.find('label[for="total_budget"].text-danger').text('₱' + $.for_approval_jev_income.price_separator(parseFloat(response.data.total_budget).toFixed(2)));
                    }
                    ...
                    $.for_approval_jev_income.load_pr_contents();
                    $.for_approval_jev_income.load_supplier_contents();
                    $.for_approval_jev_income.load_item_contents();
                    if(_status == 'draft') {
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#jevIncomeTable .approve-btn', function (e) {
            var _self   = $(this);
            var _id     = $(this).closest('tr').attr('data-row-id');
            var _code   = $(this).closest('tr').attr('data-row-code');
            var _modal  = $('#purchase-order-modal');
            var _controlNo = _modal.find('#rfq_id'); _controlNo.find('option').remove(); 
            var _url    = _baseUrl + 'for-approvals/journal-entries/incomes/approve/' + _id;

            var d1 = $.for_approval_jev_income.fetch_status(_id);
            var d2 = $.for_approval_jev_income.validate_approver(_id);
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#jevIncomeTable .disapprove-btn', function (e) {
            var _self   = $(this);
            var _id     = $(this).closest('tr').attr('data-row-id');
            var _code   = $(this).closest('tr').attr('data-row-code');
        ...
        */
        this.$body.on('click', '.submit-disapprove-btn', function (e){
            ...
            var _error  = $.for_approval_jev_income.validate(_form, 0);
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#jevIncomeTable .view-disapprove-btn', function (e) {
            var _self   = $(this);
            var _id     = $(this).closest('tr').attr('data-row-id');
            var _code   = $(this).closest('tr').attr('data-row-code');
            var _modal  = $('#disapprove-modal');
            ...
            _self.prop('disabled', true);
            var d1 = $.for_approval_jev_income.fetch_status(_id);
            var d2 = $.for_approval_jev_income.fetch_remarks(_id);
            $.when( d1, d2 ).done(function ( v1, v2 ) 
            {   
                if (v1 == 'disapproved') {
                    _self.prop('disabled', false);
    ...
    //init for_approval_jev_income
    $.for_approval_jev_income = new for_approval_jev_income, $.for_approval_jev_income.Constructor = for_approval_jev_income

}(window.jQuery),

//initializing for_approval_jev_income
function($) {
    "use strict";
    $.for_approval_jev_income.required_fields();
    $.for_approval_jev_income.init();
}(window.jQuery);