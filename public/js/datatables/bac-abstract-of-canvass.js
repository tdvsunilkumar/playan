!function($) {
    ...
    bac_abstract.prototype.load_contents = function(_keywords = '') 
    {   
        console.log(_baseUrl + 'general-services/bac/abstract-of-canvass/lists'); 
        _table = new DataTable('#rfqTable', {
            ajax: { 
                url : _baseUrl + 'general-services/bac/abstract-of-canvass/lists',
                type: "GET", 
                data: {
                    "query": _keywords,
                    "_token": _token
    ...
    bac_abstract.prototype.load_pr_contents = function(_keywords = '') 
    {   
    ...
    bac_abstract.prototype.load_supplier_contents = function(_keywords = '') 
    {   
    ...
    bac_abstract.prototype.load_item_contents = function(_keywords = '') 
    {   
    ...
    bac_abstract.prototype.load_committee_contents = function(_keywords = '') 
    {   
        console.log(_baseUrl + 'general-services/bac/abstract-of-canvass/committee-lists/' + _rfqID); 
        _prTable = new DataTable('#committeeTable', {
            ajax: { 
                url : _baseUrl + 'general-services/bac/abstract-of-canvass/committee-lists/' + _rfqID,
                type: "GET", 
                data: {
                    "query": _keywords,
                    "_token": _token
    ...
    bac_abstract.prototype.fetchID = function()
    {
        return _rfqID;
    }

    bac_abstract.prototype.updateID = function(_id)
    {
        return _rfqID = _id;
    }

    bac_abstract.prototype.fetchProjectName = function()
    {
        return _projectName;
    }

    bac_abstract.prototype.updateProjectName = function(_proj)
    {
        return _projectName = _proj;
    }

    bac_abstract.prototype.fetchSupplierStatus = function()
    {
        return _supplierStatus;
    }

    bac_abstract.prototype.fetchSupplierID = function()
    {
        return _supplierID;
    }

    bac_abstract.prototype.shorten = function() 
    {   
        if ($('.showLess')) {
            $('.showLess').shorten({
                "showChars" : 20,
    ...
    bac_abstract.prototype.preload_select3 = function()
    {
        if ( $('.select3') ) {
    ...
    bac_abstract.prototype.perfect_scrollbar = function()
    {
        if ($(".table-responsive")) {
    ...
    bac_abstract.prototype.validate_table = function($table)
    {   
        $('.pager').remove();
        $table.each(function() {
            var currentPage = 0;
    ...
    | ---------------------------------
    */
    bac_abstract.prototype.price_separator = function(input) {
        var output = input
        if (parseFloat(input)) {
    ...
    bac_abstract.prototype.edit_supplier = function(_supplier, _url, _modal, _code, _button, _status)
    {   
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.data.length > 0) {
                    ...
                    _table.find('tfoot th.danger').text($.bac_abstract.price_separator(parseFloat(_totalCost).toFixed(2)));
                    if (_status !== 'pending') {
    ...
    bac_abstract.prototype.hideTooltip = function()
    {
        $('.tooltip').remove();
    },

    bac_abstract.prototype.init = function()
    {   
        /*
        | ---------------------------------
        ...
        | ---------------------------------
        */
        $.bac_abstract.preload_select3();
        $.bac_abstract.load_contents();
        $.bac_abstract.perfect_scrollbar();
        ...
        */
        this.$body.on('hidden.bs.modal', '#rfq-modal', function (e) {
            ...
            _modal.find('input, select, textarea.required').prop('disabled', false);
        ...
        */
        this.$body.on('hidden.bs.modal', '#add-supplier-modal', function (e) {
            $.bac_abstract.load_supplier_contents();
        });
        ...
        */
        this.$body.on('hidden.bs.modal', '#add-purchase-request-modal', function (e) {
            $.bac_abstract.load_pr_contents();
            $.bac_abstract.load_item_contents();
        });
        ...
        */
        this.$body.on('click', '.add-btn', function (e) {
            ...
            var d1 = $.bac_abstract.load_pr_contents();
            var d2 = $.bac_abstract.load_supplier_contents();
            var d3 = $.bac_abstract.load_item_contents();
        ...
        */
        this.$body.on('click', '#rfqTable .edit-btn', function (e) {
            ...
            var _modal  = $('#rfq-modal');
            var _url    = _baseUrl + 'general-services/bac/abstract-of-canvass/edit/' + _id;
            console.log(_url);
            _rfqID = _id;
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.data.total_budget) {
                        _modal.find('label[for="total_budget"].text-danger').text('₱' + $.bac_abstract.price_separator(parseFloat(response.data.total_budget).toFixed(2)));
                    }
                    ...
                    $.each(response.suppliers, function (k, v) {
                        ...
                        console.log(_found);
                        if (v.is_selected > 0) {
                            _supplierData += '<input value="' + v.supplier_id + '" class="form-check-input me-2" type="radio" name="is_selected" checked="checked" ' + ((_found != undefined) ? 'disabled="disabled"' : '') + '/>' + v.supplier + ' -&nbsp; <strong class="text-danger">' + '₱' + $.bac_abstract.price_separator(parseFloat(v.canvass).toFixed(2)) + '</strong>';
                        } else {
                            _supplierData += '<input value="' + v.supplier_id + '" class="form-check-input me-2" type="radio" name="is_selected" ' + ((_found != undefined) ? 'disabled="disabled"' : '') + '/>' + v.supplier + ' -&nbsp; <strong class="text-danger">' + '₱' + $.bac_abstract.price_separator(parseFloat(v.canvass).toFixed(2)) + '</strong>';
                        }
                        ...
                        var _total = 0; var _increment = 0;
                            $.each(v.items, function (k1, v2){
                                ...
                                _supplierData += '<td class="text-center">' + (v2.unit_cost ? $.bac_abstract.price_separator(parseFloat(v2.unit_cost).toFixed(2)) : $.bac_abstract.price_separator(parseFloat(0).toFixed(2))) + '</td>';
                                _supplierData += '<td class="text-end text-danger">' + (v2.total_cost ? $.bac_abstract.price_separator(parseFloat(v2.total_cost).toFixed(2)) : $.bac_abstract.price_separator(parseFloat(0).toFixed(2))) + '</td>';
                                _supplierData += '</tr>';
                        ...
                        _supplierData += '<td class="text-end text-danger" colspan="1"><strong>' + '₱' + $.bac_abstract.price_separator(parseFloat(_total).toFixed(2)) + '</strong></td>';
                        _supplierData += '</tr>';
                        _supplierData += '</tfoot>';
                        _supplierData += '</tr>';
                        _supplierData += '</table>';
                    ...
                    _modal.find('.modal-header h5').html('Edit Abstract Of Canvass (<span class="variables">' + _code + '</span>)');
        ...
        */
        this.$body.on('click', '#supplierTable .edit-btn', function (e) {
            ...
            _self.prop('disabled', true).html('<div class="spinner-border spinner-border-sm text-white" role="status" aria-hidden="true"><span class="visually-hidden">Loading...</span></div>');
            var d1 = $.bac_abstractForm.fetch_status(_rfqID);
            ...
            // {  
                // if (v1 == 'draft') {
                    setTimeout(function () {
                        $.bac_abstract.edit_supplier(_supplier, _url, _modal, _code, _self, _status);
        ...
        */
        this.$body.on('click', '#supplierTable .print-btn', function (e) {
            ...
            var _supplier = _self.closest('tr').attr('data-row-supplier');
            var _url      = _baseUrl + 'general-services/bac/abstract-of-canvass/print/' + _rfqNo + '?supplier=' + _supplier;
            window.open(_url, '_blank');
        });
        ...
        */
        this.$body.on('click', '#canvass-modal .print-btn', function (e) {
            ...
            var _modal    = $('#rfq-modal');
            var _rfqNo    = _modal.find('label[for="control_no"].text-danger').text();
            var _url      = _baseUrl + 'general-services/bac/abstract-of-canvass/print/' + _rfqNo + '?supplier=' + _supplierID;
            window.open(_url, '_blank');
        });
        ...
        */
        this.$body.on('click', '#committeeTable .delete-btn', function (e) {
            ...
            var _code   = $(this).closest('tr').attr('data-row-code');
            var _urlz   = _baseUrl + 'general-services/bac/abstract-of-canvass/remove-committee/' +_rfqID + '/' + _id;
            var d1      = $.bac_abstractForm.fetch_status(_rfqID);
    ...
    //init bac_abstract
    $.bac_abstract = new bac_abstract, $.bac_abstract.Constructor = bac_abstract

}(window.jQuery),

//initializing bac_abstract
function($) {
    "use strict";
    $.bac_abstract.required_fields();
    $.bac_abstract.init();
}(window.jQuery);