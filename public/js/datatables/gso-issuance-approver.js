!function($) {
    ...
    {   
        ...
        var table = new DataTable('#issuanceRequestTable', {
            ajax: { 
                url : _baseUrl + 'general-services/issuance/approver/lists',
                type: "GET", 
                data: {
                    "query": _keywords,
                    "_token": _token
            ...
            createdRow: function( row, data, dataIndex ) {
                $(row).attr('data-row-id', data.gi_id);
            },
            columns: [
                { data: 'control_no' },
                { data: 'issuance_date' },
    ...
    menu_group.prototype.$load_contents_item = function(_id,_keywords = '') 
    {   
        var _complete = 0;
        var table = new DataTable('#itemTable', {
            ajax: { 
                url : _baseUrl + 'general-services/issuance/approver/item_lists/' + _id,
                type: "GET", 
                data: {
                    "query": _keywords,
                    "_token": _token
    ...
    {   
        ...
        */
        this.$body.on('click', '#issuanceRequestTable .edit-btn', function (e) {
            ...
            var _modal  = $('#group-menu-modal');
            $.menu_group.$load_contents_item(_id);
            var _url    = _baseUrl + 'general-services/issuance/approver/edit/' + _id;
            console.log(_url);
            _groupMenuID = _id;
            $.ajax({
                type: 'GET',
