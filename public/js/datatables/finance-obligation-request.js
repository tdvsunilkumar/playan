!function($) {
    ...
    var sortBy = '', orderBy = '', _requisitionID = 0, _lineID = 0;  var _allotTable = '';
    var _lastSegment = '';
    ...
    {   
        var table = new DataTable('#obligationRequestTable', {
            ajax: { 
                url : _baseUrl + 'finance/obligation-requests/' + _lastSegment + '/lists?type=' + _lastSegment,
                type: "GET", 
                data: {
                    "query": _keywords,
                    "_token": _token
                ...
                complete: function() {
                    ...
                    $.obligation_request.hideTooltip();
                }
            },
            language: {
            ...
            createdRow: function( row, data, dataIndex ) {
                ...
                $(row).attr('data-row-alob', data.alob_no);
    ...
    {   
        ...
        var table = new DataTable('#itemRequisitionTable', {
            ajax: { 
                url : _baseUrl + 'finance/obligation-requests/' + _lastSegment + '/item-lists/' + _requisitionID,
                type: "GET", 
                data: {
                    "query": _keywords,
                    "_token": _token
    ...
    {   
        ...
        var table = new DataTable('#allotmentBreakdownTable', {
            ajax: { 
                url : _baseUrl + 'finance/obligation-requests/' + _lastSegment + '/alob-lists/' + _requisitionID,
                type: "GET", 
                data: {
                    "query": _keywords,
                    "_token": _token
    ...
    {   
        ...
        _allotTable = new DataTable('#allotmentBreakdownTable2', {
            ajax: { 
                url : _baseUrl + 'finance/obligation-requests/' + _lastSegment + '/alob-lists2/' + _id,
                type: "GET", 
                data: {
                    "query": _keywords,
                    "_token": _token
            ...
            initComplete: function(){
                if(_allotTable.rows().data().length > 0) {
                    $('form[name="alobForm2"] select[name="allob_division_id2"], form[name="alobForm2"] select[name="allob_department_id2"], form[name="alobForm2"] select[name="fund_code_id2"]').prop('disabled', true);
                } else {
                    $('form[name="alobForm2"] select[name="allob_division_id2"], form[name="alobForm2"] select[name="allob_department_id2"], form[name="alobForm2"] select[name="fund_code_id2"]').prop('disabled', false);
                }          
            }, 
            columnDefs: [
                {  orderable: true, targets: 0, className: 'text-start' },
    ...
    // FOR PAYROLL EMPLOYEE TABLE
    obligation_request.prototype.load_emp_list = function(_keywords = '') 
    {   
        var _id = $.obligation_requestForm.fetch_alobID();
        var table = new DataTable('#empListTable', {
            ajax: { 
                url : _baseUrl + 'finance/obligation-requests/' + _lastSegment + '/employee-list/' + _id,
                type: "GET", 
                data: {
                    "query": _keywords,
                    "_token": _token
    ...
    {   
        var _total = 0;
        var _url   = _baseUrl + 'finance/obligation-requests/' + _lastSegment + '/fetch-allotment-via-pr/' + _requisitionID + '?column=total_amount'; 
        console.log(_url);
        $.ajax({
            type: 'GET',
            url: _url,
    ...
    obligation_request.prototype.getLastSegment = function()
    {
    ...
    */
    ...
    {   
        ...
        var _total = 0;
        var _url   = _baseUrl + 'finance/obligation-requests/' + _lastSegment + '/fetch-allotment-via-pr2/' + _allotmentID + '?column=total_amount'; 
        console.log(_url);
        $.ajax({
            type: 'GET',
            url: _url,
    ...
    obligation_request.prototype.validate_gl_funds = function()
    {   
        var _funds = $('#fund_code_id2'); _funds.find('option').remove(); 
        var _url   = _baseUrl + 'finance/obligation-requests/' + _lastSegment + '/validate-gl-funds?type=' + _lastSegment; 
        console.log(_url);
        $.ajax({
            type: 'GET',
            url: _url,
            success: function(response) {
                ...
                if (response.funds.length > 0) {
                    _funds.append('<option value="">select a fund code</option>');  
                    $.each(response.funds, function(i, item) {
                        _funds.append('<option value="' + item.id + '">' + item.code + ' - ' + item.description + '</option>');  
                    }); 
                }
            },
            async: false
    ...
    obligation_request.prototype.validate_table = function($table)
    {   
        $('.pager').remove();
        $table.each(function() {
            var currentPage = 0;
    ...
    obligation_request.prototype.hideTooltip = function()
    {
        $('.tooltip').remove();
    },
    ...
    {   
        ...
        | ---------------------------------
        */
        _lastSegment = $.obligation_request.getLastSegment();
        $.obligation_request.validate_gl_funds();
        ...
        */
        this.$body.on('click', '.add-btn', function (e) {
            ...
            $.obligation_request.load_emp_list();
            ...
            _modal.find('input[type="checkbox"][name="with_pr"], input[type="checkbox"][name="with_pr2"]').prop('checked', false);
            _modal.modal('show');
        });
        ...
        */
        this.$body.on('hidden.bs.modal', '#departmental-requisition-modal', function (e) {
            ...
            _modal.find('input:not([type="checkbox"]), textarea').val('').removeClass('is-invalid');
            ...
            _modal.find('form[name="alobForm2"] input[name="with_pr2"]').prop('disabled', false);
            _modal.find('input[type="checkbox"][name="with_pr"], input[type="checkbox"][name="with_pr2"]').prop('checked', false);
        ...
        */
        this.$body.on('click', '#obligationRequestTable .view-btn', function (e) {
            ...
            if (_dep > 0) {
                var _url    = _baseUrl + 'finance/obligation-requests/' + _lastSegment + '/edit/' + _dep;
                _requisitionID = _dep;
                ...
                _modal.find('.alob-v1').removeClass('hidden');
                $.obligation_requestForm.update_alobID(_id);
                var _url    = _baseUrl + 'finance/obligation-requests/' + _lastSegment + '/edits/' + _id;
                _self.prop('disabled', true).html('<div class="spinner-border spinner-border-sm text-white" role="status" aria-hidden="true"><span class="visually-hidden">Loading...</span></div>');
                $.ajax({
                    type: 'GET',
                    url: _url,
                    success: function(response) {
                        ...
                        var d3 = $.obligation_request.load_emp_list();
                        var d4 = $.obligation_request.fetch_total_allotment_amount2();
                        var d5 = $.obligation_requestForm.reload_divisions_employees(_alobForm2.find('select[name="division_id2"]'), _alobForm2.find('select[name="employee_id2"]'), _alobForm2.find('select[name="designation_id2"]'), response.alob[0].allob_department_id2);
                        var d6 = $.obligation_requestForm.reload_designation(_alobForm2.find('select[name="designation_id2"]'), response.alob[0].employee_id2);
                        $.when( d1, d2, d3, d4, d5, d6 ).done(function ( v1, v2, v3, v4, v5, v6 ) 
                        { 
                            _alobForm2.find('#budget_year2').prop('disabled', true);
                            $.each(v1[0], function (k, v) {
                                _alobForm2.find('input[type="text"][name='+k+']').val(v);
                            ...
                            if (_status != 'draft') {
                                ...
                                _alobForm2.find('input[name="with_pr2"]').prop('disabled', true);
                            }
                            _modal.find('.modal-header h5').html('View Obligation Request (<span class="variables">' + _code + '</span>)');
        ...
        */
        this.$body.on('click', '#obligationRequestTable .print-btn', function (e){
            ...
            window.open(_baseUrl +'digital-sign?url='+'finance/obligation-requests/' + _lastSegment + '/print/' + _code, '_blank');
        });
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#obligationRequestTable .print2-btn', function (e){
            e.preventDefault();
            var _self = $(this);
            var _rows = _self.closest('tr');
            var _code = _rows.attr('data-row-code');
            var _alob = _rows.attr('data-row-alob');
            if (_alob != 0) {
                window.open(_baseUrl + 'finance/obligation-requests/' + _lastSegment + '/print-disbursement/' + _code, '_blank');
            } else {
                Swal.fire({
                    title: "Oops...",
                    html: "No (ALOB No) yet!<br/>The request is under review to finance.",
                    icon: "error",
                    type: "danger",
        ...
        */
        this.$body.on('click', '#allotmentBreakdownTable2 .delete-btn', function (e) {
            ...
            var _url         = _baseUrl + 'finance/obligation-requests/' + _lastSegment + '/remove-line/' + _id;
            var _allotmentID = $.obligation_requestForm.fetch_alobID();
                                ...
                                success: function(response) {
                                    ...
                                    Swal.fire({ title: response.title, text: response.text, icon: response.type, buttonsStyling: !1, confirmButtonText: "Ok, got it!", customClass: { confirmButton: "btn btn-primary" } }).then(
                                        function (e) {
                                            ...
                                            var d1 = $.obligation_request.load_emp_list();
                                            $.when( d1 ).done(function ( v1 ) {
