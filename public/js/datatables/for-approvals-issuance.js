!function($) {
    ...
    for_approval_issuance.prototype.required_fields = function() {
    ...
    for_approval_issuance.prototype.load_contents = function(_page = 0) 
    {   
        console.log(_baseUrl + 'for-approvals/issuance/lists'); 
        _table = new DataTable('#issuanceTable', {
            ajax: { 
                url : _baseUrl + 'for-approvals/issuance/lists',
                type: "GET", 
                data: {
                    "_token": _token
                },
                complete: function() {
                    $.for_approval_issuance.shorten();
            ...
            initComplete: function(){
                ...
                $.for_approval_issuance.hideTooltip();
            }, 
            columns: [
                { data: 'id' },
    ...
    for_approval_issuance.prototype.load_item_contents = function(_keywords = '') 
    {   
        console.log(_baseUrl + 'for-approvals/issuance/item-lists/' + _issuanceID); 
        _itemTable = new DataTable('#itemTable', {
            ajax: { 
                url : _baseUrl + 'for-approvals/issuance/item-lists/' + _issuanceID,
                type: "GET", 
                data: {
                    "query": _keywords,
                    "_token": _token
    ...
    for_approval_issuance.prototype.fetchID = function()
    {
        return _issuanceID;
    }

    for_approval_issuance.prototype.updateID = function(_id)
    {
        return _issuanceID = _id;
    }

    for_approval_issuance.prototype.fetch_status = function (_id)
    {   
        var _status = '';
        if (_id == 0) {
            return _status = 'draft';
        }
        console.log(_baseUrl + 'for-approvals/issuance/fetch-status/' + _id);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'for-approvals/issuance/fetch-status/' + _id,
    ...
    for_approval_issuance.prototype.validate_approver = function (_id)
    {   
        var _status = false;
        console.log(_baseUrl + 'for-approvals/issuance/validate-approver/' + _id);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'for-approvals/issuance/validate-approver/' + _id,
            success: function(response) {
                console.log(response);
                _status = response;
            },
    ...
    for_approval_issuance.prototype.shorten = function() 
    {   
        if ($('.showLess')) {
            $('.showLess').shorten({
                "showChars" : 20,
    ...
    for_approval_issuance.prototype.preload_select3 = function()
    {
        if ( $('.select3') ) {
    ...
    for_approval_issuance.prototype.perfect_scrollbar = function()
    {
        if ($(".table-responsive")) {
    ...
    for_approval_issuance.prototype.validate_table = function($table)
    {   
        $('.pager').remove();
        $table.each(function() {
            var currentPage = 0;
    ...
    | ---------------------------------
    */
    for_approval_issuance.prototype.price_separator = function(input) {
        var output = input
        if (parseFloat(input)) {
    ...
    for_approval_issuance.prototype.fetch_remarks = function (_id)
    {   
        var _remarks = '';
        console.log(_baseUrl + 'for-approvals/issuance/fetch-remarks/' + _id);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'for-approvals/issuance/fetch-remarks/' + _id,
            success: function(response) {
                console.log(response);
    ...
    for_approval_issuance.prototype.load_line_contents = function(_keywords = '') 
    {   
    ...
    for_approval_issuance.prototype.load_alob_line_contents = function(_keywords = '') 
    {   
        var table = new DataTable('#allotmentBreakdownTable', {
            ajax: { 
                url : _baseUrl + 'for-approvals/issuance/alob-lists/' + _issuanceID,
                type: "GET", 
                data: {
                    "query": _keywords,
                    "_token": _token
    ...
    for_approval_issuance.prototype.reload_divisions_employees = function(_division, _employee, _designation, _department = 0)
    {   
        // if (_department > 0) {
            _employee.find('option').remove(); _division.find('option').remove(); _designation.val('');
            console.log(_baseUrl + 'for-approvals/issuance/reload-divisions-employees/' + _department);
            $.ajax({
                type: "GET",
                url: _baseUrl + 'for-approvals/issuance/reload-divisions-employees/' + _department,
                success: function(response) {
    ...
    for_approval_issuance.prototype.reload_designation = function(_designation, _employee = 0)
    {   
        // if (_employee > 0) {
            _designation.find('option').remove(); 
            console.log(_baseUrl + 'for-approvals/issuance/reload-designation/' + _employee);
            $.ajax({
                type: "GET",
                url: _baseUrl + 'for-approvals/issuance/reload-designation/' + _employee,
                success: function(response) {
    ...
    for_approval_issuance.prototype.reload_items = function(_item, _uom, _purchaseType = 0)
    {   
        // if (_purchaseType > 0) {
            _item.find('option').remove(); _uom.val('');
            console.log(_baseUrl + 'for-approvals/issuance/reload-items/' + _purchaseType);
            $.ajax({
                type: "GET",
                url: _baseUrl + 'for-approvals/issuance/reload-items/' + _purchaseType,
                success: function(response) {
                    console.log(response.data);
    ...
    for_approval_issuance.prototype.fetch_total_allotment_amount = function()
    {   
        var _total = 0;
        var _url   = _baseUrl + 'for-approvals/issuance/fetch-allotment-via-pr/' + _issuanceID + '?column=total_amount'; 
        console.log(_url);
        $.ajax({
            type: 'GET',
            url: _url,
    ...
    for_approval_issuance.prototype.hideTooltip = function()
    {
        $('.tooltip').remove();
    },

    for_approval_issuance.prototype.edit_supplier = function(_supplier, _url, _modal, _code, _button, _status)
    {   
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.data.length > 0) {
                    ...
                    _table.find('tfoot th.text-danger').text($.for_approval_issuance.price_separator(parseFloat(_totalCost).toFixed(2)));
                    if (_status !== 'pending') {
    ...
    for_approval_issuance.prototype.init = function()
    {   
        /*
        | ---------------------------------
        ...
        | ---------------------------------
        */
        $.for_approval_issuance.preload_select3();
        $.for_approval_issuance.load_contents();
        $.for_approval_issuance.perfect_scrollbar();
        ...
        */
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#issuanceTable .view-btn', function (e) {
            var _self   = $(this);
            ...
            var _url    = _baseUrl + 'for-approvals/issuance/edit/' + _id;
            console.log(_url);
            _issuanceID = _id;
                ...
                success: function(response) {
                    ...
                    $.each(response.data, function (k, v) {
                        ...
                        if (k == 'purchase_order_id') {
                            _modal.find('select[name="purchase_order_id[]"]').val(v).trigger('change.select3'); 
                        }
                    });
                    $.for_approval_issuance.load_item_contents();
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#issuanceTable .approve-btn', function (e) {
            var _self   = $(this);
            var _id     = $(this).closest('tr').attr('data-row-id');
            var _code   = $(this).closest('tr').attr('data-row-code');
            var _modal  = $('#purchase-order-modal');
            var _controlNo = _modal.find('#rfq_id'); _controlNo.find('option').remove(); 
            var _url    = _baseUrl + 'for-approvals/issuance/approve/' + _id;

            var d1 = $.for_approval_issuance.fetch_status(_id);
            var d2 = $.for_approval_issuance.validate_approver(_id);
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#issuanceTable .disapprove-btn', function (e) {
            var _self   = $(this);
            var _id     = $(this).closest('tr').attr('data-row-id');
            var _code   = $(this).closest('tr').attr('data-row-code');
        ...
        */
        this.$body.on('click', '.submit-disapprove-btn', function (e){
            ...
            var _error  = $.for_approval_issuance.validate(_form, 0);
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#issuanceTable .view-disapprove-btn', function (e) {
            var _self   = $(this);
            var _id     = $(this).closest('tr').attr('data-row-id');
            var _code   = $(this).closest('tr').attr('data-row-code');
            var _modal  = $('#disapprove-modal');
            ...
            _self.prop('disabled', true);
            var d1 = $.for_approval_issuance.fetch_status(_id);
            var d2 = $.for_approval_issuance.fetch_remarks(_id);
            $.when( d1, d2 ).done(function ( v1, v2 ) 
            {  
                if (v1 == 'cancelled') {
                    _self.prop('disabled', false);
        ...
        */
        this.$body.on('click', '#supplierTable .view-btn', function (e) {
            ...
            var _url      = _baseUrl + 'for-approvals/issuance/edit-supplier/' + _issuanceID + '?supplier=' + _supplier;
            var _modal    = $('#canvass-modal');
            
            _self.prop('disabled', true).html('<div class="spinner-border spinner-border-sm text-white" role="status" aria-hidden="true"><span class="visually-hidden">Loading...</span></div>');
            setTimeout(function () {
                $.for_approval_issuance.edit_supplier(_supplier, _url, _modal, _code, _self, _status);
            }, 500 + 300 * (Math.random() * 5));
        }); 
    }

    //init for_approval_issuance
    $.for_approval_issuance = new for_approval_issuance, $.for_approval_issuance.Constructor = for_approval_issuance

}(window.jQuery),

//initializing for_approval_issuance
function($) {
    "use strict";
    $.for_approval_issuance.required_fields();
    $.for_approval_issuance.init();
}(window.jQuery);