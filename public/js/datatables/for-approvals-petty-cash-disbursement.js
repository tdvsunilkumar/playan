!function($) {
    ...
    var for_approval_petty_cash_disbursement = function() {
        this.$body = $("body");
    };

    var _disburseID = 0; var _status = 'all';  var _codex; var _table; var _tableLine; 
    var _tablePage = 0, _linePage = 0;

    for_approval_petty_cash_disbursement.prototype.validate = function($form, $required)
    {   
        $required = 0;
    ...
    for_approval_petty_cash_disbursement.prototype.required_fields = function() {
    ...
    for_approval_petty_cash_disbursement.prototype.load_contents = function(_tablePage = 0) 
    {   
        _table = new DataTable('#pettyCashTable', {
            ajax: { 
                url : _baseUrl + 'for-approvals/petty-cash/disbursement/lists?status=' + _status,
                type: "GET", 
                data: {
                    "_token": _token
                },
                complete: function() {
                    $.for_approval_petty_cash_disbursement.shorten();
                    $('[data-bs-toggle="tooltip"]').tooltip({
                        trigger : 'hover'
                    });  
                    $.for_approval_petty_cash_disbursement.hideTooltip();
                }
            },
            language: {
        ...
        //     $('#pettyCashTable th input[type="checkbox"]').prop('checked', false);
        //     for (var i = 0; i < _tables.length; i++) {
        //         $('#pettyCashTable').find('tr[data-row-id="' +  _tables[i] + '"][data-row-status="draft"] input[type="checkbox"]').prop('checked', true);
        //     }
        // } );
    ...
    for_approval_petty_cash_disbursement.prototype.load_line_contents = function(_linePage = 0) 
    {   
        console.log(_baseUrl + 'for-approvals/petty-cash/disbursement/line-lists/' + _disburseID); 
        _tableLine = new DataTable('#pettyCashLineTable', {
            ajax: { 
                url : _baseUrl + 'for-approvals/petty-cash/disbursement/line-lists/' + _disburseID,
                type: "GET", 
                data: {
                    "_token": _token
                },
    ...
    for_approval_petty_cash_disbursement.prototype.validate_approver = function (_id, _sequence)
    {   
        var _status = false;
        console.log(_baseUrl + 'for-approvals/petty-cash/disbursement/validate-approver/' + _id + '/' + _sequence);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'for-approvals/petty-cash/disbursement/validate-approver/' + _id + '/' + _sequence,
    ...
    for_approval_petty_cash_disbursement.prototype.fetchID = function()
    {
        return _disburseID;
    }

    for_approval_petty_cash_disbursement.prototype.updateID = function(_id)
    {
        return _disburseID = _id;
    }

    for_approval_petty_cash_disbursement.prototype.fetch_status = function (_id)
    {   
        var _status = '';
        if (_id == 0) {
            return _status = 'draft';
        }
        console.log(_baseUrl + 'for-approvals/petty-cash/disbursement/fetch-status/' + _id);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'for-approvals/petty-cash/disbursement/fetch-status/' + _id,
            success: function(response) {
                console.log(response);
    ...
    for_approval_petty_cash_disbursement.prototype.fetch_remarks = function (_id)
    {   
        var _remarks = '';
        console.log(_baseUrl + 'for-approvals/petty-cash/disbursement/fetch-remarks/' + _id);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'for-approvals/petty-cash/disbursement/fetch-remarks/' + _id,
            success: function(response) {
                console.log(response);
    ...
    for_approval_petty_cash_disbursement.prototype.shorten = function() 
    {   
        if ($('.showLess')) {
            $('.showLess').shorten({
                "showChars" : 20,
    ...
    for_approval_petty_cash_disbursement.prototype.preload_select3 = function()
    {
        if ( $('.select3') ) {
    ...
    for_approval_petty_cash_disbursement.prototype.perfect_scrollbar = function()
    {
        if ($(".table-responsive")) {
    ...
    | ---------------------------------
    */
    for_approval_petty_cash_disbursement.prototype.price_separator = function(input) {
        var output = input
        if (parseFloat(input)) {
    ...
    for_approval_petty_cash_disbursement.prototype.hideTooltip = function()
    {
        $('.tooltip').remove();
    },
    ...
    */
    ...
    for_approval_petty_cash_disbursement.prototype.init = function()
    {   
        /*
        | ---------------------------------
        ...
        | ---------------------------------
        */
        $.for_approval_petty_cash_disbursement.preload_select3();
        $.for_approval_petty_cash_disbursement.load_contents();
        $.for_approval_petty_cash_disbursement.perfect_scrollbar();
        ...
        */
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#pettyCashTable .approve-btn', function (e) {
            var _self   = $(this);
            var _id     = $(this).closest('tr').attr('data-row-id');
            var _code   = $(this).closest('tr').attr('data-row-code');
            var _status = $(this).closest('tr').attr('data-row-status');
            var _sequence = $(this).closest('tr').attr('data-row-sequence');
            var _url    = _baseUrl + 'for-approvals/petty-cash/disbursement/approve/' + _id;

            var d1 = $.for_approval_petty_cash_disbursement.validate_approver(_id, _sequence);
            $.when( d1 ).done(function ( v1 ) 
            { 
                if (_status == 'pending' && v1 != false) {
                    _self.prop('disabled', true);
                    Swal.fire({
                        html: "Are you sure? <br/>the PPMP with <strong>code<br/>("+ _code +")</strong><br/>will be approved.",
                        icon: "warning",
                        showCancelButton: !0,
                        buttonsStyling: !1,
        ...
        */
        this.$body.on('click', '.submit-disapprove-btn', function (e){
            ...
            var _url    = _form.attr('action') + '/disapprove/' + _disburseID + '?remarks=' + encodeURIComponent(_form.find('textarea[name="disapproved_remarks"]').val());
            var _error  = $.for_approval_petty_cash_disbursement.validate(_form, 0);
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#pettyCashTable .disapprove-btn', function (e) {
            var _self   = $(this);
            var _id     = $(this).closest('tr').attr('data-row-id');
            var _code   = $(this).closest('tr').attr('data-row-code');
            var _status = $(this).closest('tr').attr('data-row-status');
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#pettyCashTable .view-disapprove-btn', function (e) {
            var _self   = $(this);
            var _id     = $(this).closest('tr').attr('data-row-id');
            var _code   = $(this).closest('tr').attr('data-row-code');
            var _modal  = $('#disapprove-modal');
            ...
            _self.prop('disabled', true);
            var d1 = $.for_approval_petty_cash_disbursement.fetch_status(_id);
            var d2 = $.for_approval_petty_cash_disbursement.fetch_remarks(_id);
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#pettyCashTable input[type="checkbox"][value="all"]', function (e) {
            var _self = $(this);
                                ...
                                if (response.type == 'success') {
                                    ...
                                    .then(
                                        function (e) {
                                            ...
                                            $.for_approval_petty_cash_disbursement.load_contents(_table.page());
                                        }
                                    );
                                } else {
                                    Swal.fire({
        ...
        */
        this.$body.on('click', '#pettyCashTable .edit-btn, #pettyCashTable .view-btn', function (e) {
            ...
            var _code   = $(this).closest('tr').attr('data-row-code');
            var _status = $(this).closest('tr').attr('data-row-status');
            var _url    = _baseUrl + 'for-approvals/petty-cash/disbursement/view/' + _id;
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $.for_approval_petty_cash_disbursement.load_line_contents();
                    if (_status != 'draft') {
                    ...
                    _modal.find('label[for="total-amount"] span').text('₱' + $.for_approval_petty_cash_disbursement.price_separator(parseFloat(Math.floor((response.total * 100))/100).toFixed(2)));
                    _modal.modal('show');
                },
                complete: function() {
                    window.onkeydown = null;
    ...
    //init for_approval_petty_cash_disbursement
    $.for_approval_petty_cash_disbursement = new for_approval_petty_cash_disbursement, $.for_approval_petty_cash_disbursement.Constructor = for_approval_petty_cash_disbursement

}(window.jQuery),

//initializing for_approval_petty_cash_disbursement
function($) {
    "use strict";
    $.for_approval_petty_cash_disbursement.required_fields();
    $.for_approval_petty_cash_disbursement.init();
}(window.jQuery);