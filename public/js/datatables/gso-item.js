!function($) {
    ...
    {   
        _table = new DataTable('#itemTable', {
            ajax: { 
                url : _baseUrl + 'general-services/setup-data/item-managements/lists',
                type: "GET", 
                data: {
                    "query": _keywords,
                    "_token": _token
    ...
    {   
        _fileTable = new DataTable('#itemUploadTable', {
            ajax: { 
                url : _baseUrl + 'general-services/setup-data/item-managements/upload-lists/' + _itemID,
                type: "GET", 
                data: {
                    "_token": _token
                },
    ...
    {   
        console.log(_baseUrl + 'general-services/setup-data/item-managements/conversion-lists/' + _itemID);
        _conversionTable = new DataTable('#itemConversionTable', {
            ajax: { 
                url : _baseUrl + 'general-services/setup-data/item-managements/conversion-lists/' + _itemID,
                type: "GET", 
                data: {
                    "_token": _token
                },
    ...
    */
    ...
    {   
        $.ajax({
            type: "GET",
            url: _baseUrl + 'general-services/setup-data/item-managements/fetch-gl-via-item-category/' + _id,
            success: function(response) {
                console.log(response.data);
                _modal.find('select[name="gl_account_id"]').val(response.data).trigger('change.select3'); 
            },
            async: false
        });
    },
    ...
    {   
        ...
        $.ajax({
            type: "GET",
            url: _baseUrl + 'general-services/setup-data/item-managements/fetch-based-uom/' + _id,
            success: function(response) {
                console.log(response.data);
    ...
    {   
        ...
        */
        ...
        var itemDropzone = new Dropzone('#import-item-dropzone', { 
            ...
            this.on("processing", function(file) {
                this.options.url = _baseUrl + 'general-services/setup-data/item-managements/upload/' + _itemID + '?category=items';
        ...
        */
        this.$body.on('click', '#itemTable .edit-btn', function (e) {
            ...
            var _modal  = $('#item-modal');
            var _url    = _baseUrl + 'general-services/setup-data/item-managements/edit/' + _id;
            console.log(_url);
            _itemID = _id;
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (parseFloat(response.validate) > 0) {
                        _modal.find('input[name="weighted_cost"], input[name="latest_cost"]').prop('disabled', true);
                    } else {
                        _modal.find('input[name="weighted_cost"], input[name="latest_cost"]').prop('disabled', false);
                    }
        ...
        */
        this.$body.on('click', '#itemTable .remove-btn, #itemTable .restore-btn', function (e) {
            ...
            var _url    = (_status == 'Active') ? _baseUrl + 'general-services/setup-data/item-managements/remove/' + _id : _baseUrl + 'general-services/setup-data/item-managements/restore/' + _id;
        ...
        */
        this.$body.on('click', '#itemUploadTable .download-btn', function (e) {
            ...
            var _url = _baseUrl + 'general-services/setup-data/item-managements/download/' + _itemID + '?category=items&file=' + _file;
        ...
        */
        this.$body.on('click', '#itemUploadTable .remove-btn', function (e) {
            ...
            var _file     = _row.attr('data-row-file');
            var _url      = _baseUrl + 'general-services/setup-data/item-managements/delete/' + _itemID + '?category=items&id=' + _id + '&file=' + _file;
        ...
        */
        this.$body.on('click', '#itemConversionTable .remove-btn, #itemConversionTable .restore-btn', function (e) {
            ...
            var _code   = $(this).closest('tr').attr('data-row-code');
            var _status = $(this).closest('tr').attr('data-row-status');
            var _url    = (_status == 'Active') ? _baseUrl + 'general-services/setup-data/item-managements/remove-conversion/' + _id : _baseUrl + 'general-services/setup-data/item-managements/restore-conversion/' + _id;
        ...
        */
        this.$body.on('click', '#itemConversionTable .edit-btn', function (e) {
            ...
            var _modal  = $('#item-conversion-modal');
            var _url    = _baseUrl + 'general-services/setup-data/item-managements/edit-conversion/' + _id;
            console.log(_url);
            _conversionID = _id;
            _self.prop('disabled', true).attr('title', 'Loading...').html('<div class="spinner-border spinner-border-sm text-white" role="status" aria-hidden="true"><span class="visually-hidden">Loading...</span></div>');
            $.ajax({
