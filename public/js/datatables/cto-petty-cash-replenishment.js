!function($) {
    ...
    var _replenishmentID = 0; var _breakdownID = 0; var _table; var _tableLine; 
    ...
    {   
        console.log(_baseUrl + 'treasury/petty-cash/replenishment/lists'); 
        _table = new DataTable('#replenishTable', {
            ajax: { 
                url : _baseUrl + 'treasury/petty-cash/replenishment/lists',
                type: "GET", 
                data: {
                    "_token": _token
                },
    ...
    {   
        console.log(_baseUrl + 'treasury/petty-cash/replenishment/line-lists/' + _replenishmentID); 
        _tableLine = new DataTable('#replenishLineTable', {
            ajax: { 
                url : _baseUrl + 'treasury/petty-cash/replenishment/line-lists/' + _replenishmentID,
                type: "GET", 
                data: {
                    "_token": _token
                },
            ...
            initComplete: function(){
                ...
                if(_tableLine.rows().data().length > 0) {
                    ...
                    $('input[name="payee_id"]').prop('disabled', true);
                } else {
                    $('select[name="voucher_id"]').prop('disabled', false);
                    $('input[name="payee_id"]').prop('disabled', false);
                }       
            }, 
            columns: [
                { data: 'id' },
    ...
    {
        return _replenishmentID;
    }

    cto_petty_cash.prototype.updateID = function(_id)
    {
        return _replenishmentID = _id;
    }
    ...
    */
    ...
    {   
        ...
        */
        ...
        | ---------------------------------
        */
        this.$body.on('hidden.bs.modal', '#replenish-modal', function (e) {
            var _modal = $(this);
        ...
        */
        this.$body.on('click', '#replenishTable .edit-btn, #replenishTable .view-btn', function (e) {
            ...
            var _url    = _baseUrl + 'treasury/petty-cash/replenishment/edit/' + _id;
            var _modal  = $('#replenish-modal');
            _replenishmentID = _id;
            console.log(_url);
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    _modal.find('.modal-header h5').html('Edit Replenishment (<span class="variables">' + _code + '</span>)');
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#replenishLineTable .remove-btn', function (e) {
            var _id   = $(this).closest('tr').attr('data-row-id');
            var _code = $(this).closest('tr').attr('data-row-code');
            var _url  = _baseUrl + 'treasury/petty-cash/replenishment/remove-line/' + _id;
            ...
            {   
                if (v1 == 'draft') {
                    Swal.fire({
                        html: "Are you sure? <br/>the replenish detail with code ("+ _code +") will be removed.",
                        icon: "warning",
                        showCancelButton: !0,
                        buttonsStyling: !1,
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#replenishTable .send-btn', function (e){
            e.preventDefault();
            var _self   = $(this);
            var _id     = $(this).closest('tr').attr('data-row-id');
            var _code   = $(this).closest('tr').attr('data-row-code');
            ...
            if (_status == 'draft' && parseFloat(_total) > 0) {
                ...
                Swal.fire({
                    html: "Are you sure? <br/>the replenishment with <strong>Control No<br/>("+ _code +")</strong> will be sent.",
                    icon: "warning",
                    showCancelButton: !0,
                    buttonsStyling: !1,
        ...
        */
        this.$body.on('click', '.print-btn', function (e) {
            ...
            var _obr  = _self.closest('tr').attr('data-row-obr');
            var d1 = $.cto_petty_cashForm.fetch_status(_id);
            $.when( d1 ).done(function ( v1 ) 
            {   
                if (v1 == 'completed' && _obr !=='') {
                    var _url  = _baseUrl + 'treasury/petty-cash/replenishment/print/' + _obr;
