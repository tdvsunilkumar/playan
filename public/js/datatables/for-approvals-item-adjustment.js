!function($) {
    ...
    var for_approval_adjustment = function() {
        this.$body = $("body");
    };

    var _issuanceID = 0; var _table; var _itemTable; var _projectName = ''; var _supplierStatus = ''; var _supplierID = 0; var _page = 0;

    for_approval_adjustment.prototype.validate = function($form, $required)
    {   
        $required = 0;
    ...
    for_approval_adjustment.prototype.required_fields = function() {
    ...
    for_approval_adjustment.prototype.load_contents = function(_page = 0) 
    {   
        console.log(_baseUrl + 'for-approvals/item-adjustment/lists'); 
        _table = new DataTable('#adjustmentTable', {
            ajax: { 
                url : _baseUrl + 'for-approvals/item-adjustment/lists',
                type: "GET", 
                data: {
                    "_token": _token
                },
                complete: function() {
                    $.for_approval_adjustment.shorten();
            ...
            initComplete: function(){
                ...
                $.for_approval_adjustment.hideTooltip();
            }, 
            columns: [
                { data: 'id' },
    ...
    for_approval_adjustment.prototype.load_item_contents = function(_keywords = '') 
    {   
        console.log(_baseUrl + 'for-approvals/item-adjustment/item-lists/' + _issuanceID); 
        _itemTable = new DataTable('#itemTable', {
            ajax: { 
                url : _baseUrl + 'for-approvals/item-adjustment/item-lists/' + _issuanceID,
                type: "GET", 
                data: {
                    "query": _keywords,
                    "_token": _token
    ...
    for_approval_adjustment.prototype.fetchID = function()
    {
        return _issuanceID;
    }

    for_approval_adjustment.prototype.updateID = function(_id)
    {
        return _issuanceID = _id;
    }

    for_approval_adjustment.prototype.fetch_status = function (_id)
    {   
        var _status = '';
        if (_id == 0) {
            return _status = 'draft';
        }
        console.log(_baseUrl + 'for-approvals/item-adjustment/fetch-status/' + _id);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'for-approvals/item-adjustment/fetch-status/' + _id,
    ...
    for_approval_adjustment.prototype.validate_approver = function (_id)
    {   
        var _status = false;
        console.log(_baseUrl + 'for-approvals/item-adjustment/validate-approver/' + _id);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'for-approvals/item-adjustment/validate-approver/' + _id,
            success: function(response) {
                console.log(response);
                _status = response;
            },
    ...
    for_approval_adjustment.prototype.shorten = function() 
    {   
        if ($('.showLess')) {
            $('.showLess').shorten({
                "showChars" : 20,
    ...
    for_approval_adjustment.prototype.preload_select3 = function()
    {
        if ( $('.select3') ) {
    ...
    for_approval_adjustment.prototype.perfect_scrollbar = function()
    {
        if ($(".table-responsive")) {
    ...
    for_approval_adjustment.prototype.validate_table = function($table)
    {   
        $('.pager').remove();
        $table.each(function() {
            var currentPage = 0;
    ...
    | ---------------------------------
    */
    for_approval_adjustment.prototype.price_separator = function(input) {
        var output = input
        if (parseFloat(input)) {
    ...
    for_approval_adjustment.prototype.fetch_remarks = function (_id)
    {   
        var _remarks = '';
        console.log(_baseUrl + 'for-approvals/item-adjustment/fetch-remarks/' + _id);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'for-approvals/item-adjustment/fetch-remarks/' + _id,
            success: function(response) {
                console.log(response);
    ...
    for_approval_adjustment.prototype.load_line_contents = function(_keywords = '') 
    {   
    ...
    for_approval_adjustment.prototype.load_alob_line_contents = function(_keywords = '') 
    {   
        var table = new DataTable('#allotmentBreakdownTable', {
            ajax: { 
                url : _baseUrl + 'for-approvals/item-adjustment/alob-lists/' + _issuanceID,
                type: "GET", 
                data: {
                    "query": _keywords,
                    "_token": _token
    ...
    for_approval_adjustment.prototype.reload_divisions_employees = function(_division, _employee, _designation, _department = 0)
    {   
        // if (_department > 0) {
            _employee.find('option').remove(); _division.find('option').remove(); _designation.val('');
            console.log(_baseUrl + 'for-approvals/item-adjustment/reload-divisions-employees/' + _department);
            $.ajax({
                type: "GET",
                url: _baseUrl + 'for-approvals/item-adjustment/reload-divisions-employees/' + _department,
                success: function(response) {
    ...
    for_approval_adjustment.prototype.reload_designation = function(_designation, _employee = 0)
    {   
        // if (_employee > 0) {
            _designation.find('option').remove(); 
            console.log(_baseUrl + 'for-approvals/item-adjustment/reload-designation/' + _employee);
            $.ajax({
                type: "GET",
                url: _baseUrl + 'for-approvals/item-adjustment/reload-designation/' + _employee,
                success: function(response) {
    ...
    for_approval_adjustment.prototype.reload_items = function(_item, _uom, _purchaseType = 0)
    {   
        // if (_purchaseType > 0) {
            _item.find('option').remove(); _uom.val('');
            console.log(_baseUrl + 'for-approvals/item-adjustment/reload-items/' + _purchaseType);
            $.ajax({
                type: "GET",
                url: _baseUrl + 'for-approvals/item-adjustment/reload-items/' + _purchaseType,
                success: function(response) {
                    console.log(response.data);
    ...
    for_approval_adjustment.prototype.fetch_total_allotment_amount = function()
    {   
        var _total = 0;
        var _url   = _baseUrl + 'for-approvals/item-adjustment/fetch-allotment-via-pr/' + _issuanceID + '?column=total_amount'; 
        console.log(_url);
        $.ajax({
            type: 'GET',
            url: _url,
    ...
    for_approval_adjustment.prototype.hideTooltip = function()
    {
        $('.tooltip').remove();
    },

    for_approval_adjustment.prototype.edit_supplier = function(_supplier, _url, _modal, _code, _button, _status)
    {   
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.data.length > 0) {
                    ...
                    _table.find('tfoot th.text-danger').text($.for_approval_adjustment.price_separator(parseFloat(_totalCost).toFixed(2)));
                    if (_status !== 'pending') {
    ...
    for_approval_adjustment.prototype.init = function()
    {   
        /*
        | ---------------------------------
        ...
        | ---------------------------------
        */
        $.for_approval_adjustment.preload_select3();
        $.for_approval_adjustment.load_contents();
        $.for_approval_adjustment.perfect_scrollbar();
        ...
        */
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#adjustmentTable .view-btn', function (e) {
            var _self   = $(this);
            ...
            var _url    = _baseUrl + 'for-approvals/item-adjustment/edit/' + _id;
            console.log(_url);
            _issuanceID = _id;
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    _modal.find('.modal-header h5').html('View Adjustment (<span class="variables">' + _code + '</span>)');
                    _modal.modal('show');
                },
                complete: function() {
                    window.onkeydown = null;
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#adjustmentTable .approve-btn', function (e) {
            var _self   = $(this);
            var _id     = $(this).closest('tr').attr('data-row-id');
            var _code   = $(this).closest('tr').attr('data-row-code');
            var _modal  = $('#purchase-order-modal');
            var _controlNo = _modal.find('#rfq_id'); _controlNo.find('option').remove(); 
            var _url    = _baseUrl + 'for-approvals/item-adjustment/approve/' + _id;

            var d1 = $.for_approval_adjustment.fetch_status(_id);
            var d2 = $.for_approval_adjustment.validate_approver(_id);
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#adjustmentTable .disapprove-btn', function (e) {
            var _self   = $(this);
            var _id     = $(this).closest('tr').attr('data-row-id');
            var _code   = $(this).closest('tr').attr('data-row-code');
        ...
        */
        this.$body.on('click', '.submit-disapprove-btn', function (e){
            ...
            var _url = _form.attr('action') + '/disapprove/' + _issuanceID;
            var _error  = $.for_approval_adjustment.validate(_form, 0);
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#adjustmentTable .view-disapprove-btn', function (e) {
            var _self   = $(this);
            var _id     = $(this).closest('tr').attr('data-row-id');
            var _code   = $(this).closest('tr').attr('data-row-code');
            var _modal  = $('#disapprove-modal');
            ...
            _self.prop('disabled', true);
            var d1 = $.for_approval_adjustment.fetch_status(_id);
            var d2 = $.for_approval_adjustment.fetch_remarks(_id);
            $.when( d1, d2 ).done(function ( v1, v2 ) 
            {  
                if (v1 == 'cancelled') {
                    _self.prop('disabled', false);
        ...
        */
        this.$body.on('click', '#supplierTable .view-btn', function (e) {
            ...
            var _url      = _baseUrl + 'for-approvals/item-adjustment/edit-supplier/' + _issuanceID + '?supplier=' + _supplier;
            var _modal    = $('#canvass-modal');
            
            _self.prop('disabled', true).html('<div class="spinner-border spinner-border-sm text-white" role="status" aria-hidden="true"><span class="visually-hidden">Loading...</span></div>');
            setTimeout(function () {
                $.for_approval_adjustment.edit_supplier(_supplier, _url, _modal, _code, _self, _status);
            }, 500 + 300 * (Math.random() * 5));
        }); 
    }

    //init for_approval_adjustment
    $.for_approval_adjustment = new for_approval_adjustment, $.for_approval_adjustment.Constructor = for_approval_adjustment

}(window.jQuery),

//initializing for_approval_adjustment
function($) {
    "use strict";
    $.for_approval_adjustment.required_fields();
    $.for_approval_adjustment.init();
}(window.jQuery);