!function($) {
    ...
    var for_approval_ppmp = function() {
        this.$body = $("body");
    };

    var _ppmpID = 0; var _ppmp; var _ppmpStatus = 'all'; var _ppmpPage = 0; var _codex = '';

    for_approval_ppmp.prototype.validate = function($form, $required)
    {   
        $required = 0;
    ...
    for_approval_ppmp.prototype.required_fields = function() {
    ...
    for_approval_ppmp.prototype.load_contents = function(_ppmpPage = 0) 
    {   
        _ppmp = new DataTable('#ppmpTable', {
            ajax: { 
                url : _baseUrl + 'for-approvals/ppmp/lists?status=' + _ppmpStatus,
                type: "GET", 
                data: {
                    "_token": _token
                },
                complete: function() {
                    $.for_approval_ppmp.shorten();
                    $('[data-bs-toggle="tooltip"]').tooltip({
                        trigger : 'hover'
                    });  
                    $.for_approval_ppmp.hideTooltip();
                }
            },
            language: {
            ...
            initComplete: function(){
                this.api().page(_ppmpPage).draw( 'page' );   
            },      
            columns: [
                { data: 'id' },
        ...
        // _ppmp.on( 'draw', function () {
        //     $('#ppmpTable th input[type="checkbox"]').prop('checked', false);
        //     for (var i = 0; i < _ppmps.length; i++) {
        //         $('#ppmpTable').find('tr[data-row-id="' +  _ppmps[i] + '"][data-row-status="draft"] input[type="checkbox"]').prop('checked', true);
        //     }
        // } );
    ...
    for_approval_ppmp.prototype.validate_approver = function (_id, _sequence)
    {   
        var _status = false;
        console.log(_baseUrl + 'for-approvals/ppmp/validate-approver/' + _id + '/' + _sequence);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'for-approvals/ppmp/validate-approver/' + _id + '/' + _sequence,
    ...
    for_approval_ppmp.prototype.fetchID = function()
    {
        return _ppmpID;
    }

    for_approval_ppmp.prototype.updateID = function(_id)
    {
        return _ppmpID = _id;
    }

    for_approval_ppmp.prototype.fetch_status = function (_id)
    {   
        var _status = '';
        if (_id == 0) {
            return _status = 'draft';
        }
        console.log(_baseUrl + 'for-approvals/ppmp/fetch-status/' + _id);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'for-approvals/ppmp/fetch-status/' + _id,
            success: function(response) {
                console.log(response);
    ...
    for_approval_ppmp.prototype.fetch_remarks = function (_id)
    {   
        var _remarks = '';
        console.log(_baseUrl + 'for-approvals/ppmp/fetch-remarks/' + _id);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'for-approvals/ppmp/fetch-remarks/' + _id,
            success: function(response) {
                console.log(response);
    ...
    for_approval_ppmp.prototype.shorten = function() 
    {   
        if ($('.showLess')) {
            $('.showLess').shorten({
                "showChars" : 20,
    ...
    for_approval_ppmp.prototype.preload_select3 = function()
    {
        if ( $('.select3') ) {
    ...
    for_approval_ppmp.prototype.perfect_scrollbar = function()
    {
        if ($(".table-responsive")) {
    ...
    | ---------------------------------
    */
    for_approval_ppmp.prototype.price_separator = function(input) {
        var output = input
        if (parseFloat(input)) {
    ...
    for_approval_ppmp.prototype.hideTooltip = function()
    {
        $('.tooltip').remove();
    },

    for_approval_ppmp.prototype.init = function()
    {   
        /*
        | ---------------------------------
        ...
        | ---------------------------------
        */
        $.for_approval_ppmp.preload_select3();
        $.for_approval_ppmp.load_contents();
        $.for_approval_ppmp.perfect_scrollbar();
        ...
        */
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#ppmpTable .approve-btn', function (e) {
            var _self   = $(this);
            var _id     = $(this).closest('tr').attr('data-row-id');
            var _code   = $(this).closest('tr').attr('data-row-code');
            var _status = $(this).closest('tr').attr('data-row-status');
            var _sequence = $(this).closest('tr').attr('data-row-sequence');
            var _url    = _baseUrl + 'for-approvals/ppmp/approve/' + _id;

            var d1 = $.for_approval_ppmp.validate_approver(_id, _sequence);
        ...
        */
        this.$body.on('click', '.submit-disapprove-btn', function (e){
            ...
            var _url    = _form.attr('action') + '/disapprove/' + _ppmpID + '?remarks=' + encodeURIComponent(_form.find('textarea[name="disapproved_remarks"]').val());
            var _error  = $.for_approval_ppmp.validate(_form, 0);
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#ppmpTable .disapprove-btn', function (e) {
            var _self   = $(this);
            var _id     = $(this).closest('tr').attr('data-row-id');
            var _code   = $(this).closest('tr').attr('data-row-code');
            var _status = $(this).closest('tr').attr('data-row-status');
        ...
        */
        this.$body.on('click', '#ppmpTable .view-disapprove-btn', function (e) {
            ...
            _self.prop('disabled', true);
            var d1 = $.for_approval_ppmp.fetch_status(_id);
            var d2 = $.for_approval_ppmp.fetch_remarks(_id);
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#ppmpTable input[type="checkbox"][value="all"]', function (e) {
            var _self = $(this);
            ...
            if (_self.is(':checked')) {
                ...
                $.each(_table.find('tr[data-row-status="pending"] input[type="checkbox"][value!="all"]'), function(){
                    ...
                    if (checkbox.is(":checked")) {
                        var found = false;
                        for (var i = 0; i < _ppmps.length; i++) {
                            if (_ppmps[i] == checkbox.val()) {
                                found == true;
                                return;
                            }
                        } 
                        if (found == false) {
                            _ppmps.push(checkbox.val());
                        }
                    } 
                });
            } else {
        ...
        */
        this.$body.on('click', '#ppmpTable input[type="checkbox"][value!="all"]', function (e) {
            var _self = $(this);
            if (_self.is(':checked')) {
                _ppmps.push(_self.val());
            } else {
                for (var i = 0; i < _ppmps.length; i++) {
                    if (_ppmps[i] == _self.val()) {
                        _ppmps.splice(i, 1);
                    }
                }
            }
        ...
        */
        this.$body.on('click', '#approved-btn', function (e) {
            ...
            if (_ppmps.length > 0) {
                ...
                _self.prop('disabled', true);
                Swal.fire({
                    html: "Are you sure? <br/>the (" + _ppmps.length + ") selected payments <br/>will be approved.",
                    icon: "warning",
                    showCancelButton: !0,
                    buttonsStyling: !1,
                    confirmButtonText: "Yes, approve it!",
                                ...
                                if (response.type == 'success') {
                                    ...
                                    .then(
                                        function (e) {
                                            ...
                                            $.for_approval_ppmp.load_contents(_ppmp.page());
                                        }
                                    );
                                } else {
                                    Swal.fire({
    ...
    //init for_approval_ppmp
    $.for_approval_ppmp = new for_approval_ppmp, $.for_approval_ppmp.Constructor = for_approval_ppmp

}(window.jQuery),

//initializing for_approval_ppmp
function($) {
    "use strict";
    $.for_approval_ppmp.required_fields();
    $.for_approval_ppmp.init();
}(window.jQuery);