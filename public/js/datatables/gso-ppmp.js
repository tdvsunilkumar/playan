!function($) {
    ...
    gso_ppmp.prototype.load_years = function()
    {   
        _YrList = '<select name="ppmp_year" class="form-select form-select-sm ms-1">';
        $.ajax({
            type: 'GET',
            url: _baseUrl + 'general-services/project-procurement-management-plan/year-lists',
    ...
    gso_ppmp.prototype.load_contents = function(_page = 0) 
    {   
        _table = new DataTable('#ppmpTable', {
            ajax: { 
                url : _baseUrl + 'general-services/project-procurement-management-plan/lists?status=' + encodeURIComponent(_status) + '&year=' + _Yr,
                type: "GET", 
                data: {
                    "_token": _token
                },
            ...
            createdRow: function( row, data, dataIndex ) {
                ...
                $(row).attr('data-row-app-status', data.budget_status);
                $(row).attr('data-row-position', data.order);
            },
            dom: 'lf<"toolbar-2 float-end d-flex flex-row">rtip',
            initComplete: function(){
                this.api().page(_page).draw( 'page' );   
                $("div.toolbar-2").html('<label class="d-inline-flex ms-2 line-30">Year: ' + _YrList + '</label><label class="d-inline-flex ms-2 line-30">Status:<select name="ppmp_status" aria-controls="ppmp_status" class="ms-2 form-select form-select-sm d-inline-flex"><option value="draft">Draft</option><option value="for approval">For Approval</option><option value="locked">Locked</option><option value="all">All</option></label>');           
    ...
    gso_ppmp.prototype.fetchID = function()
    {
        return _ppmpID;
    }

    gso_ppmp.prototype.updateID = function(_id)
    {
        return _ppmpID = _id;
    }

    gso_ppmp.prototype.shorten = function() 
    {   
        if ($('.showLess')) {
            $('.showLess').shorten({
                "showChars" : 20,
    ...
    gso_ppmp.prototype.preload_select3 = function()
    {
        if ( $('.select3') ) {
    ...
    gso_ppmp.prototype.perfect_scrollbar = function()
    {
        if ($(".table-responsive")) {
    ...
    | ---------------------------------
    */
    gso_ppmp.prototype.price_separator = function(input) {
        var output = input
        if (parseFloat(input)) {
    ...
    gso_ppmp.prototype.preload_yearpicker = function()
    {
        if ( $('#budget_year') ) {
    ...
    gso_ppmp.prototype.getLastSegment = function()
    {
    ...
    gso_ppmp.prototype.getIdentity = function ()
    {   
        _ppmpID = 0;
        var _card = $('#ppmp-card');
        console.log(_baseUrl + 'general-services/project-procurement-management-plan/get-identity');
        $.ajax({
            type: "GET",
            url: _baseUrl + 'general-services/project-procurement-management-plan/get-identity',
            success: function(response) {
    ...
    gso_ppmp.prototype.hideTooltip = function()
    {
        $('.tooltip').remove();
    },

    gso_ppmp.prototype.validate_ppmp = function()
    {   
        var _validate = 0;
    ...
    gso_ppmp.prototype.fetch_status = function (_id)
    {   
        var _status = '';
        if (_id == 0) {
    ...
    gso_ppmp.prototype.fetch_ppmp = function (_id)
    {   
        var _status = '';
        if (_id == 0) {
            return _status = 'draft';
        }
        console.log(_baseUrl + 'general-services/project-procurement-management-plan/find/' + _id);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'general-services/project-procurement-management-plan/find/' + _id,
            success: function(response) {
                console.log(response);
                _status = response.budget_status;
            },
            async: false
        });
        return _status;
    ...
    gso_ppmp.prototype.fetch_remarks = function (_id)
    {   
        var _remarks = '';
        console.log(_baseUrl + 'general-services/project-procurement-management-plan/fetch-remarks/' + _id);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'general-services/project-procurement-management-plan/fetch-remarks/' + _id,
            success: function(response) {
                console.log(response);
    ...
    gso_ppmp.prototype.validate_item_removal = function (_id) 
    {
        var _validate = false;
        console.log(_baseUrl + 'general-services/project-procurement-management-plan/validate-item-removal/' + _id);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'general-services/project-procurement-management-plan/validate-item-removal/' + _id,
            success: function(response) {
                console.log(response);
    ...
    gso_ppmp.prototype.init = function()
    {   
        /*
        | ---------------------------------
        ...
        | ---------------------------------
        */
        if($.gso_ppmp.getLastSegment() == 'manage' || $.gso_ppmp.getLastSegment() == 'add' || $.gso_ppmp.getLastSegment() == 'edit') {
            $.gso_ppmp.getIdentity();
            $.gso_ppmp.perfect_scrollbar();
        }
        $.gso_ppmp.preload_select3();
        $.gso_ppmp.load_contents();
        $.gso_ppmp.preload_yearpicker();
        ...
        */
        this.$body.on('keyup', '#datatable-2 input[type="search"]', function (e) {
            ...
            if (e.which == 13) {
                var d1 = $.gso_ppmp.load_contents(_self.val());
        ...
        */
        this.$body.on('hidden.bs.modal', '#ppmp-modal', function (e) {
            ...
            _modal.find('table thead').empty(); _modal.find('table tbody').empty();
            $.gso_ppmp.load_contents();
            $.gso_ppmp.hideTooltip();
            _ppmpID = 0;
        });
        ...
        */
        this.$body.on('click', '#ppmp-lock-btn', function (e) {
            ...
            var _url     = _baseUrl + 'general-services/project-procurement-management-plan/lock-division/' + _id + '?division=' + $('#division_id').val();
            var d1       = $.gso_ppmpForm.fetch_status(_id);
            var d2       = $.gso_ppmpForm.fetch_division_status(_id, $('#division_id').val());
            var d3       = $.gso_ppmp.validate_ppmp();
        ...
        */
        this.$body.on('click', '#ppmp-table .remove-btn', function (e) {
            ...
            var _url    = _baseUrl + 'general-services/project-procurement-management-plan/remove-lines/' + _id;
            var d1      = $.gso_ppmpForm.fetch_status(_ppmpID);
            var d2      = $.gso_ppmp.validate_item_removal(_id);
        ...
        */
        this.$body.on('click', '#ppmpTable .view-disapprove-btn', function (e) {
            ...
            _self.prop('disabled', true);
            var d1 = $.gso_ppmp.fetch_status(_id);
            var d2 = $.gso_ppmp.fetch_remarks(_id);
            $.when( d1, d2 ).done(function ( v1, v2 ) 
            {  
                if (v1 == 'cancelled') {
                    _ppmpID = _id;
        ...
        */
        this.$body.on('click', '.copy-btn', function (e) {
            ...
            var _url   = _baseUrl + 'general-services/project-procurement-management-plan/find/' + _id;
            _ppmpID = _id;
        ...
        */
        this.$body.on('click', '#ppmpTable .unlock-btn', function (e) {
            var _id     = $(this).closest('tr').attr('data-row-id');
            var _code   = $(this).closest('tr').attr('data-row-code');
            var _url    = _baseUrl + 'general-services/project-procurement-management-plan/unlock/' + _id;

            var d1 = $.gso_ppmp.fetch_ppmp(_id);
            $.when( d1 ).done(function ( v1 ) 
            {   
                if (v1 = 'locked') {
                    Swal.fire({
                        title: "Oops...",
                        html: "Unable to unlock!<br/>The request is already been processed.",
                        icon: "error",
                        type: "danger",
                        showCancelButton: false,
                        closeOnConfirm: true,
                    ...
                    console.log(_url);
                    Swal.fire({
                        html: "Are you sure? <br/>the PPMP with control no ("+ _code +") will be unlocked.",
                        icon: "warning",
                        showCancelButton: !0,
                        buttonsStyling: !1,
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#ppmpTable .send-btn', function (e) {
            var _id     = $(this).closest('tr').attr('data-row-id');
            var _code   = $(this).closest('tr').attr('data-row-code');
            ...
            {   
                if (v2 == false) {
                    ...
                    Swal.fire({
                        html: "Are you sure? <br/>the PPMP with control no ("+ _code +") will be sent.",
                        icon: "warning",
                        showCancelButton: !0,
                        buttonsStyling: !1,
    ...
    //init gso_ppmp
    $.gso_ppmp = new gso_ppmp, $.gso_ppmp.Constructor = gso_ppmp

}(window.jQuery),

//initializing gso_ppmp
function($) {
    "use strict";
    $.gso_ppmp.load_years();
    $.gso_ppmp.required_fields();
    $.gso_ppmp.init();
}(window.jQuery);