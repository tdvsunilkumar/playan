!function($) {
    ...
    var for_approval_cemetery_application = function() {
        this.$body = $("body");
    };

    var _econCemeteryID = 0; var _table; var _page = 0; var _status = 'all';

    for_approval_cemetery_application.prototype.validate = function($form, $required)
    {   
        $required = 0;
    ...
    for_approval_cemetery_application.prototype.required_fields = function() {
    ...
    for_approval_cemetery_application.prototype.load_contents = function(_page = 0) 
    {   
        console.log(_baseUrl + 'for-approvals/economic-and-investment/cemetery-application/lists'); 
        _table = new DataTable('#econCemeteryTable', {
            ajax: { 
                url : _baseUrl + 'for-approvals/economic-and-investment/cemetery-application/lists',
                type: "GET", 
                data: {
                    "_token": _token
                },
                complete: function (data) {  
                    $.for_approval_cemetery_application.shorten(); 
                    $('[data-bs-toggle="tooltip"]').tooltip({
                        trigger : 'hover'
                    });  
                    $.for_approval_cemetery_application.hideTooltip();
                }
            },
            language: {
    ...
    for_approval_cemetery_application.prototype.validate_approver = function (_id)
    {   
        var _status = false;
        console.log(_baseUrl + 'for-approvals/economic-and-investment/cemetery-application/validate-approver/' + _id);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'for-approvals/economic-and-investment/cemetery-application/validate-approver/' + _id,
    ...
    for_approval_cemetery_application.prototype.fetchID = function()
    {
        return _econCemeteryID;
    }

    for_approval_cemetery_application.prototype.updateID = function(_id)
    {
        return _econCemeteryID = _id;
    }

    for_approval_cemetery_application.prototype.fetch_status = function (_id)
    {   
        var _status = '';
        if (_id == 0) {
            return _status = 'draft';
        }
        console.log(_baseUrl + 'for-approvals/economic-and-investment/cemetery-application/fetch-status/' + _id);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'for-approvals/economic-and-investment/cemetery-application/fetch-status/' + _id,
            success: function(response) {
                console.log(response);
    ...
    for_approval_cemetery_application.prototype.fetch_remarks = function (_id)
    {   
        var _remarks = '';
        console.log(_baseUrl + 'for-approvals/economic-and-investment/cemetery-application/fetch-remarks/' + _id);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'for-approvals/economic-and-investment/cemetery-application/fetch-remarks/' + _id,
            success: function(response) {
                console.log(response);
    ...
    for_approval_cemetery_application.prototype.shorten = function() 
    {   
        if ($('.showLess')) {
            $('.showLess').shorten({
                "showChars" : 20,
    ...
    for_approval_cemetery_application.prototype.preload_select3 = function()
    {
        if ( $('.select3') ) {
    ...
    for_approval_cemetery_application.prototype.perfect_scrollbar = function()
    {
        if ($(".table-responsive")) {
    ...
    | ---------------------------------
    */
    for_approval_cemetery_application.prototype.price_separator = function(input) {
        var output = input
        if (parseFloat(input)) {
    ...
    for_approval_cemetery_application.prototype.hideTooltip = function()
    {
        $('.tooltip').remove();
    },

    for_approval_cemetery_application.prototype.reload_cemetery_name = function(_modal, _location)
    {   
    ...
    for_approval_cemetery_application.prototype.reload_cemetery_lot = function(_id, _modal, _location, _cemetery, _style)
    {   
    ...
    for_approval_cemetery_application.prototype.init = function()
    {   
        /*
        | ---------------------------------
        ...
        | ---------------------------------
        */
        $.for_approval_cemetery_application.preload_select3();
        $.for_approval_cemetery_application.load_contents();
        $.for_approval_cemetery_application.perfect_scrollbar();
        ...
        */
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#econCemeteryTable .approve-btn', function (e) {
            var _self   = $(this);
            var _id     = $(this).closest('tr').attr('data-row-id');
            var _code   = $(this).closest('tr').attr('data-row-code');
            var _modal  = $('#cemetery-modal');
            var _controlNo = _modal.find('#rfq_id'); _controlNo.find('option').remove(); 
            var _url    = _baseUrl + 'for-approvals/economic-and-investment/cemetery-application/approve/' + _id;

            var d1 = $.for_approval_cemetery_application.fetch_status(_id);
            var d2 = $.for_approval_cemetery_application.validate_approver(_id);
        ...
        */
        this.$body.on('click', '.submit-disapprove-btn', function (e){
            ...
            var _url = _form.attr('action') + '/disapprove/' + _econCemeteryID;
            var _error  = $.for_approval_cemetery_application.validate(_form, 0);
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#econCemeteryTable .view-btn', function (e) {
            var _self   = $(this);
            var _id     = _self.closest('tr').attr('data-row-id');
            var _code   = _self.closest('tr').attr('data-row-code');
            var _modal  = $('#cemetery-modal');
            var _url    = _baseUrl + 'for-approvals/economic-and-investment/cemetery-application/view/' + _id;
            console.log(_url);
            _econCemeteryID = _id;
            ...
            $.ajax({
                ...
                success: function(response) {
                    console.log(response);
                    var d1 = $.for_approval_cemetery_application.reload_cemetery_name(_modal, response.data.location_id);
                    var d2 = $.for_approval_cemetery_application.reload_cemetery_lot(_id, _modal, response.data.location_id, response.data.cemetery_id, response.data.cemetery_style_id);
                    $.when( d1, d2 ).done(function (v1, v2) { 
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#econCemeteryTable .disapprove-btn', function (e) {
            var _self   = $(this);
            var _id     = $(this).closest('tr').attr('data-row-id');
            var _code   = $(this).closest('tr').attr('data-row-code');
        ...
        */
        this.$body.on('click', '#econCemeteryTable .view-disapprove-btn', function (e) {
            ...
            _self.prop('disabled', true);
            var d1 = $.for_approval_cemetery_application.fetch_status(_id);
            var d2 = $.for_approval_cemetery_application.fetch_remarks(_id);
            $.when( d1, d2 ).done(function ( v1, v2 ) 
            {  
                if (v1 == 'cancelled') {
                    _econCemeteryID = _id;
    ...
    //init for_approval_cemetery_application
    $.for_approval_cemetery_application = new for_approval_cemetery_application, $.for_approval_cemetery_application.Constructor = for_approval_cemetery_application

}(window.jQuery),

//initializing for_approval_cemetery_application
function($) {
    "use strict";
    $.for_approval_cemetery_application.required_fields();
    $.for_approval_cemetery_application.init();
}(window.jQuery);