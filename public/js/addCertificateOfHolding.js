$(document).ready(function () {
    ...
    select3Ajaxorno("rpc_or_no","rpc_or_no_group","certOfPropertyholdinggetOrNumberajax");
...
//     if ($(this).is(":checked")) {
//         var id = [];
//         $('.clientdata:checked').each(function() {
//             id.push($(this).val());
//         });
//         getClient(id);
//         getClientMech(id);
...
// // You can trigger this initially if some checkboxes are checked on page load
// $('.clientdata:checked').each(function() {
//     var id = $(this).val();
//     var rpc_code=$("#id").val();
//     getClient(id,rpc_code);
...
//     // alert('Clicked');
//     var recordid = $(button).attr('id');
//     DeleteRecord(recordid);
// }
function getOrNumber(id){
   $.ajax({
 
        url :DIR+'getctoCashierDetailsPropertyHolding', // json datasource
        type: "POST", 
        data: {
          "id": id, 
...
/* function DeleteRecord(id){
  // alert(id);
         ...
         swalWithBootstrapButtons.fire({
             ...
             {
              $.ajax({
              url :DIR+'rptpropertycertofpropertyholding/deletes', // json datasource
              type: "POST", 
              data: {
                "id": id, 
                "_token": $("#_csrf_token").val(),
...
function dataSearch(lastName) {
    $.ajax({
        type: "POST",
        url: DIR + 'getClientCodeAddressPropertyLand',
        data: {
...
function getClientMech(id){
  ...
  $.ajax({
      type: "POST",
      url: DIR+'getClientsAppraisalsMech',
      data: filtervars,
...
function getClientBuilding(id){
  ...
  $.ajax({
      type: "POST",
      url: DIR+'getClientsAppraisalsBuilding',
      data: filtervars,
...
function loadBuildingStructureForm() {
  ...
  $.ajax({
      type: "post",
      url: DIR+'rptpropertycertofpropertyholding/client',
      data: filtervars,
      dataType: "html",
      success: function(html){ 
