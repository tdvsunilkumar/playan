$(document).ready(function(){
    if($('#id').val() > 0){
        $('select[name=top_transaction_id]').find(':not(:selected)').prop('disabled',true);
    }
    ...
    $('input[name=isuserrange]').click(function() {
        $("#jqPaidAmount").attr("disabled",false);
        var checked = '0'; 
    ...
    $('input[name=submit_form]').click(function(e) {
        checkOrUsedOrNot();
...
function loadAcceptedTds() {
    var id = $('#top_transaction_id').val();
    showLoader();
    var url = DIR+'online-payment-history/loadacceptedtds';
     $.ajax({
        url :url, // json datasource
        type: "get", 
        dataType: "html", 
...
function loadCasheiringInfo() {
    showLoader();
    var id = $('#top_transaction_id').val();
    var url = DIR+'online-payment-history/loadcasheringinfo';
     $.ajax({
        url :url, // json datasource
        type: "get", 
        dataType: "html", 
 ...
 function checkOrUsedOrNot(){
      $.ajax({
        ...
        success: function(data){
            if(data.isUsed){
                $("#err_or_no_new").html(data.errMsg);
