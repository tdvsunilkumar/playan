$(document).ready(function(){	
	...
	$('#rptPropertySearchByRange').on('change',function(){
		$('#rangeSerachFilter').submit();
	});

	$('#rptPropertySearchByText').on('keyup',function(){
		$('#rangeSerachFilter').submit();
	});
	$('#rangeSerachFilter').on('submit',function(e){
		e.preventDefault();
		datatablefunction();
	});
...
function datatablefunction()
{
	...
	var table = $('#Jq_datatablelist').DataTable({ 
		...
		"pageLength": 10,
		"ajax":{ 
			url :DIR+'healthy-and-safety/reports/morbid-cases/getList', // json datasource
			type: "GET", 
			"data": {
				"q":$("#rptPropertySearchByText").val(),
				'range':$('#rptPropertySearchByRange').val(),
...
function DeleteRecord(id){
	// alert(id);
        ...
        swalWithBootstrapButtons.fire({
            ...
            {
               $.ajax({
			        url :DIR+'reporange/delete', // json datasource
			        type: "POST", 
			        data: {
			          "id": id, 
			          "_token": $("#_csrf_token").val(),
			        ...
			        success: function(html){
			        	Swal.fire({
	    				  position: 'center',
	    				  icon: 'success',
	    				  title: 'Reporting Range Deleted Successfully.',
...
function ActiveInactiveUpdate(id,is_activeinactive){
   ...
   swalWithBootstrapButtons.fire({
	   ...
	   {
		  $.ajax({
			   url :DIR+'reporange/reporangeActiveInactive', // json datasource
			   type: "POST", 
			   data: {
				 "id": id,
