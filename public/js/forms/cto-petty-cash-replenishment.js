!function($) {
    ...
    var $required = 0; var _disbursements = []; 
    ...
    {   
        ...
        console.log(_baseUrl + 'treasury/petty-cash/replenishment/fetch-status/' + _id);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'treasury/petty-cash/replenishment/fetch-status/' + _id,
    ...
    {   
        ...
        var _url = _baseUrl + 'treasury/petty-cash/replenishment/update/' + _id + '?particulars=' + encodeURIComponent(_form.find('textarea[name="particulars"]').val());
        console.log(_id);
        $.ajax({
            type: 'PUT',
            url: _url,
    ...
    cto_petty_cashForm.prototype.view_available_disbursements = function(_id, _button, _fund)
    {   
        var _table = $('#available-disbursement-table'); _table.find('tbody').empty();
        var _modal = _table.closest('.modal');
        var _rows  = '';
        var _url = _baseUrl + 'treasury/petty-cash/replenishment/view-available-disbursements/' + _id;
        console.log(_url);
        $.ajax({
            type: 'GET',
            url: _url,
            success: function(response) {
                ...
                if (response.data.length > 0) {
                    $.each(response.data, function(i, row) {
                        ...
                        _rows += '<td>' + (row.control_no ? row.control_no : '') + '</td>';
                        _rows += '<td>' + (row.particulars ? row.particulars : '') + '</td>';
    ...
    {   
        ...
        */
        ...
        | ---------------------------------
        */
        this.$body.on('hidden.bs.modal', '#add-disbursement-modal', function (e) {
            var _modal = $(this); _disbursements = [];
        ...
        */
        this.$body.on('keyup', '#keyword1', function (event) {
            ...
            table = document.getElementById("available-disbursement-table");
            ...
            if (input.value.length > 0) {
               ...
               $.cto_petty_cash.validate_table($('#available-disbursement-table'));
            }
        });
        this.$body.on('search', '#add-disbursement-modal input[type="search"]', function (e) {
            $.cto_petty_cash.validate_table($('#available-disbursement-table'));
        });
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#add-disbursement-modal input[type="checkbox"][value="all"]', function (e) {
            var _self = $(this);
            ...
            if (_self.is(':checked')) {
                ...
                $.each(_modal.find('input[type="checkbox"][value!="all"]'), function(){
                    ...
                    if (checkbox.is(":checked")) {
                        var found = false;
                        for (var i = 0; i < _disbursements.length; i++) {
                            if (_disbursements[i] == checkbox.val()) {
                                found == true;
                                return;
                            }
                        } 
                        if (found == false) {
                            _disbursements.push(checkbox.val());
                        }
                    } 
                });
            } else {
                ...
                $.each(_modal.find("input[type='checkbox'][value!='all']"), function(){
                    ...
                    for (var i = 0; i < _disbursements.length; i++) {
                        if (_disbursements[i] == checkbox.val()) {
                            _disbursements.splice(i, 1);
                        }
                    }
                });
            }
            console.log(_disbursements);
        });
        /*
        | ---------------------------------
        ...
        */
        this.$body.on('click', '#add-disbursement-modal input[type="checkbox"][value!="all"]', function (e) {
            var _self = $(this);
            if (_self.is(':checked')) {
                _disbursements.push(_self.val());
            } else {
                for (var i = 0; i < _disbursements.length; i++) {
                    if (_disbursements[i] == _self.val()) {
                        _disbursements.splice(i, 1);
                    }
                }
            }
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#add-disbursement-modal button', function (e) {
            e.preventDefault();
            var _self = $(this);
            var _modal = _self.closest('.modal');
            var _id = $.cto_petty_cash.fetchID();
            ...
            if (_disbursements.length > 0) {
                ...
                {  
                    if (v1 == 'draft') {
                        $.ajax({
                            type: 'POST',
                            url: _baseUrl + 'treasury/petty-cash/replenishment/add-line/' + _id,
                            data: {'disbursements' : _disbursements},
                            success: function(response) {
                                console.log(response);
                                if (response.type == 'success') {
                                    if (_id <= 0) {
                                    ...
                                    setTimeout(function () {
                                        $('#replenish-modal').find('label[for="total-amount"] span').text('₱' + $.cto_petty_cash.price_separator(parseFloat(Math.floor((response.total * 100))/100).toFixed(2)));
        ...
        | ---------------------------------
        */
        this.$body.on('change', 'form[name="replenishForm"] select:not(:disabled)', function (e) {
            var _self = $(this);
            var _form = _self.closest('form');
            var _id   = $.cto_petty_cash.fetchID();
            var d1    = $.cto_petty_cashForm.fetch_status(_id);
        ...
        | ---------------------------------
        */
        this.$body.on('blur', 'form[name="replenishForm"] textarea:not(:disabled)', function (e) {
            var _self = $(this);
            var _form = _self.closest('form');
            var _id   = $.cto_petty_cash.fetchID();
            var d1    = $.cto_petty_cashForm.fetch_status(_id);
        ...
        // | ---------------------------------
        // */
        // this.$body.on('click', 'form[name="replenishForm"] button.send-btn', function (e) {
        //     e.preventDefault();
        //     var _self      = $(this);
        //     var _modal     = _self.closest('.modal');
        //     var _form      = _self.closest('form');
        ...
        */
            ...
            {  
                if (v1 == 'draft') {
                    if (_error != 0) {
                        ...
                        setTimeout(function () {
                            $.cto_petty_cashForm.view_available_disbursements(_id, _self,);
                        }, 500 + 300 * (Math.random() * 5));
                    }
                } else {
                    Swal.fire({
        ...
        | ---------------------------------
        */
        this.$body.on('click', '#replenish-modal .send-btn', function (e){
            e.preventDefault();
            var _self  = $(this);
            var _modal = _self.closest('.modal');
            var _toast = $('#modalToast');
            var _id    = $.cto_petty_cash.fetchID();            
            var _url   = _baseUrl + 'treasury/petty-cash/replenishment/send/for-approval/' + _id;
