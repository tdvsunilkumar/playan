!function($) {
    ...
    var acctg_trial_balance = function() {
        this.$body = $("body");
    };

    var $required = 0;

    acctg_trial_balance.prototype.required_fields = function() {
    ...
    acctg_trial_balance.prototype.validate = function($form, $required)
    {   
        $required = 0;
    ...
    acctg_trial_balance.prototype.preload_select3 = function()
    {
        if ( $('.select3') ) {
    ...
    acctg_trial_balance.prototype.reload_account_code = function(_type, _acctCode)
    {   
        _acctCode.find('option').remove(); 
        console.log(_baseUrl + 'reports/accounting/trial-balance/reload?type=' + _type);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'reports/accounting/trial-balance/reload?type=' + _type,
    ...
    acctg_trial_balance.prototype.reload_category_name = function(_category, _name)
    {   
        _name.find('option').remove(); 
        console.log(_baseUrl + 'reports/accounting/trial-balance/reload-category-name?category=' + _category);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'reports/accounting/trial-balance/reload-category-name?category=' + _category,
            success: function(response) {
    ...
    acctg_trial_balance.prototype.init = function()
    {   
        $.acctg_trial_balance.preload_select3();
        ...
        */
        this.$body.on('change', 'select[name="ledger_type"]', function (e) {
            ...
            var _self = $(this);
            var _account = $('#code');
            $.acctg_trial_balance.reload_account_code(_self.val(), _account);
        });
        ...
        */
        this.$body.on('change', 'select[name="category"]', function (e) {
            ...
            var _self = $(this);
            var _name = $('#name');
            $.acctg_trial_balance.reload_category_name(_self.val(), _name);
        });
        ...
        */
        this.$body.on('click', '.submit-btn', function (e){
            e.preventDefault();
            var _self   = $(this);
            var _form   = $('form[name="reportsAcctgTrialBalanceForm"]');
            var _action = _form.attr('action') + '/export/' + _form.find('select[name="export_as"]').val() + '?' + _form.serialize();
            var _error  = $.acctg_trial_balance.validate(_form, 0);
    ...
    //init acctg_trial_balance
    $.acctg_trial_balance = new acctg_trial_balance, $.acctg_trial_balance.Constructor = acctg_trial_balance

}(window.jQuery),

//initializing acctg_trial_balance
function($) {
    "use strict";
    $.acctg_trial_balance.required_fields();
    $.acctg_trial_balance.init();
}(window.jQuery);
