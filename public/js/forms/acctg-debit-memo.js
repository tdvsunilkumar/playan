!function($) {
    ...
    {   
        ...
        // Validate Amount pay
            console.log($('#add-payment-modal').find('#amount').val());
        var _amount_pay = $('#add-payment-modal').find('#amount')
        
        var _disburse_type = $('#disburse_type_id').find('option:selected').text()
        var _total_pay = null;
        var _pay_type = '';
        switch (true) {
            case _disburse_type.toLowerCase().includes('salaries'):
                _total_pay = $('#total-salaries').text();
                _pay_type = 'Salaries'
                break;
        
            case _disburse_type.toLowerCase().includes('pag-ibig'):
                _total_pay = $('#total-pagibig').text();
                _pay_type = 'Pag-ibig'
                break;
        
            case _disburse_type.toLowerCase().includes('philhealth'):
                _total_pay = $('#total-philhealth').text();
                _pay_type = 'Philhealth'
                break;
        
            case _disburse_type.toLowerCase().includes('bir'):
                _total_pay = $('#total-bir').text();
                _pay_type = 'BIR'
                break;
    
            case _disburse_type.toLowerCase().includes('gsis'):
                _total_pay = $('#total-gsis').text();
                _pay_type = 'GSIS'
                break;
        ...
        if (_total_pay && parseFloat(_total_pay.replace(/,/g, '')) != parseFloat(_amount_pay.val().replace(/,/g, '')) ) {
            _amount_pay.addClass('is-invalid');
            _amount_pay.next('.m-form__help').text('Must be equal to Total '+_pay_type)
            $required++;                                          
        }
        return $required;
    },
    ...
    {   
        ...
        var _replenish =  ($('input[name=is_replenish]:checked').length > 0) ? 1 : 0;
        var _url = _baseUrl + 'accounting/journal-entries/' + voucherSegment + '/update/' + _id + '?replenish=' + _replenish + '&fund_code=' + _form.find('select[name="fund_code_id"]').val() + '&payee_id=' + _form.find('select[name="payee_id"]').val() + '&remarks=' + _form.find('textarea[name="remarks"]').val();
        console.log(_url);
        $.ajax({
            type: 'PUT',
            url: _url,
    ...
    {   
        ...
        */
        ...
        /*
        | ---------------------------------
        | # when replenish on click
        | ---------------------------------
        */
        this.$body.on('click', 'input[name="is_replenish"]', function (event) {
            var _self = $(this);
            var _form = $('form[name="voucherForm"]');
            var _id   = $.voucher.fetchID();
            var d1    = $.voucherForm.fetch_status(_id);
            $.when( d1 ).done(function ( v1) 
            {  
            ...
            if (_payables.length > 0) {
                ...
                {  
                    if (v1 == 'draft') {
                        $.ajax({
                            type: 'POST',
                            url: _baseUrl + 'accounting/journal-entries/' + voucherSegment + '/add-payables/' + _id,
        ...
        */
        ...
        | ---------------------------------
        */
        this.$body.on('change', '#add-payment-modal #sl_account_id', function (event) {
            var _self = $(this);
            var _modal = _self.closest('.modal');
            if (_self.val() > 0) {
                $.ajax({
                    type: 'GET',
                    url: _baseUrl + 'accounting/journal-entries/' + voucherSegment + '/find-sl-bank/' + _self.val(),
                    success: function(response) {
        ...
        */
        this.$body.on('click', '#add-date-modal .submit-btn', function (e){
            ...
            var _urlPrint  = _baseUrl + 'accounting/journal-entries/' + voucherSegment + '/print/' + _voucher + '?type=' + _modal.find('.variables').text(); 
