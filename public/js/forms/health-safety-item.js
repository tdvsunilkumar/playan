!function($) {
    ...
    {   
        _code.val('');
        console.log(_baseUrl + 'health-and-safety/setup-data/item-managements/generate-item-code/' + _itemCategory);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'health-and-safety/setup-data/item-managements/generate-item-code/' + _itemCategory,
    ...
    itemForm.prototype.validateCategory = function ()
    {
        var _error = 0;
        $.ajax({
            type: 'GET',
            url: _baseUrl + 'health-and-safety/setup-data/item-managements/validate-category/' + $('#item-modal select[name="item_category_id"]').val(),
            success: function(response) {
                console.log(response);
    ...
    {   
        ...
        */
        this.$body.on('click', '#item-modal .submit-btn', function (e){
            ...
            var _expiry  = (_form.find('input[type="checkbox"][name="is_expirable"]:checked').length  > 0) ? 1 : 0;
            var _action  = (_id > 0) ? _form.attr('action') + '/update/' + _id + '?is_expirable=' + _expiry + '&item_category_id=' + _itemCat + '&gl_account_id=' + _glAcct : _form.attr('action') + '/store?is_expirable=' + _expiry + '&item_category_id=' + _itemCat + '&gl_account_id=' + _glAcct;
            var _error   = $.itemForm.validate(_form, 0);
            var _error2  = $.itemForm.validateCategory();
            ...
            if (_error != 0) {
                ...
                Swal.fire({
                    title: "Oops...",
                    html: "Unable to proceed!<br/>Please select health & safety item category.",
                    icon: "warning",
                    type: "warning",
                    showCancelButton: false,
                    closeOnConfirm: true,
        ...
        */
        this.$body.on('click', '#item-conversion-modal .submit-btn', function (e) {
            ...
            var _action       = (_conversionID > 0) ? _baseUrl + 'health-and-safety/setup-data/item-managements/update-conversion/' + _conversionID : _form.attr('action') + '/' + _id;
