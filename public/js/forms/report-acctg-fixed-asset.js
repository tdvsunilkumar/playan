!function($) {
    ...
    var acctg_fixed_asset = function() {
        this.$body = $("body");
    };

    var $required = 0;

    acctg_fixed_asset.prototype.required_fields = function() {
    ...
    acctg_fixed_asset.prototype.validate = function($form, $required)
    {   
        $required = 0;
    ...
    acctg_fixed_asset.prototype.preload_select3 = function()
    {
        if ( $('.select3') ) {
    ...
    acctg_fixed_asset.prototype.reload_account_code = function(_type, _acctCode)
    {   
        _acctCode.find('option').remove(); 
        console.log(_baseUrl + 'reports/accounting/fixed-assets/reload?type=' + _type);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'reports/accounting/fixed-assets/reload?type=' + _type,
    ...
    acctg_fixed_asset.prototype.reload_category_name = function(_category, _name)
    {   
        _name.find('option').remove(); 
        console.log(_baseUrl + 'reports/accounting/fixed-assets/reload-category-name?category=' + _category);
        $.ajax({
            type: "GET",
            url: _baseUrl + 'reports/accounting/fixed-assets/reload-category-name?category=' + _category,
            success: function(response) {
    ...
    acctg_fixed_asset.prototype.init = function()
    {   
        $.acctg_fixed_asset.preload_select3();
        ...
        */
        this.$body.on('change', 'select[name="category"]', function (e) {
            ...
            $.acctg_fixed_asset.required_fields();
        });
        ...
        */
        this.$body.on('click', '.submit-btn', function (e){
            e.preventDefault();
            var _self   = $(this);
            var _form   = $('form[name="reportsAcctgFixedAssetForm"]');
            var _action = _form.attr('action') + '/export/' + _form.find('select[name="export_as"]').val() + '?' + _form.serialize();
            var _error  = $.acctg_fixed_asset.validate(_form, 0);
    ...
    //init acctg_fixed_asset
    $.acctg_fixed_asset = new acctg_fixed_asset, $.acctg_fixed_asset.Constructor = acctg_fixed_asset

}(window.jQuery),

//initializing acctg_fixed_asset
function($) {
    "use strict";
    $.acctg_fixed_asset.required_fields();
    $.acctg_fixed_asset.init();
}(window.jQuery);
