...
function datatablefunction()
{
	...
	var table = $('#Jq_datatablelist').DataTable({ 
		...
		"pageLength": 10,
		"ajax":{ 
			url :DIR+'cron-job/getList', // json datasource
			type: "GET", 
			"data": {
				"q":$("#q").val(),
...
function quickRunCron(cid) {
   ...
   swalWithBootstrapButtons.fire({
   	...
   	if(result.isConfirmed){
   		showLoader();
		  	$.ajax({
			   url :DIR+'cron-job/quickRunCron', // json datasource
			   type: "POST", 
			   data: {
				 "id": cid,
				 "_token": $("#_csrf_token").val(),
...
function ActiveInactiveUpdate(id,is_activeinactive){
   ...
   swalWithBootstrapButtons.fire({
	   ...
	   reverseButtons: true
	   	...
	   	if(result.isConfirmed){
		  	$.ajax({
			   url :DIR+'cron-job/ActiveInactive', // json datasource
			   type: "POST", 
			   data: {
				 "id": id,
