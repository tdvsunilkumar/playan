...
function datatablefunction()
{
	...
	var table = $('#Jq_datatablelist').DataTable({ 
		...
		"pageLength": 5,
		"ajax":{ 
			url :DIR+'rpt-tax-collection/getList', // json datasource
			type: "GET", 
			"data": {
				/*"q":$("#q").val(),*/
        ...
        "columns": [
			...
			{ "data": "or_no" },
        	{ "data": "full_name" },
        	{ "data": "cbd_assessed_value" },
        	{ "data": "currentYearBasicTax", className: "right_side"},
            { "data": "previousYearBasicTax" , className: "right_side"},
            { "data": "priorYearBasicTaxes" , className: "right_side"},
            { "data": "currentYearSefTax" , className: "right_side"},
            { "data": "previousYearSefTax" , className: "right_side"},
            { "data": "priorYearSefTaxes" , className: "right_side"},
            { "data": "currentYearShtTax" , className: "right_side"},
            { "data": "previousYearShtTax" , className: "right_side"},
            { "data": "priorYearShtTaxes" , className: "right_side"},

            { "data": "penalty", className: "right_side" },
            { "data": "taxCredit" , className: "right_side"},
            { "data": "advancePayment" , className: "right_side"},
            { "data": "total_paid_amount" , className: "right_side"}
        ],
        ...
        footerCallback: function(tfoot, data, start, end, display) {
         var api = this.api();
         for (var i = 4; i <= 17; i++) {
             var p = api.column(i).data().reduce(function (a, b) {
                return parseFloat(a) + parseFloat(b.replace(/\,/g,''));
            }, 0)
            $(api.column(i).footer()).html(parseFloat(p).toFixed(2));
         }
    },
	});  
}