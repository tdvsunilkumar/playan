...
function datatablefunction()
{
	...
	var table = $('#Jq_datatablelist').DataTable({ 
		...
		"pageLength": 10,
		"ajax":{ 
			url :DIR+'rptupdatecode/getList', // json datasource
			type: "GET", 
			"data": {
				"q":$("#q").val(),
				'year':$('#yeardate').val(),
        ...
        "columns": [
			...
			{ "data": "uc_description" },
        	{ "data": "uc_usage_land" },
        	{ "data": "uc_usage_building" },
        	{ "data": "uc_usage_machine" },
        	{ "data": "stp_print" },
            { "data": "is_active" },
            { "data": "action" }
        ],
...
function ActiveInactiveUpdate(id,is_activeinactive){
   ...
   swalWithBootstrapButtons.fire({
	   ...
	   {
		  $.ajax({
			   url :DIR+'rptupdatecode/ActiveInactive', // json datasource
			   type: "POST", 
			   data: {
				 "id": id,
...
function PrintOptionUpdate(id,is_print){
    ...
    swalWithBootstrapButtons.fire({
        title: 'Are you sure?',
        text: "You wont to update New/Fresh?",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonText: 'Yes',
        cancelButtonText: 'No',
        ...
        {
           $.ajax({
		        url :DIR+'rptupdatecode/PrintOptionUpdateCode', // json datasource
		        type: "POST", 
		        data: {
		          "id": id,
		          "is_print": is_print,  
		        ...
		        success: function(html){
		        	Swal.fire({
    				  position: 'center',
    				  icon: 'success',
    				  title: 'New/Fresh Update Successfully.',
    				  showConfirmButton: false,
    				  timer: 1500
    				})
		           location.reload();
