$(document).ready(function(){	
	...
	$("#btn_search").click(function(){
	 ...
	 $("#tfoc_id").on("change", function(){
		var id = document.getElementById("tfoc_id");
		if(id.value != "")
		{
			feeByid(id.value);
		}
...
{   
	var fees = document.getElementById("fees");
		$.ajax({
			type: "GET",
			url: _baseUrl + 'BploEndorsingDept/reload-fees/' + id,
			success: function(response) {
				console.log(response.data);
				fees.value = response.description;
			},
			async: false
		});
}
...
function datatablefunction()
{
	...
	var table = $('#Jq_datatablelist').DataTable({ 
		...
		"pageLength": 10,
		"ajax":{ 
			url :DIR+'BploEndorsingDept/getList', // json datasource
			type: "GET", 
			"data": {
				"q":$("#q").val(),
   ...
   swalWithBootstrapButtons.fire({
	   ...
	   reverseButtons: true
	   	...
	   	if(result.isConfirmed){
		  	$.ajax({
			   url :DIR+'BploEndorsingDept/ActiveInactive', // json datasource
			   type: "POST", 
			   data: {
				 "id": id,
