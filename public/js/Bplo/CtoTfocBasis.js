...
function datatablefunction()
{
	...
	var table = $('#Jq_datatablelist').DataTable({ 
		...
		"pageLength": 10,
		"ajax":{ 
			url :DIR+'bploCtoTfocBasis/getList', // json datasource
			type: "GET", 
			"data": {
				"q":$("#q").val(),
       ...
       "columns": [
			{ "data": "srno" },
        	{ "data": "basis_name" },
         { "data": "basis_ref_table" },
         { "data": "basis_ref_field" },
         { "data": "basis_is_retire" },
        	{ "data": "basis_status" },
        	{ "data": "action" }
        ],
   ...
   swalWithBootstrapButtons.fire({
	   ...
	   reverseButtons: true
	   	...
	   	if(result.isConfirmed){
		  	$.ajax({
			   url :DIR+'bploCtoTfocBasis/ActiveInactive', // json datasource
			   type: "POST", 
			   data: {
				 "id": id,
