$(document).ready(function(){
   $('#status').select3({dropdownAutoWidth : false,dropdownParent: $("#multiCollapseExample1")});	
	datatablefunction();
...
function datatablefunction()
{
	...
	var table = $('#Jq_datatablelist').DataTable({ 
		...
		"pageLength": 10,
		"ajax":{ 
			url :DIR+'CtoPaymentOrAssignment/getList', // json datasource
			type: "GET", 
			"data": {
				"q":$("#q").val(),
				"status":$("#status").val(),
				"accountref":$("#accountref").val(),
       ...
       "columns": [
        	...
        	{ "data": "ora_to" },
        	{ "data": "or_count" },
        	{ "data": "ora_is_completed" },
        	{ "data": "ora_date_issued" },
        	{ "data": "ora_completed_date" },
        	{ "data": "ora_date_returned" },
        	{ "data": "ora_cashier" },
        	{ "data": "ora_remarks" },
        	{ "data": "ora_is_active" },
        	{ "data": "action" }
   ...
   swalWithBootstrapButtons.fire({
	   ...
	   reverseButtons: true
	   	...
	   	if(result.isConfirmed){
		  	$.ajax({
			   url :DIR+'CtoPaymentOrAssignment/ActiveInactive', // json datasource
			   type: "POST", 
			   data: {
				 "id": id,
