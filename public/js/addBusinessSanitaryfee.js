$(document).ready(function () {
    ...
    if($("#id").val()>0){
      var currentid = $('input[type=radio][name="bsf_fee_option"]:checked').val();
       if(currentid =='2'){ var bsf_cagetory_code=0;
          if($("#id").val()>0){ 
           var bsf_cagetory_code = $('#bsf_cagetory_code').val();
          }
          getByCategoryDropdown(bsf_cagetory_code);
        }
     ...
     $('input[type=radio][name="bsf_fee_option"]').change(function() {
        var id = $(this).val(); 
...
function commonhideshowdiv(id){
        ...
        if(currentval =='1' || currentval =='4'){
            ...
            $('#bsf_fee_amount').prop('required', true);
         ...
         if(currentval =='2'){
            ...
            $('#byareadiv').addClass('hide');
            $('#bsf_fee_amount').prop('required', false);
...
function getByCategoryDropdown(id){
  ...
  $.ajax({
      type: "POST",
      url: DIR+'fees-master/business-sanitary-fee/getSanitaryCategoryDropdown',
      data: {
...
function getByAreaDropdown(amt){
  ...
  $.ajax({
      type: "POST",
      url: DIR+'fees-master/business-sanitary-fee/getSanitaryAreaDropdown',
      data: {
          "classificationid":classificationid, 
          "activityid":activityid,
