$(document).ready(function(){
    ...
    //select3Ajax("flt_busn_office_barangay","barangaydiv","engjobrequest/getBarngayList");
	$("#service").select3({dropdownAutoWidth : false,dropdownParent: $("#multiCollapseExample1")});
 ...
 function printpermit(id,serviceid){
              var appliid = id;
              $.ajax({
                url: DIR+'engjobrequest/printpermit',
                type: 'POST',
                data: {
                    "id": appliid, "serviceid":serviceid,"_token": $("#_csrf_token").val(),
                },
...
function syncapplicationtoremote(id){
     var filtervars = {
        ...
        $.ajax({
          type: "POST",
          url: DIR+'engjobrequestonline/syncapptoremote',
          data: filtervars,
          dataType: "json",
...
function datatablefunction()
{
	...
	var table = $('#Jq_datatablelist').DataTable({ 
		...
		"searching": false,
		"order": [],
		"columnDefs": [{ orderable: false, targets: [0,3,10,7,8,10,11,12,13] }],
		"pageLength": 10,
		"ajax":{ 
			url :DIR+'engjobrequest/getList', // json datasource
			type: "GET", 
			"data": {
				"q":$("#q").val(),
...
function DeleteRecord(id){
	alert(id);
        ...
        swalWithBootstrapButtons.fire({
            ...
            {
               $.ajax({
			        url :DIR+'engjobrequest/delete', // json datasource
			        type: "POST", 
			        data: {
			          "id": id, 
			          "_token": $("#_csrf_token").val(),
...
function ActiveInactiveUpdate(id,is_activeinactive){
   ...
   swalWithBootstrapButtons.fire({
	   ...
	   {
		  $.ajax({
			   url :DIR+'engjobrequest/ActiveInactive', // json datasource
			   type: "POST", 
			   data: {
				 "id": id,
