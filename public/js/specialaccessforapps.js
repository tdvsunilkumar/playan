$(document).ready(function(){	
	...
	$('#menu_description').on('change', function () {
        datatablefunction();
	});	
...
function datatablefunction()
{
	...
	var table = $('#Jq_datatablelist').DataTable({ 
		...
		"pageLength": 10,
		"ajax":{ 
			url :DIR+'special-access-for-apps/getList', // json datasource
			type: "GET", 
			"data": {
				"q":$("#q").val(),
				"menu_group_id":$("#menu_description").val(),
   ...
   swalWithBootstrapButtons.fire({
	   ...
	   reverseButtons: true
	   	...
	   	if(result.isConfirmed){
		  	$.ajax({
			   url :DIR+'special-access-for-apps/ActiveInactive', // json datasource
			   type: "POST", 
			   data: {
				 "id": id,
